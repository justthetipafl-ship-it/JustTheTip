/* JTT NBL signals module — basketball Degen engines in the shell's JTTSignals.create() format.
...
window.JTTSignals = (function () {
  ...
  function create(deps) {
    ...
    function slate() {
      var fs = _fixtureSet();
      return (players || []).filter(function (p) { return fs.has(p.team) && (p.games || p.matches || 0) >= MIN_GAMES; });
    }
    ...
    function streak() {
      ...
      slate().forEach(function (p) {
        ...
        var line = Math.max(10, Math.round((p.points || 0) * 0.7));
        var s = 0; for (var i = 0; i < lg.length; i++) { if ((+(lg[i].points || 0)) >= line) s++; else break; }
        if (s >= 5) out.push({ p: p, opp: nextOpp(p.team), mkt: 'points', sc: s,
          headline: s + '-game ' + line + '+ pts', detail: 'hot streak' });
      });
    ...
    function form() {
      ...
      slate().forEach(function (p) {
        ...
        if (l5 == null || season == null || season < 8) return;
        var delta = (l5 - season) / season * 100;
        if (delta >= 18) out.push({ p: p, opp: nextOpp(p.team), mkt: 'points', sc: delta,
          headline: 'L5 ' + l5.toFixed(1) + ' pts', detail: '+' + Math.round(delta) + '% vs season' });
      });
    ...
    // ---- double trouble: double-double frequency ----
    function dd() {
      var out = [];
      slate().forEach(function (p) {
        var near = [p.points, p.rebounds, p.assists].filter(function (v) { return v != null && v >= 8; }).length;
        if (near < 2) return;
        var lg = recentLogs(p.name).slice(0, 10); if (!lg.length) return;
        var rate = lg.filter(function (r) {
          return [+r.points || 0, +r.rebounds || 0, +r.assists || 0].filter(function (x) { return x >= 10; }).length >= 2;
        }).length / lg.length;
        if (rate >= 0.4) out.push({ p: p, opp: nextOpp(p.team), mkt: 'pra', sc: rate * 100,
          headline: Math.round(rate * 100) + '% DD (L10)',
          detail: (p.points || 0).toFixed(0) + '/' + (p.rebounds || 0).toFixed(0) + '/' + (p.assists || 0).toFixed(0) });
      });
      return wrap('ti-stack-3', 'Double Trouble', out, 'c-soft', 'No double-double watch on the slate.');
    }
    ...
    function collectOU(teamList, kind) {
      ...
      var MARKETS = [['points', 'Points'], ['rebounds', 'Rebounds'], ['assists', 'Assists'],
    ...
    var tiles = {
      ...
      slash: function () { return specialist('points', 'ti-sword', 'Slashers'); },
      splash: function () { return specialist('threes', 'ti-droplet', 'Splashers'); },
      dish: function () { return specialist('assists', 'ti-hand-finger', 'Dishers'); },
      board: function () { return specialist('rebounds', 'ti-wall', 'Boarders'); },
      bunnies: function () { return h2hEngine(true); },
      bogey: function () { return h2hEngine(false); },
      streak: streak, form: form, dd: dd, usage: usage
    };
