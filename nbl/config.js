...
window.JTTScoring = (function () {
  ...
  function prob(p, statKey, line, opp){
    ...
    const all = dvpByName(p.name).slice().sort((a,b) => String(a.Date||'').localeCompare(String(b.Date||'')));
  ...
  function configure(ctx){
    ...
    MIN_FLOOR=ctx.minutesFloor!=null?ctx.minutesFloor:12;
    LEAGUE=ctx.league||'nbl';
    BA=_avgsOf(TD);
    TF._avgs=_avgsOf(TF);
    buildPDPoolAvgs();
