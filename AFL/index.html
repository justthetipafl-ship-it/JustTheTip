...
<body class="auth-locked">
  ...
  const SPORTS = {
    afl: { key:'afl', dir:'AFL', name:'AFL', oddsUrl:'https://jtt-afl-odds.justthetipafl.workers.dev/odds.json', liveWorker:'https://health.justthetipafl.workers.dev' },
    // nfl: { key:'nfl', dir:'nfl', name:'NFL', oddsUrl:'https://jtt-nfl-odds.justthetipafl.workers.dev/odds.json', liveWorker:'' },  // ← un-comment to flip NFL live once its full config is migrated
  };
  ...
  function teamDegen(team, opp){
    ...
    try{
      ...
      _degenTileKeys().filter(k=>!_fxHid('sig',k)).forEach(k=>{ html+=renderTile(k); });
  ...
  function _pmRender(){
    ...
    document.getElementById('pm-modal').innerHTML=
      ...
      (opp?_pmSec('Similar Players',similarPlayers(p,_pcState?_pcState.stat:'disposals',opp,_pcState?_pcState.line:NaN,6,_pcState?_pcState.side:'over'),false):'');
  }
  ...
  function _jInRange(d){ const S=_jS(); if(S.oddsMin==null&&S.oddsMax==null) return true; if(S.sel[d.p.name]) return true; if(d.price==null) return false; return (S.oddsMin==null||d.price>=S.oddsMin)&&(S.oddsMax==null||d.price<=S.oddsMax); }
  ...
  function _jDetail(data){
    ...
    return '<div class="j-detail"><div class="j-dhd"><span class="j-dnm" data-n="'+_attrEsc(d.p.name)+'" onclick="openPlayer(this.dataset.n)">'+esc(d.p.name)+'</span>'+
      ...
      _jBar('Season',r.season,r.seasonN)+_jBar('L5',r.l5,r.l5N)+_jBar('H2H',r.h2h,r.h2hN)+_jBar('Venue',r.venue,r.venueN)+
      '<div class="j-chips">'+chip('DVP',d.dvp)+scChip+chip('Def',d.def)+'</div>'+
  ...
  function jupiter(g){
    ...
    if(S.key!==key){ S.key=key; S.sel={}; S.focus=null; }
    const data=_jData(g);
    ...
    const controls='<div class="j-controls"><div class="j-crow"><span class="j-clab">Market</span><div class="j-chips2">'+mkts+'</div></div>'+
  ...
  function _degenTileKeys(){
    ...
    const S=_signals(); return (S&&S.tiles)?Object.keys(S.tiles):[];
  }
  ...
  function _liveTeamEq(a,b){ a=(a||'').toLowerCase(); b=(b||'').toLowerCase(); return a===b || a.startsWith(b) || b.startsWith(a); }
  ...
  (function init(){
    ...
    fetch(_sportPath('data/meta.json'),{cache:'no-cache'}).then(r=>r.ok?r.json():null).then(m=>{ if(m&&m.password_hash)AUTH_HASH=m.password_hash; }).catch(()=>{}).finally(()=>{
