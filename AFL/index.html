...
.pm-odds{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:4px;}
  ...
  .ob-filters{display:flex;flex-direction:row;flex-wrap:wrap;gap:9px 14px;margin:10px 0 12px;align-items:center;}
.ob-sel{background:var(--surface-2);border:1px solid var(--border);color:var(--text);font-family:var(--font-display);font-weight:700;font-size:12px;padding:6px 10px;border-radius:6px;cursor:pointer;max-width:190px;}
.ob-sel:focus{outline:none;border-color:var(--accent);}
.ob-dd{position:relative;}
.ob-dd-sum{list-style:none;display:inline-flex;align-items:center;gap:6px;user-select:none;white-space:nowrap;}
.ob-dd-sum::-webkit-details-marker{display:none;}
.ob-dd[open]>.ob-dd-sum{border-color:var(--accent);}
.ob-dd-body{position:absolute;z-index:30;top:calc(100% + 4px);left:0;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:9px;display:flex;flex-wrap:wrap;gap:6px;width:min(280px,80vw);box-shadow:0 8px 24px rgba(0,0,0,.45);}
...
.lt-meta{display:flex;align-items:center;justify-content:space-between;font-size:11px;color:var(--text-3);margin-bottom:10px;font-weight:700;}
  ...
  .card-toast.show{opacity:1;transform:translateX(-50%) translateY(0);}
    .mbv-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}
  .mbv-title{font-size:16px;font-weight:800;font-family:var(--font-display);display:flex;align-items:center;gap:8px}
  .mbv-sum{font-size:12px;color:var(--text-3)}
  .mbv-filters{display:flex;gap:8px;margin-bottom:12px}
  .mbv-filters .ob-sel{flex:1}
  .mbv-list{display:flex;flex-direction:column;gap:10px}
  .mbv-card{background:var(--surface-2);border:1px solid var(--border);border-radius:12px;padding:11px 12px;cursor:pointer}
  .mbv-card.on{border-color:var(--accent)}
  .mbv-row1{display:flex;align-items:center;gap:10px;margin-bottom:8px}
  .mbv-pn{flex:1;min-width:0}
  .mbv-nm{font-size:14px;font-weight:700;display:flex;justify-content:space-between;align-items:baseline;gap:8px}
  .mbv-tm{font-size:11px;color:var(--text-3);font-weight:400}
  .mbv-mk{font-size:13px;color:var(--text-2);display:flex;justify-content:space-between;align-items:baseline;margin-top:1px;gap:8px}
  .mbv-proj{font-size:12px;color:var(--text-3)}
  .mbv-add{font-size:22px;color:var(--text-3);flex-shrink:0;line-height:1}
  .mbv-op{display:flex;flex-direction:column;align-items:center;justify-content:center;min-width:58px;padding:4px 8px;border:1px solid var(--accent);border-radius:8px;flex-shrink:0}
  .mbv-op-nb{border-color:var(--border)}
  .mbv-op-price{font-size:15px;font-weight:800;color:var(--text);font-family:var(--font-display);line-height:1.1}
  .mbv-op-book{font-size:9px;color:var(--text-3);text-transform:uppercase;letter-spacing:.4px;margin-top:1px}
  .mbv-card.on .mbv-add{color:var(--accent)}
  .mbv-row2{display:flex;align-items:center;gap:10px;margin-bottom:8px;font-size:12px;color:var(--text-3)}
  .mbv-dots{display:inline-flex;gap:3px;align-items:center}
  .mbv-dot{width:6px;height:6px;border-radius:50%;background:var(--border)}
  .mbv-dot.on{background:var(--accent)}
  .mbv-edge{color:var(--ok,#22c55e)}
  .mbv-odds{color:var(--text);font-weight:700}
  .mbv-opp{margin-left:auto}
  .mbv-ctx{font-size:11px;color:var(--text-3);margin-bottom:9px;line-height:1.35}
  .mbv-badges{display:flex;flex-wrap:wrap;gap:6px}
  .mbv-bdg{display:inline-flex;align-items:center;gap:4px;font-size:11px;font-weight:600;padding:3px 8px;border-radius:20px;line-height:1}
  .mbv-bdg i{font-size:12px}
  .mbv-nerd{display:inline-flex;align-items:center;gap:4px;font-size:10px;color:var(--text-3);border:1px dashed var(--border-light,var(--border));font-family:var(--font-mono);padding:3px 7px;border-radius:20px;line-height:1}
  .mbv-nerd i{font-size:11px}
  .mbv-pager{display:flex;align-items:center;justify-content:center;gap:14px;margin-top:12px}
  .mbv-pg{background:var(--surface-2);border:1px solid var(--border);color:var(--text);border-radius:8px;padding:6px 12px;cursor:pointer;font-size:15px}
  .mbv-pg[disabled]{opacity:.4;cursor:default}
  .mbv-pg-lbl{font-size:12px;color:var(--text-3);font-variant-numeric:tabular-nums}
  .dv-card .mbv-mk{flex-wrap:wrap;gap:3px 6px;align-items:center}
  .dv-cardctrl{display:flex;align-items:center;gap:3px;flex-shrink:0}
  .dv-card .dv-swap,.dv-cardctrl .dv-swap{display:inline-flex;gap:2px}
  .dv-card .dv-swap button,.dv-cardctrl button,.dv-card .dv-statswap,.dv-card .dv-linedown,.dv-card .dv-linereset,.dv-cardctrl .dv-rm{background:var(--surface-3,var(--surface));border:1px solid var(--border);color:var(--text-2);border-radius:6px;padding:2px 7px;font-size:12px;cursor:pointer;line-height:1.3}
  .dv-card .dv-statswap{display:inline-flex;align-items:center;gap:4px}
  .dv-card .dv-linedown,.dv-card .dv-linereset{padding:2px 6px;margin-right:3px}
  .dv-cardbadges{align-items:center;gap:8px}
  .dv-vals{font-size:11px;color:var(--text-3);font-family:var(--font-mono)}
  .dv-card.dv-reduced{border-color:var(--ok,#22c55e)}
  .deg-page{display:flex;flex-direction:column;gap:16px}
  .deg-sec{background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:12px}
  .deg-sec-h{font-size:15px;font-weight:800;font-family:var(--font-display);display:flex;align-items:center;gap:8px;margin-bottom:3px}
  .deg-sec-sub{font-size:12px;color:var(--text-3);margin-bottom:10px}
  .ma-vs{display:flex;align-items:center;gap:8px;margin:8px 0 4px}.ma-p{flex:1;min-width:0}.ma-p.r{text-align:right}.ma-nm{font-weight:700;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.ma-sub{font-size:9px;color:var(--text-2);text-transform:uppercase;letter-spacing:.03em}.ma-mid{font-size:11px;color:var(--text-2);font-weight:700}.ma-splitnote{display:flex;justify-content:space-between;font-size:9px;color:var(--text-2);text-transform:uppercase;letter-spacing:.03em;margin-bottom:6px;opacity:.8}.ma-h2h,.ma-cond{font-size:11px;color:var(--text-2);margin-bottom:5px}.ma-h2h b{color:var(--text-1)}.ma-rows{display:flex;flex-direction:column;gap:4px;margin-top:4px}.ma-row{display:grid;grid-template-columns:46px 1fr 74px 1fr 46px;align-items:center;gap:5px;font-size:11px}.ma-v{color:var(--text-1);font-weight:600;font-variant-numeric:tabular-nums;white-space:nowrap}.ma-v.r{text-align:right}.ma-lbl{text-align:center;font-size:9px;color:var(--text-2);text-transform:uppercase;letter-spacing:.02em;line-height:1.1}.ma-bar{position:relative;height:15px;background:var(--surface-2,rgba(128,128,128,.15));border-radius:8px;overflow:hidden}.ma-fill{position:absolute;top:0;bottom:0;border-radius:8px;opacity:.85}.ma-bar.l .ma-fill{right:0}.ma-bar.r .ma-fill{left:0}.ma-pct{position:absolute;top:0;bottom:0;display:flex;align-items:center;font-size:9px;font-weight:700;color:#fff}.ma-bar.l .ma-pct{left:6px}.ma-bar.r .ma-pct{right:6px}.ma-na{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:var(--text-2);font-size:10px;opacity:.6}
.ma-arsenal{margin:8px 0;padding:8px 9px;background:var(--surface-2,rgba(128,128,128,.08));border-radius:9px}.ma-arsenal-lbl{font-size:9px;color:var(--text-2);text-transform:uppercase;letter-spacing:.03em;margin-bottom:7px}.ma-chips{display:flex;flex-wrap:wrap;gap:5px;margin-bottom:7px}.ma-chip{font-size:10px;padding:3px 9px;border-radius:12px;border:1px solid var(--border);background:transparent;color:var(--text-2);cursor:pointer;font-weight:600;font-family:inherit}.ma-chip.on{background:rgba(34,197,94,.16);border-color:#22c55e;color:var(--text-1)}.ma-arsrow{display:flex;justify-content:space-between;align-items:center;gap:8px;font-size:12px;padding-top:6px;border-top:1px solid var(--border)}.ma-arslbl{color:var(--text-2);text-transform:uppercase;font-size:10px;letter-spacing:.03em}.ma-arsval{font-weight:700;font-variant-numeric:tabular-nums;white-space:nowrap}.ma-na2{color:var(--text-2);font-size:10px}
.afl-dist{margin:6px 0 4px}.afl-pies{display:flex;gap:16px;justify-content:center;margin:4px 0 8px}.afl-pie-wrap{text-align:center}.afl-pie{width:82px;height:82px;border-radius:50%;box-shadow:0 0 0 1px rgba(0,0,0,.15) inset}.afl-pie-cap{font-size:10px;color:var(--text-2);text-transform:uppercase;letter-spacing:.03em;margin-top:5px;font-weight:600}.afl-leg{display:grid;grid-template-columns:1fr 1fr;gap:3px 12px}.afl-leg-r{display:flex;align-items:center;gap:5px;font-size:10px}.afl-dot{width:9px;height:9px;border-radius:2px;flex:none}.afl-leg-nm{font-weight:700;color:var(--text-1);width:26px}.afl-leg-v{color:var(--text-2);font-variant-numeric:tabular-nums;margin-left:auto}.afl-leg-note{font-size:9px;color:var(--text-2);text-align:center;margin-top:5px;text-transform:uppercase;letter-spacing:.03em;opacity:.8}
.cov-dhdr{display:flex;align-items:center;gap:8px;margin:5px 0 7px}.cov-bar{flex:none;width:64px;height:9px;border-radius:5px;background:#3b82f6;overflow:hidden;position:relative}.cov-man{position:absolute;left:0;top:0;bottom:0;background:#ef4444}.cov-rates{font-size:11px;color:var(--text-2)}.cov-rates b{color:var(--text-1)}.cov-rec{display:flex;align-items:center;gap:8px;font-size:11px;padding:3px 0;border-top:1px solid var(--border)}.cov-nm{font-weight:600;color:var(--text-1);cursor:pointer;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.cov-split{color:var(--text-2);font-variant-numeric:tabular-nums;white-space:nowrap}.cov-split .hot{color:#22c55e;font-weight:700}.cov-flag{font-size:9px;font-weight:700;padding:2px 7px;border-radius:10px;background:rgba(34,197,94,.16);color:#22c55e;border:1px solid #22c55e;white-space:nowrap;flex:none}
.deg-bookbar{display:flex;align-items:center;gap:10px;margin-bottom:12px;flex-wrap:wrap}
  .deg-bl{font-size:12px;color:var(--text-3);font-weight:700}
  .pyr{display:flex;flex-direction:column;gap:10px;align-items:center;padding:6px 0}
  .pyr-row{display:flex;gap:6px;justify-content:center;align-items:stretch}
  .pyr-card{background:var(--surface-2);border:1px solid var(--border);border-radius:10px;padding:10px 9px;text-align:center;width:116px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;position:relative}
  .pyr-swap{position:absolute;top:2px;right:2px;background:none;border:none;color:var(--text-3);cursor:pointer;padding:3px;font-size:12px;line-height:1;opacity:.55;border-radius:5px}
  .pyr-swap:hover{opacity:1;color:var(--accent);background:var(--surface-3,var(--surface))}
  .pyr-reset{background:var(--surface-2);border:1px solid var(--border);border-radius:6px;color:var(--text-3);font-size:11px;font-weight:700;padding:3px 8px;cursor:pointer}
  .pyr-reset:hover{color:var(--accent);border-color:var(--accent)}
  .pyr-nm{font-size:12px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:104px}
  .pyr-ln{font-size:11px;color:var(--accent);font-weight:700;line-height:1.25;white-space:normal}
  .pyr-od{font-size:13px;color:var(--text);font-weight:800;font-family:var(--font-display)}
  .deg-empty{padding:18px;text-align:center;color:var(--text-3);font-size:13px;display:flex;align-items:center;justify-content:center;gap:8px}
  .pyr{display:flex;flex-direction:column;gap:12px}
  .pyr-line{font-size:11px;font-weight:700;color:var(--accent);text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px}
  .yk-sgm{padding:0!important;overflow:hidden}
  .yk-sgm-h{display:flex;align-items:center;gap:8px;padding:8px 10px;background:var(--surface-3,var(--surface));border-bottom:1px solid var(--border);font-size:12px;font-weight:700}
  .yk-sgm-g{flex:1}
  .yk-sgm-od{color:var(--accent);font-family:var(--font-display)}
  .yk-leg{display:flex;align-items:center;gap:8px;padding:7px 10px;font-size:12px;border-bottom:1px solid var(--border-light,var(--border))}
  .yk-leg:last-child{border-bottom:none}
  .yk-lp{flex:1;font-weight:600}
  .yk-lm{color:var(--text-2)}
  .yk-lo{color:var(--text-3);min-width:44px;text-align:right}
  .yk-sys{margin-top:10px;background:var(--surface-2);border:1px solid var(--border);border-radius:10px;padding:10px 12px}
  .yk-sys-r{display:flex;justify-content:space-between;font-size:13px;padding:3px 0}
  .yk-sys-max{font-weight:800;color:var(--ok,#22c55e);border-top:1px solid var(--border);margin-top:4px;padding-top:7px}
  .yk-sys-note{font-size:11px;color:var(--text-3);margin-top:6px;line-height:1.4}
  .lad-grid{display:flex;flex-direction:column;gap:8px}
  .lad-day{background:var(--surface-2);border:1px solid var(--border);border-left-width:3px;border-radius:10px;padding:9px 11px}
  .lad-day-h{display:flex;align-items:center;gap:8px;font-size:12px;font-weight:700}
  .lad-n{color:var(--text-3)}
  .lad-ic{font-size:15px}
  .lad-win{border-left-color:var(--ok,#22c55e)}
  .lad-win .lad-ic{color:var(--ok,#22c55e)}
  .lad-loss{border-left-color:var(--bad,#ef4444)}
  .lad-loss .lad-ic{color:var(--bad,#ef4444)}
  .lad-pending{border-left-color:var(--border)}
  .lad-pending .lad-ic{color:var(--text-3)}
  .lad-odds{color:var(--accent);font-family:var(--font-display)}
  .lad-book{color:var(--text-3);font-weight:400;font-size:11px}
  .lad-bank{margin-left:auto;color:var(--text-2);font-family:var(--font-display)}
  .lad-legs{display:grid;grid-template-columns:repeat(auto-fill,minmax(96px,1fr));gap:5px;margin-top:2px}
  .lad-leg{background:var(--surface-3,var(--surface));border:1px solid var(--border-light,var(--border));border-radius:6px;padding:2px 7px;font-size:11px;color:var(--text-2)}
  .lad-leg{display:inline-flex;align-items:center;gap:4px;flex-wrap:wrap;max-width:100%;min-width:0}
  .lad-leg img,.lad-leg svg{width:14px;height:14px;border-radius:3px;flex:none}
  .lad-leg b{color:var(--text);font-family:var(--font-display)}
  .lad-vs{color:var(--text-3);font-size:10px}
  .lad-sub{display:flex;align-items:center;gap:6px;margin-top:4px;padding-top:6px;border-top:1px dashed var(--border);font-size:11px;flex-wrap:wrap;min-width:0}
  .lad-sub-lbl{font-size:9px;font-weight:800;letter-spacing:.4px;color:var(--text-3);background:var(--surface-3,var(--surface));border:1px solid var(--border);border-radius:4px;padding:1px 5px;flex:none}
  .lad-sub .lad-leg{width:100%}
  .lad-type{font-size:9px;font-weight:800;letter-spacing:.4px;padding:1px 5px;border-radius:4px;text-transform:uppercase}
  .lad-type-multi{background:color-mix(in srgb,var(--accent) 18%,transparent);color:var(--accent)}
  .lad-type-sgm{background:color-mix(in srgb,var(--ok,#22c55e) 18%,transparent);color:var(--ok,#22c55e)}
  .lad-hero{background:var(--surface-2);border:1px solid var(--border);border-radius:12px;padding:12px 14px;margin-bottom:10px}
  .lad-hero-top{display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px}
  .lad-goal{font-size:12px;font-weight:700;color:var(--text-3);letter-spacing:.3px}
  .lad-cur{font-size:22px;font-weight:800;font-family:var(--font-display);color:var(--accent)}
  .lad-bar{height:8px;border-radius:5px;background:var(--surface-3,var(--surface));overflow:hidden}
  .lad-bar-fill{height:100%;border-radius:5px;background:linear-gradient(90deg,var(--accent),var(--ok,#22c55e));transition:width .5s ease}
  .lad-hero-sub{font-size:12px;color:var(--text-2);margin-top:7px}
  .lad-tag-win{color:var(--ok,#22c55e)}
  .lad-tag-bust{color:var(--bad,#ef4444)}
  .lad-rung{color:var(--text);font-weight:800}
  .lad-money{margin-left:auto;font-family:var(--font-display);font-size:12px;color:var(--text-2)}
  .lad-money b{color:var(--text)}
  .lad-win .lad-money b{color:var(--ok,#22c55e)}
  .lad-loss .lad-money b{color:var(--bad,#ef4444)}
  .lad-day{display:flex;flex-direction:column;align-items:stretch;gap:6px;padding:9px 11px;background:var(--surface-2);border:1px solid var(--border);border-radius:10px;font-size:12px}
  .lad-n{font-weight:700;min-width:46px;color:var(--text-2)}
  .lad-pick{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  .lad-odds{color:var(--text-3);font-variant-numeric:tabular-nums}
  .lad-bank{font-weight:700;font-variant-numeric:tabular-nums;min-width:60px;text-align:right}
  .lad-ic{font-size:18px}
  .lad-win .lad-ic{color:var(--ok,#22c55e)} .lad-win{border-color:rgba(34,197,94,.35)}
  .lad-loss .lad-ic{color:var(--bad,#ef4444)} .lad-loss{opacity:.65}
  .lad-pending .lad-ic{color:var(--text-3)}
  .mbv-save{width:100%;margin-top:12px;padding:11px;background:var(--accent);color:#0b1220;border:none;border-radius:10px;font-weight:800;font-size:14px;cursor:pointer;font-family:var(--font-display)}

/* === WC-style soccer panels (EPL) === */
.tp-collapse { background: var(--surface); border: 1px solid var(--border);
    border-radius: 6px; margin: 0 0 16px;
    overflow: hidden; }
.tp-collapse-sum { list-style: none; cursor: pointer; padding: 11px 14px;
...
.tp-collapse-meta { font-family: var(--font-mono); font-size: 10px;
    color: var(--text-3); font-weight: 500;
    background: var(--surface-3);
    padding: 1px 6px; border-radius: 3px; }
.tp-collapse-chev { margin-left: auto; color: var(--text-3); font-size: 14px;
    transition: transform 0.18s ease; }
.tp-collapse[open] .tp-collapse-chev { transform: rotate(180deg); }
.ref-collapse .tp-collapse-sum { flex-wrap: wrap; }
.tp-collapse-body { border-top: 1px solid var(--border);
    background: var(--surface-2);
    padding: 10px 12px; }
.mk-collapse { margin-bottom: 8px;
    background: var(--surface);
    border: 1px solid var(--border);
    border-radius: 6px;
    overflow: hidden; }
.mk-collapse > .tp-collapse-sum { padding: 12px 14px;
    background: var(--surface); }
.mk-collapse[open] > .tp-collapse-sum { border-bottom: 1px solid var(--border); }
.mk-collapse > .tp-collapse-body { padding: 12px;
    background: var(--surface); }
.mk-collapse .markets-grid { display: grid;
    grid-template-columns: repeat(auto-fit, minmax(260px, 1fr));
    gap: 10px; }
.market-card { background: var(--surface-2); border: 1px solid var(--border);
    border-left: 3px solid var(--neutral);
    border-radius: 5px; padding: 8px 10px;
    position: relative; }
.market-card.lean-over { border-left-color: var(--good); }
.market-card.lean-under { border-left-color: var(--bad); }
.market-card.lean-neutral { border-left-color: var(--neutral); }
.market-name { font-family: var(--font-mono); font-size: 9px; letter-spacing: 1px;
    color: var(--text-3); text-transform: uppercase; margin-bottom: 3px; }
.market-headline { display: flex; align-items: baseline; gap: 6px;
    margin-bottom: 4px; }
.market-value { font-family: var(--font-display); font-weight: 700; font-size: 17px;
    line-height: 1; letter-spacing: 0.3px; }
.market-line { font-family: var(--font-mono); font-size: 10px; color: var(--text-2);
    letter-spacing: 0.4px; }
.market-verdict { display: inline-block;
    font-family: var(--font-display); font-weight: 700; font-size: 10px;
    letter-spacing: 0.8px; text-transform: uppercase;
    padding: 2px 6px; border-radius: 3px;
    margin-bottom: 4px; }
.market-verdict.over { background: rgba(34,197,94,0.15); color: var(--good); }
.market-verdict.under { background: rgba(255,90,90,0.15); color: var(--bad); }
.market-verdict.neutral { background: var(--surface-3); color: var(--neutral); }
.market-detail { font-family: var(--font-mono); font-size: 10px; color: var(--text-2);
    line-height: 1.4; }
.market-detail .home-val { color: var(--text); font-weight: 500; }
.market-detail .away-val { color: var(--text); font-weight: 500; }
.market-detail .lens-tag { display: inline-block;
    font-family: var(--font-mono); font-size: 8px; font-weight: 700;
    padding: 1px 5px; border-radius: 2px;
    letter-spacing: 0.5px;
    margin: 0 2px;
    vertical-align: 1px; }
.market-detail .lens-tag.style { background: rgba(34,197,94,0.15); color: #4ade80; }
.market-detail .lens-tag.tier { background: rgba(59,130,246,0.15); color: #60a5fa; }
.market-detail .lens-tag.overall { background: var(--surface-3); color: var(--text-3); }
.market-odds { margin-top: 6px; padding-top: 6px;
    border-top: 1px dashed var(--border);
    display: flex; align-items: center; gap: 6px; flex-wrap: wrap;
    font-family: var(--font-mono); font-size: 9px; }
.market-odds .odds-book { color: var(--text-3); font-weight: 700;
    font-size: 9px; letter-spacing: 0.4px;
    background: var(--surface-3);
    padding: 1px 5px; border-radius: 2px;
    flex-shrink: 0; }
.market-odds .odds-body { color: var(--text); font-weight: 600; }
.market-bar { height: 4px; background: var(--surface-3); border-radius: 2px;
    margin-top: 6px; position: relative; overflow: hidden; }
.market-bar .fill { position: absolute; top: 0; left: 0; bottom: 0; }
.market-bar .fill.good { background: var(--good); }
.market-bar .fill.bad { background: var(--bad); }
.market-bar .fill.neutral { background: var(--neutral); }
.market-bar .marker { position: absolute; top: -2px; bottom: -2px; width: 2px;
    background: var(--text); }
.market-card .sample { position: absolute; top: 8px; right: 10px;
    font-family: var(--font-mono); font-size: 9px;
    color: var(--text-3); letter-spacing: 0.5px; }
.pm-hitrate-card .market-name { font-family: var(--font-mono); font-size: 9px;
    color: var(--text-3); letter-spacing: 1px; text-transform: uppercase;
    margin-bottom: 4px; }

/* EPL match panel: team header + 1X2 segmented bar */
.mm-teamhdr{display:flex;align-items:flex-start;justify-content:space-between;gap:8px;padding:12px 10px 8px}
.mm-th{flex:1;text-align:center;min-width:0}
.mm-th .team-logo,.mm-th img{margin:0 auto 5px;display:block}
.mm-th-nm{font-family:var(--font-display);font-weight:700;font-size:15px;line-height:1.15}
.mm-th-sub{font-family:var(--font-mono);font-size:10px;color:var(--text-3);margin-top:3px}
.mm-th-st{font-family:var(--font-mono);font-size:9px;color:var(--text-2);margin-top:3px;text-transform:uppercase;letter-spacing:.3px;line-height:1.4}
.mm-vs{align-self:center;color:var(--text-3);font-size:10px;font-family:var(--font-mono)}
.mm-meta{text-align:center;font-family:var(--font-mono);font-size:10px;color:var(--text-3);padding:0 10px 10px}
.mm-probbar{display:flex;height:32px;border-radius:5px;overflow:hidden;margin:4px 0}
.mm-probseg{display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:#0b0f14;white-space:nowrap;overflow:hidden;min-width:0}
.mm-probseg.h{background:#38bdf8}.mm-probseg.d{background:#64748b;color:#fff}.mm-probseg.a{background:#fb7185}

/* EPL stat breakdown (rank / vs-tier / vs-style) */
.brk-title{font-family:var(--font-mono);font-size:10px;color:var(--text-3);text-transform:uppercase;letter-spacing:.4px;margin:12px 0 8px}
.brk-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}
@media(max-width:520px){.brk-grid{grid-template-columns:1fr}}
.brk-team{background:var(--surface);border:1px solid var(--border);border-radius:6px;padding:8px 10px}
.brk-team-hdr{display:flex;align-items:center;gap:5px;font-family:var(--font-display);font-weight:700;font-size:12px;margin-bottom:4px;flex-wrap:wrap}
.brk-rank{font-family:var(--font-mono);font-size:9px;color:var(--text-3);background:var(--surface-3);padding:1px 5px;border-radius:3px}
.brk-substat{font-family:var(--font-mono);font-size:10px;color:var(--accent);text-transform:uppercase;letter-spacing:.3px;margin:6px 0 1px}
.tier-row{display:flex;justify-content:space-between;align-items:baseline;font-size:11px;padding:1px 0}
.tier-row-lbl{color:var(--text-2)}
.tier-row-val{color:var(--text);font-weight:600}
.tier-row-val em{color:var(--text-3);font-style:italic;font-weight:400}
.tier-row-n{color:var(--text-3);font-size:9px;margin-left:4px;font-weight:400}

/* EPL By The Halves panel */
.bth-meta{font-family:var(--font-mono);font-size:10px;color:var(--text-3);text-transform:uppercase;margin-bottom:8px}
.bth-callout{display:inline-block;font-family:var(--font-mono);font-size:10px;font-weight:700;color:var(--accent);background:var(--surface-3);padding:3px 8px;border-radius:4px;margin-bottom:10px;text-transform:uppercase}
.bth-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:6px;margin-bottom:8px}
.bth-cell{background:var(--surface);border:1px solid var(--border);border-radius:5px;padding:8px 4px;text-align:center}
.bth-v{font-family:var(--font-display);font-weight:700;font-size:16px}
.bth-pct .bth-v{font-size:14px}
.bth-l{font-family:var(--font-mono);font-size:9px;color:var(--text-3);text-transform:uppercase;margin-top:2px}
.bth-note{font-family:var(--font-mono);font-size:9px;color:var(--text-3);font-style:italic;margin-top:2px}

/* EPL Recent Games rows (WC style) */
.rg2-row{display:flex;align-items:center;gap:7px;padding:6px 2px;border-bottom:1px solid var(--border);font-size:12px;flex-wrap:wrap}
.rg2-row:last-child{border-bottom:0}
.rg2-res{width:18px;height:18px;border-radius:4px;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:11px;flex-shrink:0}
.rg2-res.st-w{background:rgba(34,197,94,.2);color:#22c55e}.rg2-res.st-l{background:rgba(239,68,68,.2);color:#ef4444}.rg2-res.st-d{background:var(--surface-3);color:var(--text-2)}
.rg2-date{font-family:var(--font-mono);font-size:10px;color:var(--text-3);width:42px;flex-shrink:0}
.rg2-opp{flex:1;display:flex;align-items:center;gap:4px;min-width:90px;overflow:hidden;white-space:nowrap}
.rg2-onm{overflow:hidden;text-overflow:ellipsis}
.rg2-sc{font-weight:700;font-family:var(--font-mono);flex-shrink:0}
.rg2-tags{display:flex;gap:3px;flex-shrink:0;flex-wrap:wrap;justify-content:flex-end;margin-left:auto}
.rg-tag{font-family:var(--font-mono);font-size:8.5px;padding:1px 4px;border-radius:3px;background:var(--surface-3);color:var(--text-3);white-space:nowrap}
.rg-tag.on{background:rgba(34,197,94,.18);color:#22c55e}

/* EPL goal-timing buckets / First & Last / Sub Trends */
.bth-sub{font-family:var(--font-mono);font-size:10px;color:var(--text-3);text-transform:uppercase;margin:12px 0 6px}
.gtb-row{display:flex;gap:4px;align-items:flex-end}
.gtb-col{flex:1;display:flex;flex-direction:column;align-items:center}
.gtb-bars{display:flex;gap:2px;align-items:flex-end;height:60px;width:100%;justify-content:center}
.gtb-bar{width:9px;border-radius:2px 2px 0 0;min-height:1px}
.gtb-bar.sc{background:#22c55e}.gtb-bar.cc{background:#ef4444}
.gtb-lbl{font-family:var(--font-mono);font-size:8px;color:var(--text-3);margin-top:3px}
.gtb-n{font-family:var(--font-mono);font-size:8px;color:var(--text-2)}
.gtb-legend{display:flex;align-items:center;gap:5px;font-family:var(--font-mono);font-size:9px;color:var(--text-3);margin-top:8px}
.gtb-key{width:8px;height:8px;border-radius:2px;display:inline-block;margin-left:6px}
.gtb-key.sc{background:#22c55e}.gtb-key.cc{background:#ef4444}
.fl-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-bottom:8px}
.fl-mins{grid-template-columns:repeat(2,1fr)}
.fl-stat{background:var(--surface);border:1px solid var(--border);border-radius:5px;padding:7px 4px;text-align:center}
.fl-v{font-family:var(--font-display);font-weight:700;font-size:14px}
.fl-l{font-family:var(--font-mono);font-size:8.5px;color:var(--text-3);text-transform:uppercase;margin-top:2px}
.fl-sec{font-family:var(--font-mono);font-size:10px;color:var(--accent);text-transform:uppercase;margin:10px 0 4px;letter-spacing:.3px}
.fl-row{display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid var(--border);font-size:12px}
.fl-row:last-child{border-bottom:0}
.fl-nm{font-weight:600}
.fl-c{font-family:var(--font-mono);font-size:10px;color:#22c55e}
.st2-row{display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid var(--border)}
.st2-row:last-child{border-bottom:0}
.st2-nm{font-weight:600;font-size:12px}
.st2-meta{font-family:var(--font-mono);font-size:10px;color:var(--text-3)}

/* EPL Degen signal card: prominent pick+odds row + visible last-5 */
.lc-line{display:flex;align-items:center;justify-content:space-between;gap:8px;margin:5px 0;padding:5px 9px;background:var(--surface-3);border-radius:5px}
.lc-line-pick{font-family:var(--font-display);font-weight:700;font-size:13px;color:var(--text)}
.lc-odds{font-family:var(--font-mono);font-weight:700;font-size:13px;color:#22c55e}
.lc-odds.noodds{color:var(--text-3);font-weight:400;font-size:10px}
.lc-book{font-size:9px;color:var(--text-3);font-weight:400}
.lc-l5{display:flex;align-items:center;gap:7px;margin-bottom:5px}
.lc-l5-lbl{font-family:var(--font-mono);font-size:9px;color:var(--text-3);letter-spacing:.5px;flex-shrink:0}

.lc-spark{display:inline-flex;gap:3px;flex-wrap:wrap}
.lc-spark .v{font-family:var(--font-mono);font-size:11px;font-weight:700;min-width:20px;text-align:center;padding:2px 5px;border-radius:4px}
.lc-spark .v.hit{background:rgba(34,197,94,.2);color:#22c55e}
.lc-spark .v.miss{background:var(--surface-3);color:var(--text-3)}

/* fixture-list local kickoff (date + time) */
.fxr-ko-date{display:block;font-size:9px;color:var(--text-3);font-family:var(--font-mono);text-transform:uppercase;letter-spacing:.3px;line-height:1.3}
.fxr-ko-time{display:block;font-size:12px;font-weight:700;color:var(--text);line-height:1.2}
</style>
</head>
<body class="auth-locked">
  ...
  const SPORTS = {
    ...
    epl: { key:'epl', dir:'EPL', name:'EPL', oddsUrl:'', liveWorker:'' },   // EPL odds are committed (API-Football via workflow), not live-worker
  ...
  function _applyConfig(){
    ...
    try{ if(SETTINGS&&SETTINGS.multi&&Array.isArray(SETTINGS.multi.markets)){ const _vm=new Set(MULTI_MKTS.map(m=>m[0])); SETTINGS.multi.markets=SETTINGS.multi.markets.filter(k=>_vm.has(k)); if(!SETTINGS.multi.markets.length) SETTINGS.multi.markets=MULTI_MKTS.map(m=>m[0]); } }catch(e){}
    ...
    {k:'degenerates',n:'Degenerates', i:'ti-flame',       d:'Round multis & ladder'},
    ...
    if(Array.isArray(CREW)&&!CREW.some(function(c){return c.k==='degenerates';})){ var _mi=CREW.findIndex(function(c){return c.k==='multi';}); CREW.splice(_mi>=0?_mi+1:1,0,{k:'degenerates',n:'Degenerates',i:'ti-flame',d:'Round multis & ladder'}); }
    ...
    ELITE_OPTS=CFG.eliteDefs||null;   // per-sport Elite Matchups stats; falls back to DVPS_OPTS
  ...
  function _statPositions(stat){
    if(!(POS_MARKETS)&&!(DISPLAY_SETS_CFG&&DISPLAY_SETS_CFG.All)) return POS_ORDER;   // no map => every position
    return (POS_ORDER||[]).filter(function(pos){ var a=_posAllow(pos); return !a || a.indexOf(stat)>=0; });
  }
  ...
  function adaptData(){
    ...
    state.idx.players.forEach(p=>{ if(p.matches==null) p.matches=(p.games!=null?p.games:(p.gp!=null?p.gp:0)); });   // WNBA carries 'games' not 'matches'
  ...
  function buildOddsIdx(){
    ...
    state.idx.oddsMeta={ count, altCount:altArr0.length, bookCount:bookArr0.length,
      total: count + altArr0.length + bookArr0.length, fromPaste,
  ...
  /* ===================== TAB NAV / RENDER ===================== */
  function _renderTabnav(){document.getElementById('tabnav').innerHTML=TABS.filter(t=>(!t.sports||t.sports.indexOf(SPORT.key)>=0)&&!(t.name==='odds'&&(window.SPORT_CONFIG||{}).noPlayerOdds)).map(t=>'<button class="tab'+(t.name===state.view.activeTab?' active':'')+'" onclick="_switchTab(\''+t.name+'\')"><i class="ti '+t.icon+'"></i>'+t.label+'</button>').join('');}
  function _switchTab(n){state.view.activeTab=n;state.view.fixtureId=null;state.view.crewTile=null;_renderTabnav();_renderApp();window.scrollTo({top:0});}
  ...
  function renderTab(t){ if(t==='league')return renderLeague(); if(t==='fixture')return renderFixture(); if(t==='odds')return ((window.SPORT_CONFIG||{}).noPlayerOdds)?renderFixture():renderOdds(); if(t==='stats')return renderStats(); if(t==='degen')return renderDegen(); if(t==='info')return renderInfo(); return ''; }
  ...
  function renderOdds(){
    ...
    const gameSel=_obPool.length?('<div class="ob-filter"><span class="ob-fl">Game</span><select class="ob-sel" onchange="setOddsGame(+this.value)">'+_obPool.map(({g,i})=>'<option value="'+i+'"'+(i===gi?' selected':'')+'>'+abbr(g.home)+' v '+abbr(g.away)+'</option>').join('')+'</select></div>'):'';
    const mktSel='<div class="ob-filter"><span class="ob-fl">Market</span><select class="ob-sel" onchange="setOddsMkt(this.value)">'+ODDS_MKTS.map(m=>'<option value="'+m[0]+'"'+(m[0]===mkt?' selected':'')+'>'+m[1]+'</option>').join('')+'</select></div>';
    const _nbk=SETTINGS.books.length, _bkOpen=state.view.oddsBooksOpen?' open':'';
    const bookSel='<div class="ob-filter"><span class="ob-fl">Books</span><details class="ob-dd"'+_bkOpen+' ontoggle="state.view.oddsBooksOpen=this.open"><summary class="ob-sel ob-dd-sum">'+(_nbk?_nbk+' book'+(_nbk>1?'s':''):'All books')+'<i class="ti ti-chevron-down"></i></summary><div class="ob-dd-body"><button class="ob-chip'+(_nbk?'':' active')+'" onclick="_sBookAll()">All</button>'+ALL_BOOKS.map(([k,l])=>'<button class="ob-chip'+(SETTINGS.books.indexOf(k)>=0?' active':'')+'" onclick="_sBook(\''+k+'\')">'+l+'</button>').join('')+'</div></details></div>';
    const filters='<div class="ob-filters">'+gameSel+mktSel+bookSel+'</div>';
  ...
  // local date + time for the fixture list (user's timezone)
  function _localKickoff(g){ const ms=gameStart(g); if(ms==null) return esc((g&&g.time)||'TBC');
    const d=new Date(ms);
    return '<span class="fxr-ko-date">'+d.toLocaleDateString([],{weekday:'short',day:'numeric',month:'short'})+'</span>'+
           '<span class="fxr-ko-time">'+d.toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})+'</span>'; }
  ...
  function gameStart(g){
    ...
    if(g&&g.date&&/T\d/.test(g.date)){ const s=/[zZ]|[+-]\d\d:?\d\d$/.test(g.date)?g.date:(g.date+'Z'); const t=Date.parse(s); if(!isNaN(t)) return t; }
  ...
  function renderFixture(){
    ...
    games.forEach(({g,i})=>{
      ...
      h+='<div class="fixture-row'+(gameDone(g)?' fxr-done':'')+'" onclick="state.view.fixtureId='+i+';_renderApp();window.scrollTo({top:0})">'+
        ...
        '<div class="fxr-center"><div class="fxr-kickoff">'+_localKickoff(g)+'</div><div class="fxr-vs">'+(gameDone(g)?'<span class="fxr-ft">FT</span>':'v')+'</div><div class="fxr-venue">'+(g.venue||'TBC')+'</div>'+weatherChip(g.home,g.away)+'</div>'+
  ...
  // ---- EPL Match Markets: 1X2 + Goals (O/U + BTTS) + Cards, Poisson model off GF/GA ----
  function _mmTeamGoals(t){ var rs=(state.data.results||[]).filter(function(r){return r.home===t||r.away===t;}); if(!rs.length) return null;
    var gf=0,ga=0,btts=0; rs.forEach(function(r){ var isH=r.home===t; gf+=isH?r.homeScore:r.awayScore; ga+=isH?r.awayScore:r.homeScore; if(r.homeScore>0&&r.awayScore>0)btts++; });
    return {n:rs.length, gf:gf/rs.length, ga:ga/rs.length, btts:btts/rs.length}; }
  function _mmTeamCards(t){ var gl=(state.data.gamelogs||[]), byM={}; gl.forEach(function(r){ if(r.Team!==t)return; var m=r.MatchId||r.Date; (byM[m]=byM[m]||{y:0,r:0}); byM[m].y+=r.yellowCard||0; byM[m].r+=r.redCard||0; });
    var ms=Object.keys(byM).map(function(k){return byM[k];}); if(!ms.length) return null;
    return {n:ms.length, y:ms.reduce(function(s,x){return s+x.y;},0)/ms.length, r:ms.reduce(function(s,x){return s+x.r;},0)/ms.length}; }
  function _mmTeamCorners(t){ var tr=(state.data.teams||[]).find(function(x){return (x.team||x.name)===t;}); if(!tr||tr.cornersFor==null||tr.cornersAgainst==null) return null; return {cf:+tr.cornersFor, ca:+tr.cornersAgainst}; }
  function _mmTeamY(t){ var rs=(state.data.results||[]).filter(function(r){return (r.home===t||r.away===t)&&r.cardsHome!=null;}); if(rs.length<3) return null;
    var f=0,a=0; rs.forEach(function(r){var isH=r.home===t; f+=isH?r.cardsHome:r.cardsAway; a+=isH?r.cardsAway:r.cardsHome;}); return {n:rs.length,f:f/rs.length,a:a/rs.length}; }
  function _mmStyleOf(t){ var tr=(state.data.teams||[]).find(function(x){return (x.team||x.name)===t;}); return tr&&tr.style?tr.style:null; }
  var _MM_STYLE_C={'Possession Masters':'#3b82f6','All-Out Attack':'#ef4444','Pressers':'#eab308','Counter-Attackers':'#22c55e','Park the Bus':'#9aa3b0','Box-to-Box':'#a78bfa'};
  function _mmVsStyle(team,oppStyle){ if(!oppStyle) return null; var sm={}; (state.data.teams||[]).forEach(function(t){sm[t.team||t.name]=t.style;});
    var rs=(state.data.results||[]).filter(function(r){ var isH=r.home===team; var opp=isH?r.away:r.home; return (r.home===team||r.away===team)&&sm[opp]===oppStyle; });
    if(rs.length<3) return null; var gf=0,ga=0; rs.forEach(function(r){var isH=r.home===team; gf+=isH?r.homeScore:r.awayScore; ga+=isH?r.awayScore:r.homeScore;}); return {n:rs.length,gf:gf/rs.length,ga:ga/rs.length}; }
  function _mmRank(team,statKey,higher){ var ts=(state.data.teams||[]).filter(function(t){return t[statKey]!=null;}); if(!ts.length) return '';
    ts.sort(function(a,b){return higher?(b[statKey]-a[statKey]):(a[statKey]-b[statKey]);});
    var i=ts.map(function(t){return t.team||t.name;}).indexOf(team); return i<0?'':'<span class="mm-rk">#'+(i+1)+'</span>'; }
  function _mmVsGen(team,oppStyle,fn){ if(!oppStyle) return null; var sm={}; (state.data.teams||[]).forEach(function(t){sm[t.team||t.name]=t.style;});
    var rs=(state.data.results||[]).filter(function(r){ var isH=r.home===team; var opp=isH?r.away:r.home; return (r.home===team||r.away===team)&&sm[opp]===oppStyle; });
    var vals=[]; rs.forEach(function(r){ var v=fn(r,r.home===team); if(v) vals.push(v); }); if(vals.length<3) return null;
    return {n:vals.length, f:vals.reduce(function(s,x){return s+x.f;},0)/vals.length, a:vals.reduce(function(s,x){return s+x.a;},0)/vals.length}; }
  function _mmVsBreakGen(home,away,aSt,hSt,fn,fl,al){ var hV=_mmVsGen(home,aSt,fn), aV=_mmVsGen(away,hSt,fn); if(!hV&&!aV) return '';
    var rows=(hV?'<div class="mm-tr">'+abbr(home)+' vs '+aSt+' <b>'+hV.f.toFixed(2)+'</b> '+fl+' / '+hV.a.toFixed(2)+' '+al+' <span class="mm-sub">'+hV.n+'g</span></div>':'')+
      (aV?'<div class="mm-tr">'+abbr(away)+' vs '+hSt+' <b>'+aV.f.toFixed(2)+'</b> '+fl+' / '+aV.a.toFixed(2)+' '+al+' <span class="mm-sub">'+aV.n+'g</span></div>':'');
    return '<div class="mm-brk"><div class="mm-brk-t">Vs Similar Styles</div>'+rows+'</div>'; }
  function _mmVsCards(home,away,aSt,hSt){ var sm={}; (state.data.teams||[]).forEach(function(t){sm[t.team||t.name]=t.style;}); var gl=(state.data.gamelogs||[]);
    function team(t,oppStyle){ if(!oppStyle) return null; var byM={}; gl.forEach(function(r){ if(r.Team!==t)return; var m=r.MatchId||r.Date; (byM[m]=byM[m]||{c:0,opp:r.Opp}); byM[m].c+=(r.cards||0); });
      var vals=Object.keys(byM).filter(function(m){return sm[byM[m].opp]===oppStyle;}).map(function(m){return byM[m].c;}); if(vals.length<3) return null;
      return {n:vals.length, c:vals.reduce(function(s,x){return s+x;},0)/vals.length}; }
    var hV=team(home,aSt), aV=team(away,hSt); if(!hV&&!aV) return '';
    var rows=(hV?'<div class="mm-tr">'+abbr(home)+' vs '+aSt+' <b>'+hV.c.toFixed(2)+'</b> cards <span class="mm-sub">'+hV.n+'g</span></div>':'')+
      (aV?'<div class="mm-tr">'+abbr(away)+' vs '+hSt+' <b>'+aV.c.toFixed(2)+'</b> cards <span class="mm-sub">'+aV.n+'g</span></div>':'');
    return '<div class="mm-brk"><div class="mm-brk-t">Vs Similar Styles</div>'+rows+'</div>'; }
  function _mmPois(k,lam){ var p=Math.exp(-lam); for(var i=1;i<=k;i++)p*=lam/i; return p; }
  function _mmModel(lamH,lamA){ var pH=0,pD=0,pA=0,o25=0,btts=0,sc={};
    for(var h=0;h<=8;h++)for(var a=0;a<=8;a++){ var p=_mmPois(h,lamH)*_mmPois(a,lamA); if(h>a)pH+=p; else if(h<a)pA+=p; else pD+=p; if(h+a>2.5)o25+=p; if(h>0&&a>0)btts+=p; sc[h+'-'+a]=p; }
    var top=Object.keys(sc).map(function(k){return [k,sc[k]];}).sort(function(a,b){return b[1]-a[1];}).slice(0,3);
    return {pH:pH,pD:pD,pA:pA,o25:o25,btts:btts,top:top,tot:lamH+lamA}; }
  function _mmLean(proj,line,overWord,underWord){ var d=proj-line;
    if(d>=0.75) return {t:'STRONG '+overWord,c:'#22c55e'}; if(d>=0.25) return {t:'LEAN '+overWord,c:'#86efac'};
    if(d<=-0.75) return {t:'STRONG '+underWord,c:'#22c55e'}; if(d<=-0.25) return {t:'LEAN '+underWord,c:'#86efac'};
    return {t:'LINE BALL',c:'#888'}; }
  function _mmVsBreak(home,away,aSt,hSt){
    var hVs=_mmVsStyle(home,aSt), aVs=_mmVsStyle(away,hSt);
    if(!hVs&&!aVs) return '';
    var rows=(hVs?'<div class="mm-tr">'+abbr(home)+' vs '+aSt+' sides <b>'+hVs.gf.toFixed(2)+'</b> GF / '+hVs.ga.toFixed(2)+' GA <span class="mm-sub">'+hVs.n+'g</span></div>':'')+
      (aVs?'<div class="mm-tr">'+abbr(away)+' vs '+hSt+' sides <b>'+aVs.gf.toFixed(2)+'</b> GF / '+aVs.ga.toFixed(2)+' GA <span class="mm-sub">'+aVs.n+'g</span></div>':'');
    return '<div class="mm-brk"><div class="mm-brk-t">Vs Similar Styles</div>'+rows+'</div>';
  }
  function _mkCard(c){
    var vc=c.lean==='over'?'over':(c.lean==='under'?'under':'neutral');
    var bar='';
    if(c.bar){ var vp=Math.min(100,(c.bar.value/c.bar.max)*100), lp=(c.bar.line/c.bar.max)*100, fc=c.lean==='over'?'good':(c.lean==='under'?'bad':'neutral');
      bar='<div class="market-bar"><div class="fill '+fc+'" style="width:'+vp+'%"></div><div class="marker" style="left:'+lp+'%"></div></div>'; }
    var odds=c.oddsBody?'<div class="market-odds"><span class="odds-book">'+esc(c.oddsBook||'Book')+'</span><span class="odds-body">'+c.oddsBody+'</span></div>':'';
    return '<div class="market-card lean-'+c.lean+'">'+
      (c.sample?'<div class="sample">'+c.sample+'</div>':'')+
      '<div class="market-name"><i class="ti '+c.icon+'"></i> '+c.name+'</div>'+
      '<div class="market-headline"><div class="market-value">'+c.value+'</div><div class="market-line">'+c.line+(c.valueLabel?' · '+c.valueLabel:'')+'</div></div>'+
      '<div class="market-verdict '+vc+'">'+c.leanLabel+'</div>'+
      '<div class="market-detail">'+c.detail+'</div>'+bar+odds+'</div>';
  }
  function _mkCollapse(title,icon,meta,body,open){
    return '<details class="mk-collapse"'+(open?' open':'')+'><summary class="tp-collapse-sum"><i class="ti '+icon+'"></i><span class="tp-collapse-title">'+esc(title)+'</span>'+(meta?'<span class="tp-collapse-meta">'+meta+'</span>':'')+'<i class="ti ti-chevron-down tp-collapse-chev"></i></summary><div class="tp-collapse-body">'+body+'</div></details>';
  }
  function _mmTier(t){ var n=_eplStrength()||20; if(!t||!t._rank) return 'Mid'; var r=t._rank/n; return r<=0.3?'Strong':(r<=0.7?'Mid':'Weak'); }
  function _eplTeamTier(team){ var n=_eplStrength()||20, t=state.idx.teamMap[team]; if(!t||!t._rank) return 'Mid'; var r=t._rank;
    if(r<=Math.ceil(n*0.20)) return 'Elite'; if(r<=Math.ceil(n*0.45)) return 'Strong'; if(r<=Math.ceil(n*0.72)) return 'Mid'; return 'Weak'; }
  function _eplStyleName(team){ var t=state.idx.teamMap[team]; if(!t) return null; var st=(typeof _eplStyle==='function')?_eplStyle(t):[]; return (st&&st.length)?st[0]:null; }
  function _eplRankOf(team,key,hb){ var ts=Object.values(state.idx.teamMap||{}).filter(function(t){return t[key]!=null;}); if(!ts.length) return null;
    ts.sort(function(a,b){return hb?(b[key]-a[key]):(a[key]-b[key]);}); var i=ts.indexOf(state.idx.teamMap[team]); return i>=0?(i+1):null; }
  function _eplSplits(team,oppTier,oppStyle,fn){ var rs=_teamResults(team),all=[],bt=[],bs=[];
    rs.forEach(function(r){ var isH=r.home===team, opp=isH?r.away:r.home, v=fn(r,isH); if(v==null||isNaN(v)) return; all.push(v);
      if(oppTier && _eplTeamTier(opp)===oppTier) bt.push(v); if(oppStyle && _eplStyleName(opp)===oppStyle) bs.push(v); });
    var avg=function(a){return a.length?a.reduce(function(p,q){return p+q;},0)/a.length:null;};
    return {season:{v:avg(all),n:all.length}, tier:{v:avg(bt),n:bt.length}, style:{v:avg(bs),n:bs.length}}; }
  var _EPL_MKT={
    goals:[{key:'gf',label:'GF',hb:true,fn:function(r,isH){return isH?r.homeScore:r.awayScore;}},
           {key:'ga',label:'GA',hb:false,fn:function(r,isH){return isH?r.awayScore:r.homeScore;}}],
    corners:[{key:'cornersFor',label:'CF',hb:true,fn:function(r,isH){return r.cornersHome==null?null:(isH?r.cornersHome:r.cornersAway);}},
             {key:'cornersAgainst',label:'CA',hb:false,fn:function(r,isH){return r.cornersHome==null?null:(isH?r.cornersAway:r.cornersHome);}}],
    cards:[{key:'cards',label:'Yel',hb:true,fn:function(r,isH){return r.cardsHome==null?null:(isH?r.cardsHome:r.cardsAway);}}]
  };
  function _eplRow(lbl,d){ return '<div class="tier-row"><span class="tier-row-lbl">'+esc(lbl)+'</span><span class="tier-row-val">'+(d&&d.v!=null?d.v.toFixed(1):'<em>\u2014</em>')+(d&&d.n?'<span class="tier-row-n">'+d.n+'g</span>':'')+'</span></div>'; }
  function _eplStatBreakdown(g,stats){
    var home=g.home, away=g.away, hTier=_eplTeamTier(home), aTier=_eplTeamTier(away), hSt=_eplStyleName(home), aSt=_eplStyleName(away);
    function card(team,oppTier,oppStyle){
      var t=state.idx.teamMap[team]||{};
      var chips=stats.map(function(st){ var rk=_eplRankOf(team,st.key,st.hb); return rk?'<span class="brk-rank">'+st.label+' #'+rk+'</span>':''; }).join('');
      var rows=stats.map(function(st){ var sp=_eplSplits(team,oppTier,oppStyle,st.fn); var seasonV=(t[st.key]!=null)?+t[st.key]:sp.season.v;
        return '<div class="brk-substat">'+st.label+'</div>'+_eplRow('Season',{v:seasonV,n:sp.season.n})+
          (oppTier?_eplRow('vs '+oppTier+' tier',sp.tier):'')+(oppStyle?_eplRow('vs '+oppStyle,sp.style):''); }).join('');
      return '<div class="brk-team"><div class="brk-team-hdr">'+teamLogo(team,18)+'<span>'+esc(abbr(team))+'</span>'+chips+'</div>'+rows+'</div>';
    }
    return '<div class="brk-title">Stat breakdown \u00b7 rank \u00b7 vs similar</div><div class="brk-grid">'+card(home,aTier,aSt)+card(away,hTier,hSt)+'</div>';
  }
  function _eplMatchOdds(g, market){
    var mo=(state.data.odds&&state.data.odds.matchOdds)||[]; var nk=function(x){return String(x||'').toLowerCase().replace(/[^a-z0-9]/g,'');};
    var gh=nk(g.home), ga=nk(g.away);
    for(var i=0;i<mo.length;i++){ var m=mo[i]; if(m.market!==market) continue;
      if((m.home===g.home&&m.away===g.away)||(nk(m.home)===gh&&nk(m.away)===ga)) return m; }
    return null;
  }
  function _moStr(mo){ return (mo&&mo.over!=null&&mo.under!=null)?('O'+mo.line+' $'+(+mo.over).toFixed(2)+' \u00b7 U'+mo.line+' $'+(+mo.under).toFixed(2)):''; }
  function _eplMarketPanels(g){
    if(!_isMatchProps()) return '';
    var home=g.home, away=g.away, hG=_mmTeamGoals(home), aG=_mmTeamGoals(away);
    if(!hG||!aG) return '';
    _eplStrength();
    var hT=state.idx.teamMap[home]||{}, aT=state.idx.teamMap[away]||{};
    var lamH=(hG.gf+aG.ga)/2*1.10, lamA=(aG.gf+hG.ga)/2*0.95, m=_mmModel(lamH,lamA);
    // team header — logos, side-by-side, rank/tier/styles, + match details
    var pc=function(x){return (x*100).toFixed(0);};
    function th(team,t){ var st=(typeof _eplStyle==='function')?_eplStyle(t):[];
      var stc=(st&&st.length)?st.map(function(x){var c=(window._MM_STYLE_C&&_MM_STYLE_C[x])||'';return c?'<span style="color:'+c+'">'+esc(x)+'</span>':esc(x);}).join(' · '):'';
      return '<div class="mm-th">'+teamLogo(team,38)+'<div class="mm-th-nm">'+esc(team)+'</div><div class="mm-th-sub">'+(t._rank?'#'+t._rank+' · ':'')+_mmTier(t)+'</div>'+(stc?'<div class="mm-th-st">'+stc+'</div>':'')+'</div>'; }
    var det=[]; if(g.venue)det.push(esc(g.venue)); if(g.gw)det.push('GW'+g.gw); if(g.date)det.push(esc(String(g.date).slice(0,10)));
    var header='<div class="mm-teamhdr">'+th(home,hT)+'<span class="mm-vs">vs</span>'+th(away,aT)+'</div>'+(det.length?'<div class="mm-meta">'+det.join(' \u00b7 ')+'</div>':'');
    // Match Result (1X2) — segmented bar inside a market-card
    var favP=Math.max(m.pH,m.pD,m.pA), fav=m.pH===favP?home:(m.pA===favP?away:'Draw'), strong=favP>=0.55?'STRONG ':(favP>=0.42?'LEAN ':'');
    var resBar='<div class="mm-probbar"><div class="mm-probseg h" style="width:'+pc(m.pH)+'%"><span>'+abbr(home)+' '+pc(m.pH)+'%</span></div><div class="mm-probseg d" style="width:'+pc(m.pD)+'%"><span>'+pc(m.pD)+'%</span></div><div class="mm-probseg a" style="width:'+pc(m.pA)+'%"><span>'+abbr(away)+' '+pc(m.pA)+'%</span></div></div>';
    var tops='<div class="market-detail" style="margin-top:6px">Top scores: '+m.top.map(function(t){return '<b>'+t[0]+'</b> '+pc(t[1])+'%';}).join(' \u00b7 ')+'</div>';
    var resultCard='<div class="market-card lean-neutral"><div class="market-name"><i class="ti ti-trophy"></i> Match Result</div><div class="market-verdict '+(strong?'over':'neutral')+'" style="display:inline-block;margin:2px 0 8px">'+(strong?strong+abbr(fav):'Line-ball')+'</div>'+resBar+tops+'</div>';
    // Goals
    var gLean=m.tot>=2.5?'over':'under'; var _mog=_eplMatchOdds(g,'goals');
    var gDetail='<div>'+abbr(home)+' GF <b>'+hG.gf.toFixed(2)+'</b> / GA '+hG.ga.toFixed(2)+'</div><div>'+abbr(away)+' GF <b>'+aG.gf.toFixed(2)+'</b> / GA '+aG.ga.toFixed(2)+'</div><div style="margin-top:4px">BTTS '+pc(m.btts)+'% <span class="market-line">('+abbr(home)+' '+pc(hG.btts)+'% / '+abbr(away)+' '+pc(aG.btts)+'%)</span></div>';
    var goalsCard=_mkCard({name:'Total Goals',icon:'ti-ball-football',value:m.tot.toFixed(2),line:'O/U 2.5',valueLabel:'proj',lean:gLean,leanLabel:'LEAN '+gLean.toUpperCase(),detail:gDetail,bar:{value:m.tot,line:2.5,max:5},oddsBody:_moStr(_mog),oddsBook:(_mog?bookName(_mog.book):'')});
    // Corners
    var cornersCard='', hCn=_mmTeamCorners(home), aCn=_mmTeamCorners(away);
    if(hCn&&aCn){ var cnProj=((hCn.cf+aCn.ca)/2)+((aCn.cf+hCn.ca)/2), cnLean=cnProj>=9.5?'over':'under'; var _moc=_eplMatchOdds(g,'corners');
      var cnDetail='<div>'+abbr(home)+' CF <b>'+hCn.cf.toFixed(1)+'</b> / CA '+hCn.ca.toFixed(1)+'</div><div>'+abbr(away)+' CF <b>'+aCn.cf.toFixed(1)+'</b> / CA '+aCn.ca.toFixed(1)+'</div>';
      cornersCard=_mkCard({name:'Total Corners',icon:'ti-flag',value:cnProj.toFixed(1),line:'O/U 9.5',valueLabel:'proj',lean:cnLean,leanLabel:'LEAN '+cnLean.toUpperCase(),detail:cnDetail,bar:{value:cnProj,line:9.5,max:16},oddsBody:_moStr(_moc),oddsBook:(_moc?bookName(_moc.book):'')}); }
    // Cards (referee-adjusted)
    var cardsCard='', hY=_mmTeamY(home), aY=_mmTeamY(away);
    if(hY&&aY){ var _ref=(typeof _eplRefFor==='function')?_eplRefFor(g):'', _rs=((state.data.referees||{}).refStats)||{}, _rr=_ref&&_rs[_ref], _rm=1.0, _adj='';
      if(_rr&&_rr.cardsPG){ var _a=Object.keys(_rs).map(function(k){return +_rs[k].cardsPG;}).filter(function(x){return x>0;}); var _lg=_a.length?_a.reduce(function(p,q){return p+q;},0)/_a.length:_rr.cardsPG; if(_lg>0){ _rm=_rr.cardsPG/_lg; _adj='<div style="margin-top:4px" class="market-line">Adj: ref '+((_rm-1)*100).toFixed(0)+'% ('+(hY.f+aY.f).toFixed(1)+' → '+((hY.f+aY.f)*_rm).toFixed(1)+')</div>'; } }
      var ctot=(hY.f+aY.f)*_rm, cdLean=ctot>=4.5?'over':'under'; var _mod=_eplMatchOdds(g,'cards');
      var cdDetail='<div>'+abbr(home)+' Y <b>'+hY.f.toFixed(2)+'</b></div><div>'+abbr(away)+' Y <b>'+aY.f.toFixed(2)+'</b></div>'+_adj;
      cardsCard=_mkCard({name:'Total Cards',icon:'ti-cards',value:ctot.toFixed(1),line:'O/U 4.5',valueLabel:'proj',lean:cdLean,leanLabel:'LEAN '+cdLean.toUpperCase(),detail:cdDetail,bar:{value:ctot,line:4.5,max:9},oddsBody:_moStr(_mod),oddsBook:(_mod?bookName(_mod.book):'')}); }
    return _mkCollapse('Match','ti-trophy','',header+resultCard,true)+
      _mkCollapse('Goals','ti-ball-football',m.tot.toFixed(1),goalsCard+_eplStatBreakdown(g,_EPL_MKT.goals),false)+
      (cornersCard?_mkCollapse('Corners','ti-flag','',cornersCard+_eplStatBreakdown(g,_EPL_MKT.corners),false):'')+
      (cardsCard?_mkCollapse('Cards','ti-cards','',cardsCard+_eplStatBreakdown(g,_EPL_MKT.cards),false):'');
  }
  function gameMatchMarkets(home,away){
    if(!_isMatchProps()) return '';
    var hG=_mmTeamGoals(home), aG=_mmTeamGoals(away);
    if(!hG||!aG) return '';   // need both sides (promoted teams w/o data skip gracefully)
    var lamH=(hG.gf+aG.ga)/2*1.10, lamA=(aG.gf+hG.ga)/2*0.95;   // light home tilt
    var m=_mmModel(lamH,lamA);
    var favP=Math.max(m.pH,m.pD,m.pA), fav=m.pH===favP?home:(m.pA===favP?away:'Draw');
    var strong=favP>=0.55?'STRONG ':(favP>=0.42?'LEAN ':'');
    // 1X2 header
    var bar='<div class="mm-bar"><div class="mm-seg mm-h" style="width:'+(m.pH*100).toFixed(0)+'%"><span>'+abbr(home)+' '+(m.pH*100).toFixed(0)+'%</span></div>'+
      '<div class="mm-seg mm-d" style="width:'+(m.pD*100).toFixed(0)+'%"><span>Draw '+(m.pD*100).toFixed(0)+'%</span></div>'+
      '<div class="mm-seg mm-a" style="width:'+(m.pA*100).toFixed(0)+'%"><span>'+abbr(away)+' '+(m.pA*100).toFixed(0)+'%</span></div></div>';
    var tops='<div class="mm-tops">Top scores: '+m.top.map(function(t){return '<b>'+t[0]+'</b> '+(t[1]*100).toFixed(0)+'%';}).join(' &middot; ')+'</div>';
    var hSt=_mmStyleOf(home), aSt=_mmStyleOf(away);
    var styleLine=(hSt||aSt)?'<div class="mm-styles">'+(hSt?'<span class="mm-style" style="border-color:'+(_MM_STYLE_C[hSt]||'#888')+'">'+abbr(home)+' &middot; <b style="color:'+(_MM_STYLE_C[hSt]||'#888')+'">'+hSt+'</b></span>':'')+(aSt?'<span class="mm-style" style="border-color:'+(_MM_STYLE_C[aSt]||'#888')+'">'+abbr(away)+' &middot; <b style="color:'+(_MM_STYLE_C[aSt]||'#888')+'">'+aSt+'</b></span>':'')+'</div>':'';
    var head='<div class="mm-head"><span class="mm-1x2">1X2</span>'+(strong?'<span class="mm-badge" style="background:'+(favP>=0.55?'#22c55e':'#86efac')+'">'+strong+abbr(fav)+'</span>':'')+'</div>'+bar+tops+styleLine;
    // Goals
    var gl=_mmLean(m.tot,2.5,'OVER','UNDER'), bl=_mmLean(m.btts*100,50,'YES','NO');
    var goals='<div class="mm-card"><div class="mm-ct"><i class="ti ti-ball-football"></i> Goals</div>'+
      '<div class="mm-line"><span class="mm-proj">'+m.tot.toFixed(2)+'</span> total &middot; O/U 2.5 <span class="mm-lean" style="color:'+gl.c+'">'+gl.t+'</span></div>'+
      '<div class="mm-teams"><div class="mm-tr">'+teamLogo(home,18)+abbr(home)+' <b>'+hG.gf.toFixed(2)+'</b> GF '+_mmRank(home,'gf',true)+' / '+hG.ga.toFixed(2)+' GA '+_mmRank(home,'ga',false)+'</div>'+
      '<div class="mm-tr">'+teamLogo(away,18)+abbr(away)+' <b>'+aG.gf.toFixed(2)+'</b> GF '+_mmRank(away,'gf',true)+' / '+aG.ga.toFixed(2)+' GA '+_mmRank(away,'ga',false)+'</div></div>'+
      '<div class="mm-line"><span class="mm-proj">'+(m.btts*100).toFixed(0)+'%</span> BTTS &middot; <span class="mm-lean" style="color:'+bl.c+'">'+bl.t+'</span> <span class="mm-sub">('+abbr(home)+' '+(hG.btts*100).toFixed(0)+'% / '+abbr(away)+' '+(aG.btts*100).toFixed(0)+'%)</span></div>'+_mmVsBreakGen(home,away,aSt,hSt,function(r,isH){return {f:isH?r.homeScore:r.awayScore,a:isH?r.awayScore:r.homeScore};},'GF','GA')+'</div>';
    // Corners (once the per-fixture stats backfill has populated cornersFor/cornersAgainst)
    var corners='';
    var hCn=_mmTeamCorners(home), aCn=_mmTeamCorners(away);
    if(hCn&&aCn){ var cnProj=((hCn.cf+aCn.ca)/2)+((aCn.cf+hCn.ca)/2), cnl=_mmLean(cnProj,9.5,'OVER','UNDER');
      corners='<div class="mm-card"><div class="mm-ct"><i class="ti ti-corner-up-right"></i> Corners</div>'+
        '<div class="mm-line"><span class="mm-proj">'+cnProj.toFixed(1)+'</span> total &middot; O/U 9.5 <span class="mm-lean" style="color:'+cnl.c+'">'+cnl.t+'</span></div>'+
        '<div class="mm-teams"><div class="mm-tr">'+teamLogo(home,18)+abbr(home)+' <b>'+hCn.cf.toFixed(1)+'</b> CF '+_mmRank(home,'cornersFor',true)+' / '+hCn.ca.toFixed(1)+' CA '+_mmRank(home,'cornersAgainst',false)+'</div>'+
        '<div class="mm-tr">'+teamLogo(away,18)+abbr(away)+' <b>'+aCn.cf.toFixed(1)+'</b> CF '+_mmRank(away,'cornersFor',true)+' / '+aCn.ca.toFixed(1)+' CA '+_mmRank(away,'cornersAgainst',false)+'</div></div>'+_mmVsBreakGen(home,away,aCn&&aSt,hSt,function(r,isH){if(r.cornersHome==null)return null;return {f:isH?r.cornersHome:r.cornersAway,a:isH?r.cornersAway:r.cornersHome};},'CF','CA')+'</div>'; }
    // Cards (yellows per match, from /fixtures/statistics via results - reliable + deduped)
    var cards='';
    var hY=_mmTeamY(home), aY=_mmTeamY(away);
    if(hY&&aY){
      var _g=(state.data.fixture||[]).find(function(x){return x.home===home&&x.away===away;})||{home:home,away:away};
      var _ref=(typeof _eplRefFor==='function')?_eplRefFor(_g):'', _rs=((state.data.referees||{}).refStats)||{}, _rr=_ref&&_rs[_ref], _rm=1.0, _rnote='';
      if(_rr&&_rr.cardsPG){ var _arr=Object.keys(_rs).map(function(k){return +_rs[k].cardsPG;}).filter(function(x){return x>0;}); var _lgr=_arr.length?_arr.reduce(function(a,b){return a+b;},0)/_arr.length:_rr.cardsPG; if(_lgr>0){ _rm=_rr.cardsPG/_lgr; _rnote=' &middot; <span class="mm-sub">'+esc(_ref)+' '+_rm.toFixed(2)+'\u00d7</span>'; } }
      var ctot=(hY.f+aY.f)*_rm, cl=_mmLean(ctot,4.5,'OVER','UNDER');
      cards='<div class="mm-card"><div class="mm-ct"><i class="ti ti-cards"></i> Cards <span class="mm-sub">yellows, referee-adjusted</span></div>'+
        '<div class="mm-line"><span class="mm-proj">'+ctot.toFixed(2)+'</span> total &middot; O/U 4.5 <span class="mm-lean" style="color:'+cl.c+'">'+cl.t+'</span>'+_rnote+'</div>'+
        '<div class="mm-teams"><div class="mm-tr">'+teamLogo(home,18)+abbr(home)+' <b>'+hY.f.toFixed(2)+'</b> Y '+_mmRank(home,'cardsPG',true)+' / '+hY.a.toFixed(2)+' drawn</div>'+
        '<div class="mm-tr">'+teamLogo(away,18)+abbr(away)+' <b>'+aY.f.toFixed(2)+'</b> Y '+_mmRank(away,'cardsPG',true)+' / '+aY.a.toFixed(2)+' drawn</div></div>'+
        _mmVsBreakGen(home,away,aSt,hSt,function(r,isH){if(r.cardsHome==null)return null;return {f:isH?r.cardsHome:r.cardsAway,a:isH?r.cardsAway:r.cardsHome};},'Y','opp')+'</div>'; }
    return '<div class="game-sec mm-sec"><div class="gs-title"><i class="ti ti-clipboard-data"></i> Match Markets &middot; model off recent GF/GA</div>'+head+goals+corners+cards+'</div>';
  }

  // ---- soccer match-trend sections (EPL): By The Halves / Shape Trend / Recent Games ----
  function _isMatchProps(){ return SPORT.key==='epl'; }
  function _teamResults(team){ return (state.data.results||[]).filter(function(r){return r.home===team||r.away===team;}); }
  function _eplByTheHalves(team){
    var rs=_teamResults(team).filter(function(r){return r.htHome!=null;});
    if(!rs.length) return '';
    var n=rs.length,h1f=0,h1a=0,h2f=0,h2a=0,s1=0,s2=0,c1=0,c2=0;
    rs.forEach(function(r){ var isH=r.home===team;
      var ftF=isH?r.homeScore:r.awayScore, ftA=isH?r.awayScore:r.homeScore;
      var htF=(isH?r.htHome:r.htAway)||0, htA=(isH?r.htAway:r.htHome)||0;
      var g1f=htF,g1a=htA,g2f=Math.max(0,ftF-htF),g2a=Math.max(0,ftA-htA);
      h1f+=g1f;h1a+=g1a;h2f+=g2f;h2a+=g2a;
      if(g1f>0)s1++; if(g2f>0)s2++; if(g1a>0)c1++; if(g2a>0)c2++; });
    var pct=function(x){return Math.round(x/n*100);};
    var a1=h1f/n,a2=h2f/n, co=a2>a1*1.15?'STRONG 2H ATTACK':(a1>a2*1.15?'STRONG 1H ATTACK':'BALANCED HALVES');
    var d1=h1a/n,d2=h2a/n, co2=d2>d1*1.2?' · LEAKY 2H':(d1>d2*1.2?' · LEAKY 1H':'');
    function cell(l,v,cls){return '<div class="bth-cell"><div class="bth-v'+(cls?' '+cls:'')+'">'+v+'</div><div class="bth-l">'+l+'</div></div>';}
    var body='<div class="bth-meta">'+n+' matches \u00b7 1H vs 2H</div>'+
      '<div class="bth-callout">'+co+co2+'</div>'+
      '<div class="bth-grid">'+cell('1H GF',(h1f/n).toFixed(2))+cell('2H GF',(h2f/n).toFixed(2))+cell('1H GA',(h1a/n).toFixed(2))+cell('2H GA',(h2a/n).toFixed(2))+'</div>'+
      '<div class="bth-grid bth-pct">'+cell('Scored 1H',pct(s1)+'%')+cell('Scored 2H',pct(s2)+'%')+cell('Conceded 1H',pct(c1)+'%')+cell('Conceded 2H',pct(c2)+'%')+'</div>';
    var _ev=(state.data.events&&state.data.events.byTeam&&state.data.events.byTeam[team]);
    if(_ev&&_ev.timing){ var sc=_ev.timing.scored,cc=_ev.timing.conceded,mx=Math.max.apply(null,[1].concat(sc,cc)),L=['0-15','16-30','31-45','46-60','61-75','76-90'];
      var bars=L.map(function(l,i){return '<div class="gtb-col"><div class="gtb-bars"><div class="gtb-bar sc" style="height:'+Math.round((sc[i]||0)/mx*100)+'%"></div><div class="gtb-bar cc" style="height:'+Math.round((cc[i]||0)/mx*100)+'%"></div></div><div class="gtb-lbl">'+l+'</div><div class="gtb-n">'+(sc[i]||0)+'-'+(cc[i]||0)+'</div></div>';}).join('');
      body+='<div class="bth-sub">Goal timing \u00b7 15-min buckets</div><div class="gtb-row">'+bars+'</div><div class="gtb-legend"><span class="gtb-key sc"></span>Scored<span class="gtb-key cc"></span>Conceded</div>'; }
    return _mkCollapse('By The Halves','ti-ball-football',n+'g',body,false);
  }
  function _eplRecentGames(team){
    var rs=_teamResults(team).slice(-8).reverse();
    if(!rs.length) return '';
    var MON=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
    function tag(t,on){ return '<span class="rg-tag'+(on?' on':'')+'">'+t+'</span>'; }
    var rows=rs.map(function(r){
      var isH=r.home===team, opp=isH?r.away:r.home, ff=isH?r.homeScore:r.awayScore, aa=isH?r.awayScore:r.homeScore;
      var res=ff>aa?'W':ff<aa?'L':'D', tot=ff+aa, btts=(ff>0&&aa>0), over=tot>=3;
      var corn=(r.cornersHome!=null)?(r.cornersHome+r.cornersAway):null, cards=(r.cardsHome!=null)?(r.cardsHome+r.cardsAway):null;
      var dt=r.date?new Date(r.date):null, ds=(dt&&!isNaN(dt))?(dt.getUTCDate()+' '+MON[dt.getUTCMonth()]):'';
      return '<div class="rg2-row">'+
        '<span class="rg2-res st-'+res.toLowerCase()+'">'+res+'</span>'+
        '<span class="rg2-date">'+esc(ds)+'</span>'+
        '<span class="rg2-opp">'+(isH?'v ':'@ ')+teamLogo(opp,15)+'<span class="rg2-onm">'+esc(abbr(opp))+'</span></span>'+
        '<span class="rg2-sc">'+ff+'-'+aa+'</span>'+
        '<span class="rg2-tags">'+tag(btts?'BTTS':'no BTTS',btts)+tag(over?'O2.5':'U2.5',over)+
          (corn!=null?tag(corn+'C',false):'')+(cards!=null?tag(cards+'Y',false):'')+'</span></div>';
    }).join('');
    return _mkCollapse('Recent Games','ti-history',rs.length+'',rows,false);
  }
  function _eplFirstLast(team){
    var ev=(state.data.events&&state.data.events.byTeam&&state.data.events.byTeam[team]); if(!ev) return '';
    var pc=function(x){return Math.round((x||0)*100)+'%';};
    function sc(l,v){return '<div class="fl-stat"><div class="fl-v">'+v+'</div><div class="fl-l">'+l+'</div></div>';}
    var top='<div class="fl-stats">'+sc('Opens scoring',pc(ev.opensScoring))+sc('Concedes 1st',pc(ev.concedesFirst))+sc('No 1H goal',pc(ev.no1hGoal))+sc('Carded 1st',pc(ev.cardedFirst))+'</div>'+
      '<div class="fl-stats fl-mins">'+sc('Avg 1st scored',ev.avgFirstScored?ev.avgFirstScored+'\u2032':'\u2014')+sc('Avg 1st conceded',ev.avgFirstConceded?ev.avgFirstConceded+'\u2032':'\u2014')+'</div>';
    function lst(t,a,sfx){ if(!a||!a.length) return ''; return '<div class="fl-sec">'+t+'</div>'+a.map(function(p){return '<div class="fl-row"><span class="fl-nm">'+esc(p[0])+'</span><span class="fl-c">'+p[1]+' '+sfx+'</span></div>';}).join(''); }
    var body=top+lst('First goal scorers',ev.firstScorers,'opens')+lst('Last goal scorers',ev.lastScorers,'last')+lst('Late merchants (\u226576\u2032)',ev.lateScorers,'late')+lst('First to be carded',ev.firstCarded,'bookings');
    return _mkCollapse('First & Last','ti-flag',(ev.matches||0)+'g',body,false);
  }
  function _eplSubTrends(team){
    var ev=(state.data.events&&state.data.events.byTeam&&state.data.events.byTeam[team]); if(!ev||!ev.hooked||!ev.hooked.length) return '';
    var rows=ev.hooked.map(function(h){return '<div class="st2-row"><span class="st2-nm">'+esc(h.player)+'</span><span class="st2-meta">off ~'+h.offAvg+'\u2032 \u00b7 '+h.n+'\u00d7</span></div>';}).join('');
    return _mkCollapse('Substitution Trends','ti-arrows-shuffle',ev.hooked.length+'','<div class="bth-meta">Hooked early \u00b7 prop-carryover risk</div>'+rows,false);
  }
  function _eplTrendBlock(team){
    const rs=_teamResults(team); let out='';
    out+=_eplByTheHalves(team);
    out+=_eplRecentGames(team);
    out+=_eplFirstLast(team);
    out+=_eplSubTrends(team);
    return out;
  }
  function gameByHalves(home,away){
    if(!_isMatchProps()) return '';
    var R=state.data.results||[]; if(!R.some(function(r){return r.htHome!=null;})) return '';
    function calc(team){ var rs=_teamResults(team).slice(-20), h1f=0,h1a=0,h2f=0,h2a=0,n=0;
      rs.forEach(function(r){ var isH=r.home===team; var ftF=isH?r.homeScore:r.awayScore, ftA=isH?r.awayScore:r.homeScore;
        var htF=(isH?r.htHome:r.htAway)||0, htA=(isH?r.htAway:r.htHome)||0;
        h1f+=htF; h1a+=htA; h2f+=(ftF-htF); h2a+=(ftA-htA); n++; });
      return n?{n:n,h1f:h1f/n,h1a:h1a/n,h2f:h2f/n,h2a:h2a/n}:null; }
    function row(team){ var d=calc(team); if(!d) return '';
      return '<div class="bh-row">'+teamLogo(team,20)+'<span class="bh-tm">'+abbr(team)+'</span>'+
        '<span class="bh-cell">1st <b>'+d.h1f.toFixed(1)+'</b> / '+d.h1a.toFixed(1)+'</span>'+
        '<span class="bh-cell">2nd <b>'+d.h2f.toFixed(1)+'</b> / '+d.h2a.toFixed(1)+'</span></div>'; }
    var b=row(home)+row(away); if(!b) return '';
    return '<div class="game-sec"><div class="gs-title"><i class="ti ti-clock-hour-6"></i> By The Halves &middot; goals for / against per half (last 20)</div><div class="bh-hint">for / against, per game</div>'+b+'</div>';
  }
  function gameShapeTrend(home,away){
    if(!_isMatchProps()) return '';
    var R=state.data.results||[]; if(!R.length) return '';
    function formStr(team){ return _teamResults(team).slice(-6).map(function(r){ var isH=r.home===team, ff=isH?r.homeScore:r.awayScore, aa=isH?r.awayScore:r.homeScore;
      var res=ff>aa?'W':ff<aa?'L':'D'; return '<span class="st-pip st-'+res.toLowerCase()+'">'+res+'</span>'; }).join(''); }
    function row(team){ var rs=_teamResults(team).slice(-6), gf=0,ga=0; if(!rs.length) return '';
      rs.forEach(function(r){ var isH=r.home===team; gf+=isH?r.homeScore:r.awayScore; ga+=isH?r.awayScore:r.homeScore; });
      return '<div class="st-row">'+teamLogo(team,20)+'<span class="st-tm">'+abbr(team)+'</span><span class="st-form">'+formStr(team)+'</span><span class="st-gd">'+gf+'-'+ga+' &middot; L'+rs.length+'</span></div>'; }
    var b=row(home)+row(away); if(!b) return '';
    return '<div class="game-sec"><div class="gs-title"><i class="ti ti-trending-up"></i> Shape Trend &middot; recent form (last 6)</div>'+b+'</div>';
  }
  function gameRecentGames(home,away){
    if(!_isMatchProps()) return '';
    var R=state.data.results||[]; if(!R.length) return '';
    function col(team){ var rs=_teamResults(team).slice(-5).reverse(); if(!rs.length) return '';
      var rows=rs.map(function(r){ var isH=r.home===team, opp=isH?r.away:r.home, ff=isH?r.homeScore:r.awayScore, aa=isH?r.awayScore:r.homeScore;
        var res=ff>aa?'W':ff<aa?'L':'D';
        return '<div class="rg-row"><span class="rg-res st-'+res.toLowerCase()+'">'+res+'</span><span class="rg-opp">'+(isH?'v ':'@ ')+abbr(opp)+'</span><span class="rg-sc">'+ff+'-'+aa+'</span></div>'; }).join('');
      return '<div class="rg-col"><div class="rg-hd">'+teamLogo(team,18)+'<span>'+abbr(team)+'</span></div>'+rows+'</div>'; }
    var b=col(home)+col(away); if(!b) return '';
    return '<div class="game-sec"><div class="gs-title"><i class="ti ti-list-details"></i> Recent Games</div><div class="rg-grid">'+b+'</div></div>';
  }
  ...
  function gameLastMeeting(home,away){
    ...
    let perf='';
    let _lmLogs=null;
    if(mid){ _lmLogs=(state.data.gamelogs||[]).filter(r=>r.MatchId===mid); }
    else if(SPORT.key==='mlb' && off && off.date){ _lmLogs=(state.data.gamelogs||[]).filter(r=>r.Date===off.date && (r.Team===home||r.Team===away)); }
    if(_lmLogs && _lmLogs.length){
      const logs=_lmLogs;
      const hl=logs.filter(r=>r.Team===home && r.role!=='pitch'), al=logs.filter(r=>r.Team===away && r.role!=='pitch');
      ...
      BET_STATS.filter(([k])=>k!=='K').forEach(([k,l])=>{ perf+='<div class="lm-gc">'+chip(topOf(hl,k),k)+'</div><div class="lm-gk">'+l+'</div><div class="lm-gc r">'+chip(topOf(al,k),k)+'</div>'; });
      perf+='</div>';
      if(SPORT.key==='mlb'){
        const _pf=logs.filter(r=>r.role==='pitch');
        const _starterOf=arr=>arr.length?arr.reduce((m,r)=>((r.outs||0)>(m.outs||0)?r:m)):null;
        const _sh=_starterOf(_pf.filter(r=>r.Team===home)), _sa=_starterOf(_pf.filter(r=>r.Team===away));
        const _pchip=r=>r?'<span class="lm-chip">'+esc(r.Player)+' <b>'+(r.K||0)+'K</b>'+(r.IP!=null?' '+r.IP+'ip':'')+(r.ER!=null?' '+r.ER+'er':'')+'</span>':'<span class="lm-chip none">\u2014</span>';
        if(_sh||_sa) perf+='<div class="lm-grid" style="margin-top:6px"><div class="lm-gc">'+_pchip(_sh)+'</div><div class="lm-gk">Starter (K / IP / ER)</div><div class="lm-gc r">'+_pchip(_sa)+'</div></div>';
      }
    }
  ...
  function allPlayStyles(p,pg){
    if(SPORT.key!=='afl'&&SPORT.key!=='aflw'){ const _S=_signals(); if(_S&&_S.playStyles) return _S.playStyles(p,pg); return [pg]; }
  ...
  let DVPS_OPTS, ELITE_OPTS;  // from SPORT_CONFIG
  ...
  // ===================== MLB Matchup Analyzer (Phase 1) =====================
  var _MA_LEAGUE={};
  function _maBatRaw(p,m){
    switch(m){
      case 'AVG': return p.AVG; case 'SLG': return p.SLG;
      case 'ISO': return (p.SLG!=null&&p.AVG!=null)?+(p.SLG-p.AVG).toFixed(3):null;
      case 'xwoba': return p.statcast&&p.statcast.xwoba; case 'barrel': return p.statcast&&p.statcast.barrelPct;
      case 'hardHit': return p.statcast&&p.statcast.hardHitPct; case 'ev': return p.statcast&&p.statcast.ev;
      case 'Kpct': return p.PA?+((p.SO||0)/p.PA*100).toFixed(1):null;
      case 'BBpct': return p.PA?+((p.BB||0)/p.PA*100).toFixed(1):null;
      case 'HRpct': return p.PA?+((p.HR||0)/p.PA*100).toFixed(1):null;
    } return null;
  }
  function _maPitRaw(p,m){
    switch(m){
      case 'oppAVG': return p.oppAVG; case 'xwoba': return p.statcast&&p.statcast.xwoba;
      case 'barrel': return p.statcast&&p.statcast.barrelPct; case 'hardHit': return p.statcast&&p.statcast.hardHitPct;
      case 'ev': return p.statcast&&p.statcast.ev; case 'K9': return p.K9; case 'HR9': return p.HR9;
    } return null;
  }
  function _maLeagueArr(role,metric){
    var key=role+'|'+metric; if(_MA_LEAGUE[key]) return _MA_LEAGUE[key];
    var arr=[];
    (state.idx.players||[]).forEach(function(p){
      if(role==='bat'&&p.role==='pitch') return; if(role==='pit'&&p.role!=='pitch') return;
      var v=(role==='bat')?_maBatRaw(p,metric):_maPitRaw(p,metric);
      if(v!=null&&!isNaN(v)) arr.push(+v);
    });
    arr.sort(function(a,b){return a-b;});
    return _MA_LEAGUE[key]=arr;
  }
  function _maPctl(role,metric,val){
    if(val==null||isNaN(val)) return null;
    var arr=_maLeagueArr(role,metric); if(arr.length<8) return null;
    var n=0; for(var i=0;i<arr.length;i++) if(arr[i]<=val) n++;
    return Math.round(n/arr.length*100);
  }
  function _maBatVal(bat,pitHand,m){
    var sp=pitHand==='L'?bat.splitVsL:bat.splitVsR;
    if(m==='AVG') return (sp&&sp.AVG!=null)?sp.AVG:bat.AVG;
    if(m==='SLG') return (sp&&sp.SLG!=null)?sp.SLG:bat.SLG;
    if(m==='ISO'){ var s=(sp&&sp.SLG!=null)?sp.SLG:bat.SLG, a=(sp&&sp.AVG!=null)?sp.AVG:bat.AVG; return (s!=null&&a!=null)?+(s-a).toFixed(3):null; }
    return _maBatRaw(bat,m);
  }
  function _maPitValV(pit,batHand,m){
    var sp=batHand==='L'?pit.splitVsL:pit.splitVsR;
    if(m==='oppAVG') return (sp&&sp.oppAVG!=null)?sp.oppAVG:pit.oppAVG;
    return _maPitRaw(pit,m);
  }
  var MA_ROWS=[
    {lbl:'AVG', bm:'AVG', pm:'oppAVG', dec:3, bdir:1, pdir:1},
    {lbl:'ISO', bm:'ISO', pm:null, dec:3, bdir:1},
    {lbl:'xwOBA', bm:'xwoba', pm:'xwoba', dec:3, bdir:1, pdir:1},
    {lbl:'Barrel%', bm:'barrel', pm:'barrel', dec:1, u:'%', bdir:1, pdir:1},
    {lbl:'Hard-Hit%', bm:'hardHit', pm:'hardHit', dec:1, u:'%', bdir:1, pdir:1},
    {lbl:'Avg EV', bm:'ev', pm:'ev', dec:1, u:'', bdir:1, pdir:1},
    {lbl:'K rate', bm:'Kpct', pm:'K9', dec:1, u:'%', pu:'', bdir:-1, pdir:-1},
    {lbl:'BB%', bm:'BBpct', pm:null, dec:1, u:'%', bdir:1},
    {lbl:'HR rate', bm:'HRpct', pm:'HR9', dec:1, u:'%', pu:'', bdir:1, pdir:1}
  ];
  function _maFmt(v,dec,u){ if(v==null||isNaN(v)) return '\u2014'; var s=(+v).toFixed(dec); if(dec===3) s=s.replace(/^0/,''); return s+(u||''); }
  function _maEdge(pctl,dir){ if(pctl==null) return null; return (dir<0)?(100-pctl):pctl; }
  function _maColor(edge){ if(edge==null) return 'var(--border)'; if(edge>=66) return '#22c55e'; if(edge>=40) return '#eab308'; return '#ef4444'; }
  function _maBar(pctl,edge,side){
    if(pctl==null) return '<div class="ma-bar '+side+'"><span class="ma-na">\u2014</span></div>';
    return '<div class="ma-bar '+side+'"><span class="ma-fill" style="width:'+pctl+'%;background:'+_maColor(edge)+'"></span><span class="ma-pct">'+pctl+'</span></div>';
  }
  var PITCH_NAMES={FF:'Fastball',FA:'Fastball',SI:'Sinker',FT:'2-Seam',FC:'Cutter',SL:'Slider',CH:'Changeup',CU:'Curveball',KC:'Knuckle-Curve',FS:'Splitter',KN:'Knuckleball',ST:'Sweeper',SV:'Slurve',FO:'Forkball',EP:'Eephus',SC:'Screwball'};
  function _maPitchName(c){ return PITCH_NAMES[c]||c; }
  function _maArsWoba(bat,arsenal,off){
    if(!bat.vsPitch||!arsenal) return null;
    var num=0,den=0;
    Object.keys(arsenal).forEach(function(pt){
      if(off&&off[pt]) return; var u=arsenal[pt]; if(u==null||u<=0) return;
      var vp=bat.vsPitch[pt]; if(!vp||vp.woba==null) return; num+=u*vp.woba; den+=u;
    });
    return den>0?+(num/den).toFixed(3):null;
  }
  function _maArsPctl(arsenal,off,val){
    if(val==null) return null; var arr=[];
    (state.idx.players||[]).forEach(function(p){ if(p.role==='pitch'||!p.vsPitch) return; var w=_maArsWoba(p,arsenal,off); if(w!=null) arr.push(w); });
    if(arr.length<8) return null; arr.sort(function(a,b){return a-b;});
    var n=0; for(var i=0;i<arr.length;i++) if(arr[i]<=val) n++; return Math.round(n/arr.length*100);
  }
  window._maTogglePitch=function(code){ state.view.maPitchOff=state.view.maPitchOff||{}; state.view.maPitchOff[code]=!state.view.maPitchOff[code]; (typeof _renderAppPreserving==='function'?_renderAppPreserving:_renderApp)(); };
  function _mlbMatchupAnalyzer(g){
    var bats=(state.idx.players||[]).filter(function(p){return p.role!=='pitch'&&(p.team===g.home||p.team===g.away)&&isPlaying(p.name,p.team);});
    if(!bats.length) return '';
    bats.sort(function(a,b){return (a.order||99)-(b.order||99)||a.name.localeCompare(b.name);});
    var selName=(state.view.maBatter&&bats.some(function(b){return b.name===state.view.maBatter;}))?state.view.maBatter:bats[0].name;
    var bat=bats.find(function(b){return b.name===selName;});
    var oppTeam=bat.team===g.home?g.away:g.home;
    var oppPitcherName=bat.team===g.home?g.awayPitcher:g.homePitcher;
    var pit=_mlbFindPitcher(oppPitcherName);
    var sel='<div class="deg-bookbar"><span class="deg-bl">Batter</span><select class="ob-sel" onchange="_maSetBatter(this.value)">'+bats.map(function(b){return '<option value="'+esc(b.name).replace(/"/g,'&quot;')+'"'+(b.name===selName?' selected':'')+'>'+esc(b.name)+' \u00b7 '+esc(b.team)+'</option>';}).join('')+'</select></div>';
    var h='<details class="tp-collapse c-green" open><summary class="tp-collapse-sum"><i class="ti ti-chart-dots"></i><span class="tp-collapse-title">Matchup Analyzer</span><i class="ti ti-chevron-down tp-collapse-chev"></i></summary><div class="tp-collapse-body">'+sel;
    if(!pit){ h+='<div class="tp-body-meta" style="border:0">Opposing starter for '+esc(bat.team)+' not posted yet.</div></div></details>'; return h; }
    var bH=bat.bats||'R', pH=pit.throws||'R';
    h+='<div class="ma-vs"><div class="ma-p"><div class="ma-nm">'+esc(pit.name)+'</div><div class="ma-sub">'+esc(oppTeam)+' \u00b7 SP \u00b7 '+pH+'HP</div></div>'+
       '<div class="ma-mid">vs</div>'+
       '<div class="ma-p r"><div class="ma-nm">'+esc(bat.name)+'</div><div class="ma-sub">'+esc(bat.team)+(bat.position?' \u00b7 '+esc(bat.position):'')+' \u00b7 '+bH+'HB</div></div></div>';
    h+='<div class="ma-splitnote"><span>Pitcher vs '+bH+'HB</span><span>Batter vs '+pH+'HP</span></div>';
    var bv=(bat.bvp&&(bat.bvp[String(pit.id)]||bat.bvp[pit.id]))||null;
    if(bv&&(bv.PA||0)>0){ h+='<div class="ma-h2h"><b>H2H</b> \u00b7 '+[bv.PA+' PA',bv.H!=null?bv.H+' H':'',bv.TB!=null?bv.TB+' TB':'',bv.HR!=null?bv.HR+' HR':'',bv.AVG!=null?_maFmt(bv.AVG,3,'')+' AVG':''].filter(Boolean).join(' \u00b7 ')+'</div>'; }
    var pk=g.park||{}, hrF=(pk.hrFactor!=null?pk.hrFactor:(pk.hr!=null?pk.hr:null)), wm=(g.weather&&g.weather.hrMult!=null)?g.weather.hrMult:1;
    var cond=[(pk.name||g.venue||'')]; if(hrF!=null) cond.push('Park HR '+(hrF>=1?'+':'')+Math.round((hrF-1)*100)+'%'); if(wm!==1) cond.push('Wind '+(wm>=1?'+':'')+Math.round((wm-1)*100)+'%');
    h+='<div class="ma-cond"><i class="ti ti-map-pin"></i> '+cond.filter(Boolean).join(' \u00b7 ')+'</div>';
    if(pit.arsenal && Object.keys(pit.arsenal).length){
      var off=state.view.maPitchOff||{}, ars=pit.arsenal;
      var maxU=Math.max.apply(null,Object.keys(ars).map(function(k){return ars[k];})), toPct=function(u){return maxU<=1.5?u*100:u;};
      var pts=Object.keys(ars).filter(function(k){return ars[k]>0;}).sort(function(a,b){return ars[b]-ars[a];});
      h+='<div class="ma-arsenal"><div class="ma-arsenal-lbl">'+esc(pit.name)+' arsenal \u00b7 tap to filter</div><div class="ma-chips">';
      pts.forEach(function(pt){ var on=!off[pt]; h+='<button class="ma-chip'+(on?' on':'')+'" onclick="_maTogglePitch(\''+pt+'\')">'+_maPitchName(pt)+' '+Math.round(toPct(ars[pt]))+'%</button>'; });
      h+='</div>';
      var aw=_maArsWoba(bat,ars,off), ap=(aw!=null)?_maArsPctl(ars,off,aw):null;
      if(aw!=null){ h+='<div class="ma-arsrow"><span class="ma-arslbl">Batter wOBA vs this mix</span><span class="ma-arsval" style="color:'+_maColor(ap)+'">'+_maFmt(aw,3,'')+(ap!=null?' \u00b7 '+ap+'th pctl':'')+'</span></div>'; }
      else { h+='<div class="ma-arsrow"><span class="ma-arslbl">Batter wOBA vs this mix</span><span class="ma-na2">by-pitch data lands on the next build</span></div>'; }
      h+='</div>';
    }

    h+='<div class="ma-rows">';
    MA_ROWS.forEach(function(row){
      var bVal=_maBatVal(bat,pH,row.bm), bP=_maPctl('bat',row.bm,bVal), bE=_maEdge(bP,row.bdir);
      var pVal=row.pm?_maPitValV(pit,bH,row.pm):null, pP=row.pm?_maPctl('pit',row.pm,pVal):null, pE=_maEdge(pP,row.pdir||1);
      h+='<div class="ma-row"><div class="ma-v">'+_maFmt(pVal,row.dec,(row.pu!=null?row.pu:row.u))+'</div>'+
         _maBar(pP,pE,'l')+'<div class="ma-lbl">'+row.lbl+'</div>'+_maBar(bP,bE,'r')+
         '<div class="ma-v r">'+_maFmt(bVal,row.dec,row.u)+'</div></div>';
    });
    h+='</div>';
    h+='<div class="tp-body-meta" style="border:0;margin-top:8px;font-size:10px">Bar = league percentile \u00b7 green favours the batter. AVG/ISO use the vs-hand split; Statcast is season-overall (hand splits arrive in Phase 2).</div>';
    h+='</div></details>'; return h;
  }
  window._maSetBatter=function(nm){ state.view.maBatter=nm; (typeof _renderAppPreserving==='function'?_renderAppPreserving:_renderApp)(); };
  ...
  function _mlbMatchPanels(g){
    return _mlbMatchPanel(g);   // starter matchups now render inside each team's panel (teamPanel)
  }
  ...
  function _eplMatchPanel(g){
    ...
    // Cards (referee-driven — team discipline x the assigned/picked ref's strictness)
    const _ref=_eplRefFor(g), _rs=((state.data.referees||{}).refStats)||{}, _rr=_ref&&_rs[_ref];
    let _rm=1.0, _rnote='league-average referee';
    if(_rr&&_rr.cardsPG){
      const _arr=Object.keys(_rs).map(function(k){return +_rs[k].cardsPG;}).filter(function(x){return x>0;});
      const _lg=_arr.length?_arr.reduce(function(a,b){return a+b;},0)/_arr.length:_rr.cardsPG;
      if(_lg>0){ _rm=_rr.cardsPG/_lg; _rnote=esc(_ref)+' \u00b7 '+(+_rr.cardsPG).toFixed(1)+' cards/game'; }
    }
    const _mt=(window.JTTScoring&&JTTScoring.projMatchTotals)?JTTScoring.projMatchTotals(g,_rm):null;
    const _projCd=_mt?_mt.cards.total:(((hT.cards||0)+(aT.cards||0))*_rm);
    let cards='<div class="section-title" style="margin:2px 0 6px">Cards</div>'+
      '<div class="statgrid">'+
      '<div class="statcell"><div class="v">'+_projCd.toFixed(1)+'</div><div class="k">proj. total cards</div></div>'+
      '<div class="statcell"><div class="v">'+_rm.toFixed(2)+'\u00d7</div><div class="k">referee factor</div></div>'+
      '<div class="statcell"><div class="v" style="font-size:12px">'+(hT.cards!=null?(+hT.cards).toFixed(1):'\u2014')+' / '+(aT.cards!=null?(+aT.cards).toFixed(1):'\u2014')+'</div><div class="k">'+esc(abbr(g.home))+' / '+esc(abbr(g.away))+' cards/g</div></div></div>'+
      '<div class="tp-body-meta" style="border:0;margin-top:6px">'+_rnote+(_ref?'':' \u2014 pick a referee in the panel below to sharpen this')+'</div>';
    return _eplWrap('Match',' ti-chart-arcs',res,'c-green')+_eplWrap('Goals','ti-ball-football',goals)+_eplWrap('Cards','ti-cards',cards);
  }

  function _eplCardsPanel(g){
    const hT=(state.idx.teamMap||{})[g.home]||{}, aT=(state.idx.teamMap||{})[g.away]||{};
    if(hT.cards==null&&aT.cards==null&&hT.cornersFor==null) return '';
    const projCorn=(hT.cornersFor!=null&&aT.cornersFor!=null)?((hT.cornersFor+aT.cornersAgainst)/2+(aT.cornersFor+hT.cornersAgainst)/2):null;
    const _ref=_eplRefFor(g), _rs=((state.data.referees||{}).refStats)||{}, _rr=_ref&&_rs[_ref];
    let _rm=1.0, _rnote='league-average referee';
    if(_rr&&_rr.cardsPG){ const _arr=Object.keys(_rs).map(function(k){return +_rs[k].cardsPG;}).filter(function(x){return x>0;}); const _lg=_arr.length?_arr.reduce(function(a,b){return a+b;},0)/_arr.length:_rr.cardsPG; if(_lg>0){ _rm=_rr.cardsPG/_lg; _rnote=esc(_ref)+' \u00b7 '+(+_rr.cardsPG).toFixed(1)+' cards/game'; } }
    const _mt=(window.JTTScoring&&JTTScoring.projMatchTotals)?JTTScoring.projMatchTotals(g,_rm):null;
    const projCd=_mt?_mt.cards.total:(((hT.cards||0)+(aT.cards||0))*_rm);
    let body='<div class="statgrid">';
    if(projCorn!=null) body+='<div class="statcell"><div class="v">'+projCorn.toFixed(1)+'</div><div class="k">proj. total corners</div></div>';
    body+='<div class="statcell"><div class="v">'+projCd.toFixed(1)+'</div><div class="k">proj. total cards</div></div>'+
      '<div class="statcell"><div class="v">'+_rm.toFixed(2)+'\u00d7</div><div class="k">referee factor</div></div></div>'+
      '<div class="tp-body-meta" style="border:0;margin-top:6px">'+_rnote+(_ref?'':' \u2014 pick a referee in the panel below to sharpen the cards line')+'</div>';
    return _eplWrap('Corners & Cards','ti-cards',body);
  }
  function _eplFocusPanels(g){ return _eplLineupDropPanel(g)+_eplRefereePanel(g)+_eplLineupPanel(g)+_eplVenuePanel(g); }
  ...
  function renderFocusedFixture(g){
    ...
    if(SPORT.key==='epl'){ h+=_eplMarketPanels(g)+_eplFocusPanels(g); }
    ...
    h+=((SPORT.key==='epl'||_fxHid('sec','matchMkts'))?'':gameMatchMarkets(g.home,g.away))+(_fxHid('sec','matchOdds')?'':gameMatchOdds(g.home,g.away))+((SPORT.key==='epl'||_fxHid('sec','h2h'))?'':gameHeadToHead(g.home,g.away))+((SPORT.key==='epl'||_fxHid('sec','clash'))?'':gameClash(g.home,g.away))+(_fxHid('sec','lastMeeting')?'':gameLastMeeting(g.home,g.away))+((SPORT.key==='epl'||_fxHid('sec','byHalves'))?'':gameByHalves(g.home,g.away))+((SPORT.key==='epl'||_fxHid('sec','shapeTrend'))?'':gameShapeTrend(g.home,g.away))+((SPORT.key==='epl'||_fxHid('sec','recentGames'))?'':gameRecentGames(g.home,g.away));
    h+='<div class="team-cols">'+teamPanel(g.home,g.away,true)+teamPanel(g.away,g.home,false)+'</div>';
    if(SPORT.key==='mlb' && !_fxHid('sec','matchupAnalyzer')) h+=_mlbMatchupAnalyzer(g);
    ...
    if(SPORT.key!=='epl' && !_fxHid('sec','insouts')) h+=insOutsSection(g.home,g.away);
    ...
    if(fxMode!=='nerd' && !_fxHid('sec','multiTool')) h+=_fxMultiCards(g);
    return h;
  }

  // ---- per-team column: stat leaders + matchup + scoped Degen signals ----
  // ===================== Positional Distribution (For / Against) - AFL/AFLW disposals, NFL targets =====================
  var AFL_POS=[
    {k:'Key Defender',s:'KD',c:'#1d4ed8'},{k:'Gen. Defender',s:'GD',c:'#38bdf8'},{k:'Midfielder',s:'MID',c:'#22c55e'},
    {k:'Mid-Forward',s:'MF',c:'#eab308'},{k:'Gen. Forward',s:'GF',c:'#f97316'},{k:'Key Forward',s:'KF',c:'#ef4444'},{k:'Ruck',s:'RUC',c:'#a855f7'}
  ];
  var NFL_POS=[ {k:'WR',s:'WR',c:'#dc2626'},{k:'RB',s:'RB',c:'#1f2937'},{k:'TE',s:'TE',c:'#9ca3af'} ];
  var POS_DIST_CFG={
    afl:{stat:'disposals',label:'Disposal',groups:AFL_POS,against:'dvp'},
    aflw:{stat:'disposals',label:'Disposal',groups:AFL_POS,against:'dvp'},
    nfl:{stat:'targets',label:'Target',groups:NFL_POS,against:'gamelog'}
  };
  var _GL_AGAINST={};
  function _gamelogAgainst(stat, groups){
    if(_GL_AGAINST[stat]) return _GL_AGAINST[stat];
    var gk={}; groups.forEach(function(p){gk[p.k]=1;});
    var posMap={}; (state.idx.players||[]).forEach(function(p){ if(p.name) posMap[p.name]=p.position; });
    var yr=0; (state.data.gamelogs||[]).forEach(function(r){ if(+r.Year>yr) yr=+r.Year; });   // most recent season only
    var ag={};
    (state.data.gamelogs||[]).forEach(function(r){
      if(yr && +r.Year!==yr) return;
      var pos=posMap[r.Player]; if(!pos||!gk[pos]) return;
      var opp=r.Opp; if(!opp) return;
      (ag[opp]=ag[opp]||{})[pos]=(ag[opp][pos]||0)+(+r[stat]||0);
    });
    return _GL_AGAINST[stat]=ag;
  }
  function _posDist(team){
    var cfg=POS_DIST_CFG[SPORT.key]; if(!cfg) return null;
    var G=cfg.groups, stat=cfg.stat, fd={}, ad={};
    G.forEach(function(p){fd[p.k]=0;ad[p.k]=0;});
    (state.idx.players||[]).forEach(function(p){ if(p.team===team && fd[p.position]!=null) fd[p.position]+=(+p[stat]||0)*(+p.matches||0); }); // games-weighted season share
    if(cfg.against==='dvp'){ (state.data.dvp||[]).forEach(function(r){ if(r.team===team && ad[r.pos]!=null) ad[r.pos]+=(+r[stat]||0); }); }
    else if(cfg.against==='gamelog'){ var t=(_gamelogAgainst(stat,G)[team])||{}; G.forEach(function(p){ ad[p.k]=+t[p.k]||0; }); }
    var ft=0,at=0; G.forEach(function(p){ft+=fd[p.k];at+=ad[p.k];});
    var fp={},ap={}; G.forEach(function(p){ fp[p.k]=ft?fd[p.k]/ft*100:0; ap[p.k]=at?ad[p.k]/at*100:0; });
    return {fp:fp,ap:ap,ft:ft,at:at,cfg:cfg};
  }
  function _pieGrad(dist,G){ var seg=[],acc=0; G.forEach(function(p){ var v=dist[p.k]||0; if(v<=0) return; seg.push(p.c+' '+acc.toFixed(2)+'% '+(acc+v).toFixed(2)+'%'); acc+=v; }); return seg.length?'conic-gradient('+seg.join(',')+')':'var(--surface-2)'; }
  function _posDistribution(team){
    var d=_posDist(team); if(!d || (!d.ft && !d.at)) return '';
    var G=d.cfg.groups;
    var h='<div class="tp-sec"><div class="sec-title"><i class="ti ti-chart-pie"></i> '+d.cfg.label+' Distribution</div></div>';
    h+='<div class="afl-dist"><div class="afl-pies">'+
       '<div class="afl-pie-wrap"><div class="afl-pie" style="background:'+_pieGrad(d.fp,G)+'"></div><div class="afl-pie-cap">For</div></div>'+
       '<div class="afl-pie-wrap"><div class="afl-pie" style="background:'+_pieGrad(d.ap,G)+'"></div><div class="afl-pie-cap">Against</div></div>'+
       '</div><div class="afl-leg">';
    G.forEach(function(p){ h+='<div class="afl-leg-r"><span class="afl-dot" style="background:'+p.c+'"></span><span class="afl-leg-nm">'+p.s+'</span><span class="afl-leg-v">'+Math.round(d.fp[p.k])+' / '+Math.round(d.ap[p.k])+'</span></div>'; });
    h+='</div><div class="afl-leg-note">For % / Against %</div></div>';
    return h;
  }
  // ===================== NFL Coverage Matchup (man/zone tendency + receiver splits) =====================
  function _covName(c){ return String(c||'').replace('COVER_','Cover-').replace('2_MAN','2-Man').replace('_',' '); }
  function _nflCoverage(team, opp){
    if(SPORT.key!=='nfl') return '';
    var _cc=(window.SPORT_CONFIG&&SPORT_CONFIG.coverage)||{};
    var _covT={manHeavy:_cc.manHeavy!=null?_cc.manHeavy:45, zoneHeavy:_cc.zoneHeavy!=null?_cc.zoneHeavy:60, lean:_cc.lean!=null?_cc.lean:1.5};
    var od=((state.idx.teamMap||{})[opp]||{}).cov;
    var recs=playersOnTeam(team).filter(function(p){ return p.cov && (p.cov.man||p.cov.zone) && isPlaying(p.name,team); });
    if((!od||!od.plays) && !recs.length) return '';
    var h='<div class="tp-sec"><div class="sec-title"><i class="ti ti-shield-half"></i> Coverage vs '+abbr(opp)+'</div></div>';
    var manHeavy=false, zoneHeavy=false;
    if(od && od.plays){
      manHeavy=od.manRate>=_covT.manHeavy; zoneHeavy=od.zoneRate>=_covT.zoneHeavy;
      var top=Object.keys(od.cover||{});
      h+='<div class="cov-dhdr"><span class="cov-bar"><span class="cov-man" style="width:'+od.manRate+'%"></span></span>'+
         '<span class="cov-rates"><b>'+Math.round(od.manRate)+'%</b> man / <b>'+Math.round(od.zoneRate)+'%</b> zone'+(top.length?' \u00b7 '+_covName(top[0])+' '+Math.round(od.cover[top[0]])+'%':'')+'</span></div>';
    }
    var rows=recs.map(function(p){
      var m=p.cov.man||{tgt:0,yds:0}, z=p.cov.zone||{tgt:0,yds:0};
      return {p:p, mypt:m.tgt?m.yds/m.tgt:0, zypt:z.tgt?z.yds/z.tgt:0, n:(m.tgt||0)+(z.tgt||0)};
    }).sort(function(a,b){return b.n-a.n;}).slice(0,5);
    rows.forEach(function(r){
      var lean=r.mypt-r.zypt;
      var flag=(lean>=_covT.lean && manHeavy)?'Coverage Buster':((lean<=-_covT.lean && zoneHeavy)?'Zone Beater':'');
      h+='<div class="cov-rec">'+
         '<span class="cov-nm" data-p="'+esc(r.p.name)+'" onclick="openPlayer(this.dataset.p)">'+esc(r.p.name)+'</span>'+
         '<span class="cov-split"><span'+(lean>=1.5?' class="hot"':'')+'>man '+r.mypt.toFixed(1)+'</span> \u00b7 <span'+(lean<=-1.5?' class="hot"':'')+'>zone '+r.zypt.toFixed(1)+'</span> y/tgt</span>'+
         (flag?'<span class="cov-flag">'+flag+'</span>':'')+'</div>';
    });
    return h;
  }
  ...
  function teamPanel(team, opp, isHome){
    ...
    const _pgLead=!!(window.SPORT_CONFIG&&SPORT_CONFIG.leadersPerGame);
    const _lval=(p,k)=>_pgLead?((p[k]||0)/(p.matches||1)):(p[k]||0);
    const leaders=BET_STATS.slice(0,4).filter(([k])=>!_fxHid('mkt',k)).map(([k,l])=>{
      const top=playersOnTeam(team).filter(p=>(p.matches||0)>=3 && isPlaying(p.name,team) && (SPORT.key!=='mlb'||p.role!=='pitch')).sort((a,b)=>_lval(b,k)-_lval(a,k))[0];
      if(!top) return '';
      return '<div class="tp-lead" onclick="openPlayer(\''+esc(top.name).replace(/\x27/g,"\\\x27")+'\')"><span class="nm">'+l+' · '+top.name+'</span><span class="vv">'+fmt(_lval(top,k),_pgLead?2:(k==='goals'?2:1))+(_pgLead?'/g':'')+'</span></div>';
    }).join('');
    ...
    if(SPORT.key==='mlb' && !_fxHid('sec','starter')){
      const _sg=(state.data.fixture||[]).find(x=>((x.home===team&&x.away===opp)||(x.home===opp&&x.away===team)));
      const _fp=_sg?(_sg.home===team?_sg.awayPitcher:_sg.homePitcher):null;
      if(_sg && _fp) h+=_mlbStarterPanel(_sg, team, _fp);
    }
    ...
    if(SPORT.key==='epl') h+=_eplTrendBlock(team);
    if(POS_DIST_CFG[SPORT.key] && !_fxHid('sec','posdist')) h+=_posDistribution(team);
    if(SPORT.key==='nfl' && !_fxHid('sec','coverage')) h+=_nflCoverage(team,opp);
  ...
  function statsTable(){
    ...
    const COLS=v.posFilter?DISPLAY_FOR(v.posFilter):DISPLAY;   // position-relevant columns when filtered
    ...
    COLS.forEach(([k,l])=>h+=th(k,l)); h+='</tr></thead><tbody>';
    ps.forEach(p=>{
      h+='<tr onclick="openPlayer(\''+esc(p.name).replace(/'/g,"\\'")+'\')"><td class="name">'+p.name+'</td>'+
        ...
        '<td>'+(p.matches||0)+'</td>';
      COLS.forEach(([k])=>h+='<td>'+fmt(p[k],k==='goals'||k==='behinds'?2:1)+'</td>');
      h+='</tr>';
    });
    h+='</tbody></table></div>';
  ...
  function teamsView(){
    ...
    let h='<div class="filterbar"><div class="pos-pills">'+
      '<button class="dfilt-chip'+(!A?' active':'')+'" onclick="setTeamsMode(false)">For</button>'+
      '<button class="dfilt-chip'+(A?' active':'')+'" onclick="setTeamsMode(true)">Allowed</button>'+
      '</div></div><div class="tt-hint" style="padding:2px 4px 8px">'+(A?'avg conceded to opponents — higher = softer defence':'team\'s own per-game output')+'</div>';
    ...
    teams.forEach(t=>{
      ...
      TEAM_COLS.forEach(([k])=>{
        ...
        const _td=(window.SPORT_CONFIG&&SPORT_CONFIG.teamDecimals); const _dec=(_td!=null)?_td:(k==='goals'?1:0);
        h+='<td'+(col?' style="color:'+col+';font-weight:700"':'')+'>'+fmt(val, _dec)+'</td>';
      });
      h+='</tr>';
    });
    h+='</tbody></table></div>';
  ...
  /* ---------- DVP (Scanner-style: pick a position, see its stats per team) ---------- */
  function dvpView(){
    const v=state.view, all=state.data.dvp||[]; if(!all.length) return emptyState('ti-shield-off','No DVP data','Run the data build to derive DVP.');
    const pos=(v.dvpPos&&(POS_ORDER||[]).indexOf(v.dvpPos)>=0)?v.dvpPos:(POS_ORDER||['All'])[0];
    // use the same short abbreviations the Scanner + Teams show, so columns stay even
    const _lbl={}; [].concat(DVP_STATS||[], TEAM_COLS||[], DISPLAY||[], DISPLAY_FOR(pos)||[]).forEach(function(x){_lbl[x[0]]=x[1];});
    const COLS=_posMkts(pos,DVP_STATS).filter((x,i,a)=>a.findIndex(y=>y[0]===x[0])===i).map(function(x){return [x[0], _lbl[x[0]]||x[1]];});   // position-relevant columns, Scanner/Teams labels
    const rows=all.filter(r=>r.pos===pos);
    const avg={}; COLS.forEach(([k])=>{const vals=rows.map(r=>r[k]).filter(x=>x!=null);avg[k]=vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:0;});
    const sk=v.dvpSortKey||'__team', sd=v.dvpSortDir||(sk==='__team'?1:-1);
    let tr=rows.slice();
    if(sk==='__team') tr.sort((a,b)=>a.team.localeCompare(b.team)*sd);
    else tr.sort((a,b)=>{const x=a[sk],y=b[sk];if(x==null)return 1;if(y==null)return -1;return (x-y)*sd;});
    const arr=k=> sk===k?(sd<0?' \u25be':' \u25b4'):'';
    const _dw=(v.dvpWindow||SETTINGS.dvpWindow||'season');
    const _winToggle='<div class="dvp-win">'+[['season','Season'],['l10','L10'],['l5','L5']].map(function(w){return '<button class="dvp-winbtn'+(_dw===w[0]?' on':'')+'" onclick="setDvpWindow(\''+w[0]+'\')" title="Rolling DVP over the last '+(w[0]==='season'?'full season':w[0]==='l10'?'10':'5')+' games \u2014 applies to Check My Bet, the modal and signals too">'+w[1]+'</button>';}).join('')+'</div>';
    let h='<div class="filterbar"><div class="pos-pills">'+(POS_ORDER||[]).map(p=>'<button class="dfilt-chip'+(p===pos?' active':'')+'" onclick="setDvpPos(\''+p+'\')" title="'+p+'">'+posShort(p)+'</button>').join('')+'</div>'+_winToggle+'</div>';
    h+='<div class="tt-hint" style="padding:2px 4px 8px">avg conceded to <strong>'+pos+'</strong> per game \u00b7 tap a column to sort \u00b7 <span style="color:#22c55e">green = soft</span> / <span style="color:#ef4444">red = tough</span></div>';
    h+='<div class="stbl-wrap"><table class="stbl"><thead><tr><th class="'+(sk==='__team'?'sorted':'')+'" onclick="sortDvpCol(\'__team\')">Team'+arr('__team')+'</th>';
    COLS.forEach(([k,l])=>h+='<th class="'+(sk===k?'sorted':'')+'" onclick="sortDvpCol(\''+k+'\')">'+l+arr(k)+'</th>');
    h+='</tr></thead><tbody>';
    tr.forEach(r=>{
      h+='<tr><td class="name">'+teamLogo(r.team,22)+'<span style="margin-left:7px">'+r.team+'</span></td>';
      COLS.forEach(([k])=>{
        const val=r[k];
        if(val==null){h+='<td class="dvp-na">\u2014</td>';return;}
        const a=avg[k], d=a?((val-a)/a*100):0;
        const col=Math.abs(d)<4?'':(d>0?'#22c55e':'#ef4444');   // match Teams: bold green/red text, no cell fill
        h+='<td'+(sk===k?' class="dvp-sortcol"':'')+(col?' style="color:'+col+';font-weight:700"':'')+'>'+fmt(val, val<3?2:val<20?1:0)+'</td>';
      });
      h+='</tr>';
    });
    h+='</tbody></table></div>';
  ...
  window.setDvpPos=function(p){state.view.dvpPos=p;state.view.dvpSortKey='__team';state.view.dvpSortDir=1;const b=document.getElementById('stats-body');if(b)b.innerHTML=dvpView();};
  window.sortDvpCol=function(k){const v=state.view;if((v.dvpSortKey||'__team')===k){v.dvpSortDir=(v.dvpSortDir||(k==='__team'?1:-1))*-1;}else{v.dvpSortKey=k;v.dvpSortDir=k==='__team'?1:-1;}const b=document.getElementById('stats-body');if(b)b.innerHTML=dvpView();};
  ...
  function renderDegen(){
    if(state.view.crewTile){ const t=_crewTileHTML(state.view.crewTile); if(t) return t; }
    ...
    h+='<div class="crew-grid">'+CREW.filter(c=>((window.SPORT_CONFIG||{}).hideCrew||[]).indexOf(c.k)<0).map(c=>'<div class="crew-tile" onclick="degenTile(\''+c.k+'\')"><div class="crew-tile-icon"><i class="ti '+c.i+'"></i></div><div class="crew-tile-name">'+c.n+'</div><div class="crew-tile-sub">'+c.d+'</div></div>').join('')+'</div>';
    return h;
  }
  ...
  function hasAnyOdds(){ const m=state.idx.oddsMeta; return !!(m && ((m.total!=null?m.total:m.count)||0)>0); }
  ...
  function _eliteDefs(){   // per-sport Elite Matchups stats: curated eliteDefs if the config sets one, else the DVP options
    var src=(typeof ELITE_OPTS!=='undefined'&&ELITE_OPTS&&ELITE_OPTS.length)?ELITE_OPTS
      :(DVPS_OPTS||[]).filter(function(o){return o[0]!=='dreamteam';}).slice(0,6);
    return src.map(function(o){return {k:o[0],l:o[1]};});
  }
  ...
  // How recently each tagger has actually been given a run-with job (2026 tagging log, to R22).
  // {l:last round tagged, n:tag jobs in 2026}. Names absent = no logged tag this season.
  const TAGGER_LOG_ASOF=22;
  const TAGGER_LOG={'James Jordon':{l:22,n:14},'Edward Allan':{l:22,n:9},'Brady Hough':{l:22,n:9},'Koltyn Tholstrup':{l:22,n:7},'Hugh Boxshall':{l:22,n:4},'Oisin Mullin':{l:21,n:14},'Touk Miller':{l:21,n:1},'Milan Murdock':{l:20,n:6},"Finn O'Sullivan":{l:18,n:11},'James Peatling':{l:18,n:3},'Nick Holman':{l:16,n:1},'Jy Simpkin':{l:14,n:3},'Marcus Windhager':{l:13,n:1},'Matthew Johnson':{l:12,n:2},'Toby Bedford':{l:12,n:1},'Jacob Konstanty':{l:11,n:2},'Josh Dunkley':{l:11,n:1},'Finn Maginness':{l:10,n:1},'Jarrod Berry':{l:3,n:2}};
  ...
  function _nmuGroup(pos){ var p=(pos||'').toLowerCase();
    if(p.indexOf('ruck')>=0) return 'ruck';
    if(p.indexOf('mid')>=0) return 'mid';        // Midfielder + Mid-Forward
    if(p.indexOf('def')>=0) return 'def';        // Key/Gen Defender
    if(p.indexOf('forward')>=0||p.indexOf('fwd')>=0) return 'fwd';
    return 'any';
  }
  function nextManUp(){
    ...
    players.forEach(Y=>{
      ...
      (byTeam[Y.team]||[]).forEach(X=>{
        ...
        if(_nmuGroup(Y.position)!=='any' && _nmuGroup(X.position)!==_nmuGroup(Y.position)) return;  // similar position only (a mid replaces a mid)
  ...
  function dejaVuSGM(home,away){
    ...
    const _nMkt=DV_MARKETS.filter(m=>_dvMktOn(m[0])).length;
    const mktChips='<details class="ob-dd dv-mktdd"'+(S.mktOpen?' open':'')+' ontoggle="_dvState().mktOpen=this.open"><summary class="ob-sel ob-dd-sum">'+(_nMkt<DV_MARKETS.length?_nMkt+' market'+(_nMkt>1?'s':''):'All markets')+'<i class="ti ti-chevron-down"></i></summary><div class="ob-dd-body">'+DV_MARKETS.map(([k,l])=>'<button class="ob-chip'+(_dvMktOn(k)?' active':'')+'" onclick="_dvMkt(\''+k+'\')">'+l+'</button>').join('')+'</div></details>';
    ...
    const meetings=_dvMeetings(home,away,meets);
    if(meetings.length<meets) return title+'<div class="dv-steps">'+meetsStep+sameSPToggle+mktChips+'</div>'+dvHelp+'<div class="tp-body-meta" style="border:0">Need at least '+meets+' prior meetings between '+abbr(home)+' and '+abbr(away)+' — lower the hit streak to widen the pool.</div>';
    // no live per-book odds yet -> preview the qualifying players (one row each)
    if(!(state.idx.bookIdx&&Object.keys(state.idx.bookIdx).length)){
      const {legs}=_dvLegs(home,away);
      const ctrls='<div class="dv-steps">'+meetsStep+sameSPToggle+mktChips+'</div>'+dvHelp;
      ...
      const rows=top.map(l=>'<div class="mbv-card dv-card">'+'<div class="mbv-row1">'+teamLogo(l.team,30)+'<div class="mbv-pn"><div class="mbv-nm">'+esc(l.name)+'<span class="mbv-tm">'+abbr(l.team)+' \u00b7 '+posShort(l.pos||'')+'</span></div>'+'<div class="mbv-mk"><b>'+l.line+'+</b> '+esc(l.marketLabel)+'</div></div></div>'+'<div class="mbv-badges dv-cardbadges"><span class="mbv-bdg" style="background:#EEEDFE;color:#26215C"><i class="ti ti-versions"></i>D\u00e9j\u00e0 Vu x'+meets+'</span><span class="dv-vals">'+l.vals.join(' / ')+'</span></div>'+'</div>').join('');
    ...
    const slip=sel.map((pi,slot)=>{ let l=pool[pi]; if(!l) return '';
      ...
      return '<div class="mbv-card dv-card'+(reduced?' dv-reduced':'')+'">'+'<div class="mbv-row1">'+teamLogo(l.team,30)+'<div class="mbv-pn"><div class="mbv-nm">'+esc(l.name)+'<span class="mbv-tm">'+abbr(l.team)+' \u00b7 '+posShort(l.pos||'')+'</span></div>'+'<div class="mbv-mk">'+lineCtl+'<b>'+l.betLine+'+</b> '+mktCell+'</div></div>'+'<div class="mbv-op"><span class="mbv-op-price">$'+l.over.toFixed(2)+'</span><span class="mbv-op-book">'+bookName(book)+'</span></div>'+'<div class="dv-cardctrl">'+ctrl+'</div></div>'+'<div class="mbv-badges dv-cardbadges"><span class="mbv-bdg" style="background:#EEEDFE;color:#26215C"><i class="ti ti-versions"></i>D\u00e9j\u00e0 Vu x'+meets+'</span><span class="dv-vals">'+(l.vals||[]).join(' / ')+'</span></div>'+'</div>';
    }).join('');
    ...
    const _tier=Math.min(Math.max(S.tier||3,2),Math.max(2,distinctN));
    const tierBtns='<span class="dv-step"><span class="dv-step-lbl">Legs</span><button onclick="_dvTierStep(-1)"'+((_tier<=2||S.hail)?' disabled':'')+'>\u2212</button><b>'+(S.hail?distinctN:_tier)+'</b><button onclick="_dvTierStep(1)"'+((_tier>=distinctN||S.hail)?' disabled':'')+'>+</button></span>';
    const hailBtn='<button class="dv-tier dv-hail'+(S.hail?' on':'')+'" onclick="_dvHail()"><i class="ti ti-bolt"></i> Hail Mary</button>';
    const bookChips=books.length>1?('<select class="ob-sel" onchange="_dvBook(this.value)">'+books.slice().sort((a,b)=>_dvDistinct(byBook[b])-_dvDistinct(byBook[a])).map(b=>'<option value="'+b+'"'+(b===book?' selected':'')+'>'+bookName(b)+' ('+_dvDistinct(byBook[b])+')</option>').join('')+'</select>'):'';
    ...
    const steppers='<div class="dv-steps">'+meetsStep+minStep+maxStep+mktChips+bookChips+'</div>'+dvHelp;
    ...
    return title+
      ...
      '<div class="dv-ctrls">'+tierBtns+hailBtn+'</div>'+steppers+
  ...
  window._dvTierStep=function(d){ const S=_dvState(); S.hail=false; S.tier=Math.max(2,(S.tier||3)+d); S.sel=null; S.removed=[]; _renderApp(); };
  ...
  function _genericCandidates(p,opp){
    const mkts=(SETTINGS.multi.markets&&SETTINGS.multi.markets.length)?SETTINGS.multi.markets:MULTI_MKTS.map(m=>m[0]);
    const NM=Object.fromEntries(MULTI_MKTS); const c=[];
    mkts.forEach(function(sk){
      var avg=+p[sk]; if(!isFinite(avg)||avg<=0) return;
      var line=(typeof _snapToRung==='function')?_snapToRung(p.name,sk,Math.max(0.5,Math.round(avg*0.8))):Math.max(0.5,Math.round(avg*0.8));
      if(line==null||line<0.5||avg<=line) return;
      var hr=JTTScoring.getHitRate(p.name,sk,line,true);
      var prob=hr?hr.rate:estimateHitProb(avg,line);
      var sigs=opp?JTTScoring.getContextSignals(p,sk,opp):[];
      var score=sigs.reduce(function(s,sig){return s+(sig.pct!=null?sig.pct*(sig.weight||1):0);},0);
      var reasons=sigs.filter(function(sg){return sg.pct>0;}).sort(function(a,b){return (b.pct*b.weight)-(a.pct*a.weight);}).slice(0,2).map(function(sg){return {label:sg.label,pct:sg.pct};});
      var _odds=(typeof priceForLine==='function')?priceForLine(p.name,sk,line):null;
      c.push({id:p.name+'-'+sk,p:p,statKey:sk,line:line,avg:avg,prob:prob,probReal:!!hr,probN:hr?hr.n:0,score:score,opp:opp,type:'',betLabel:line+'+ '+(NM[sk]||sk),reasons:reasons,_odds:_odds});
    });
    return c;
  }
  ...
  function legCandidates(p,opp){
    ...
    if(SPORT.key!=='afl'&&SPORT.key!=='aflw') return _genericCandidates(p,opp);
  ...
  function _radarTwoWayLines(player, market){
    // Some books flood a padded alt ladder into the two-way feed (BetR historically, but others do it too).
    // Betr stays fully excluded; for any other book that posts a padded ladder (many two-way lines for one
    // player+market), keep ONLY its main line (closest to the season average) so alt rungs don't reach the Radar.
    const _isBetr=(b)=>{ const x=String(b||'').toLowerCase().replace(/[^a-z]/g,''); return x==='betr'||x==='betrau'; };  // feed key is 'betr_au'; match that + 'Betr' but NOT 'betright' (BetRight is a different book)
    const LADDER_MAX=1;   // a book posts ONE genuine each-way line per player+market; more = a padded alt ladder -> keep only its main line
    const rows=(((state.idx.bookIdx||{})[player]||{})[market]||[]).filter(r=>_bookOK(r.book)&&!_isBetr(r.book)&&r.over!=null&&r.under!=null&&r.over>=1.5&&r.under>=1.5);  // both sides >=1.5 => genuine each-way line, not a padded alt rung
    const avg=+(((state.idx.byName||{})[player]||{})[market]);
    const byBook={}; rows.forEach(r=>{ (byBook[r.book]=byBook[r.book]||[]).push(r); });
    let kept=[];
    Object.keys(byBook).forEach(bk=>{ const brs=byBook[bk];
      if(brs.length>LADDER_MAX){
        const ref=isFinite(avg)?avg:brs.map(r=>r.line).sort((a,b)=>a-b)[Math.floor(brs.length/2)];  // avg, else median rung
        brs.sort((a,b)=>Math.abs(a.line-ref)-Math.abs(b.line-ref));
        kept.push(brs[0]);   // just the book's main line
      } else kept=kept.concat(brs);
    });
    const byL={}; kept.forEach(r=>{ if(!byL[r.line]||r.over>byL[r.line].over) byL[r.line]={line:r.line,over:r.over,under:r.under,book:r.book}; });
  ...
  function radarSection(home, away, venue){
    if((window.SPORT_CONFIG||{}).noPlayerOdds) return '';   // no player-prop odds -> Radar can't rate lines, hide it
  ...
  // ===== Multi Builder v2 — badge-driven leg cards (odds optional; works on stats too) =====
  const MB_BADGES={
    banker:  ['Banker','#EAF3DE','#27500A','ti-shield-check'],
    value:   ['Value','#E6F1FB','#0C447C','ti-trending-up'],
    longshot:['Longshot','#EEEDFE','#26215C','ti-rocket'],
    form:    ['Form','#FAEEDA','#633806','ti-flame'],
    bunny:   ['Bunny','#FBEAF0','#72243E','ti-carrot'],
    venue:   ['Venue','#E1F5EE','#04342C','ti-map-pin'],
    streak:  ['Streak','#FAECE7','#712B13','ti-arrow-big-up-lines'],
    dvp:     ['Soft DVP','#E6F1FB','#0C447C','ti-shield-half'],
    untagged:['Untagged','#EAF3DE','#27500A','ti-user-check'],
    ceiling: ['Ceiling','#FCEBEB','#791F1F','ti-arrow-up-right']
  };
  const MB_SPORT_BADGES={
    targethog:['Target Hog','#E6F1FB','#0C447C','ti-crosshair'],
    snaplock: ['Snap Lock','#EAF3DE','#27500A','ti-lock'],
    redzone:  ['Red Zone','#FCEBEB','#791F1F','ti-target-arrow'],
    usage:    ['Usage','#EEEDFE','#26215C','ti-activity'],
    volume:   ['Shot Volume','#FAEEDA','#633806','ti-arrows-up'],
    pp1:      ['PP1 Unit','#FBEAF0','#72243E','ti-bolt'],
    power:    ['Power','#FCEBEB','#791F1F','ti-flame'],
    ruckedge: ['Ruck Edge','#FAEEDA','#633806','ti-arrows-up']
  };
  const _MB_ALLBDG=Object.assign({},MB_BADGES,MB_SPORT_BADGES);
  function _sportBadges(leg){
    const out=[], p=leg.p, sk=leg.statKey, k=SPORT.key||'';
    if(k==='nfl'||k==='ncaaf'){
      if((+p.targets||0)>=7 && (sk==='receptions'||sk==='recYds'||sk==='rushRecYds')) out.push('targethog');
      if((+p.snapPct||0)>=85) out.push('snaplock');
      if((+p.rzTgt||+p.rzTgts||0)>=1.5 && (sk==='anytimeTd'||sk==='totalTds'||sk==='receptions')) out.push('redzone');
    } else if(k==='nba'||k==='wnba'){
      if((+p.usage||0)>=28 || (+p.minutes||+p.min||0)>=32) out.push('usage');
    } else if(k==='nhl'){
      if(sk==='shots' && (+p.shots||0)>=3) out.push('volume');
      if((+p.ppPoints||0)>=0.5) out.push('pp1');
    } else if(k==='mlb'){
      if(sk==='HR'||sk==='TB') out.push('power');
    } else if(k==='afl'||k==='aflw'){
      if((sk==='dreamteam'||sk==='disposals') && /Ruck/.test(p.position||'')) out.push('ruckedge');
    }
    return out;
  }
  function _mbLogsFor(nm){ return (state.idx.logsByName&&state.idx.logsByName[nm])||[]; }
  // ===== Projection model =====
  // Exponential recency weighting -> empirical-Bayes shrinkage to the season baseline -> opponent (DVP)
  // adjustment -> dispersion estimate -> distribution-based P(over line) blended with the empirical hit
  // rate. Returns {mean, sd, prob, n}. Poisson tail for low-count markets, Normal (with continuity) otherwise.
  ...
  function _poissonCDF(k, lam){ if(lam<=0) return k>=0?1:0; if(k<0) return 0; var t=Math.exp(-lam), sm=t; for(var i=1;i<=k;i++){ t*=lam/i; sm+=t; } return Math.min(1,sm); }
  // Per-sport projection config — shared engine below reads these. Params are starting points to calibrate per sport.
  const PROJ_CONFIG={
    afl:  {lambda:0.85, biasCorr:1.011, shrinkK:5, dvpBeta:0.55, dvpCap:0.12, poissonBelow:6, roleStat:'cba',     roleMkts:['disposals','clearances'], roleBeta:0.30, roleLo:-0.06, roleHi:0.08},
    aflw: {lambda:0.85, biasCorr:1.011, shrinkK:5, dvpBeta:0.55, dvpCap:0.12, poissonBelow:6, roleStat:'cba',     roleMkts:['disposals','clearances'], roleBeta:0.30, roleLo:-0.06, roleHi:0.08},
    nfl:  {lambda:0.83, shrinkK:4, dvpBeta:0.55, dvpCap:0.15, poissonBelow:8, roleStat:'snapPct', roleMkts:['receptions','recYds','rushRecYds'], roleBeta:0.25, roleLo:-0.06, roleHi:0.08},
    ncaaf:{lambda:0.83, shrinkK:4, dvpBeta:0.55, dvpCap:0.15, poissonBelow:8, roleStat:'snapPct', roleMkts:['receptions','recYds','rushRecYds'], roleBeta:0.25, roleLo:-0.06, roleHi:0.08},
    nba:  {lambda:0.90, shrinkK:12, dvpBeta:0.50, dvpCap:0.10, poissonBelow:4, roleStat:'minutes', roleMkts:['points','rebounds','assists'], roleBeta:0.25, roleLo:-0.06, roleHi:0.08},
    wnba: {lambda:0.90, shrinkK:12, dvpBeta:0.50, dvpCap:0.10, poissonBelow:4, roleStat:'minutes', roleMkts:['points','rebounds','assists'], roleBeta:0.25, roleLo:-0.06, roleHi:0.08},
    nhl:  {lambda:0.90, shrinkK:12, dvpBeta:0.45, dvpCap:0.10, poissonBelow:3, roleStat:'toi',     roleMkts:['shots','points'], roleBeta:0.25, roleLo:-0.06, roleHi:0.08},
    mlb:  {lambda:0.94, biasCorr:1.02, shrinkK:3, dvpBeta:0.45, dvpCap:0.12, poissonBelow:5, roleStat:'',        roleMkts:[], roleBeta:0, roleLo:0, roleHi:0}
  };
  const _PROJ_DEF={lambda:0.88, shrinkK:5, dvpBeta:0.55, dvpCap:0.12, poissonBelow:6, roleStat:'', roleMkts:[], roleBeta:0, roleLo:-0.06, roleHi:0.08};
  function _projCfg(){ return PROJ_CONFIG[SPORT.key||'']||_PROJ_DEF; }
  function _projModel(p, sk, line, opp, ctx){
    var C=_projCfg();
    var logs=_mbLogsFor(p.name), vals=logs.map(function(r){return +r[sk];}).filter(function(v){return isFinite(v);});
    var n=vals.length;
    var _mean=function(a){return a.length?a.reduce(function(x,y){return x+y;},0)/a.length:0;};
    var _clamp=function(v,a,b){return Math.max(a,Math.min(b,v));};
    var season=(+p[sk]) || _mean(vals);
    if(n<3) return {mean:season, sd:Math.max(1,season*0.35), prob:null, n:n};
    var srt=vals.slice().sort(function(a,b){return a-b;});                       // robust winsorize: cap at median +/- 2.5 MAD-sigma
    var med=srt[Math.floor((srt.length-1)/2)];
    var ad=vals.map(function(v){return Math.abs(v-med);}).sort(function(a,b){return a-b;});
    var mad=ad[Math.floor((ad.length-1)/2)]||0, cap=Math.max(mad*1.4826*2.5, med*0.30, 2);
    var lo=med-cap, hi=med+cap, w=vals.map(function(v){return v<lo?lo:(v>hi?hi:v);});
    var lam=C.lambda, wsum=0, vsum=0;                                           // exponential recency weight
    for(var i=0;i<n;i++){ var ww=Math.pow(lam, n-1-i); wsum+=ww; vsum+=ww*w[i]; }
    var recMean=vsum/wsum;
    var K=C.shrinkK, formConf=n/(n+K), mean=season + (recMean-season)*formConf; // shrink form deviation toward baseline
    if(opp && window.JTTScoring && JTTScoring.getDVPPct){                        // opponent softness (DVP)
      var dp=JTTScoring.getDVPPct(opp,(JTTScoring.POS_TO_DVP&&JTTScoring.POS_TO_DVP[p.position])||p.position,(typeof LEG_DVP!=='undefined'&&LEG_DVP[sk])||sk);
      if(dp!=null) mean *= (1 + _clamp((dp/100)*C.dvpBeta, -C.dvpCap, C.dvpCap));
    }
    if(C.roleStat && C.roleMkts.indexOf(sk)>=0){                                 // sport-specific role/usage trend
      var rv=logs.map(function(r){return +r[C.roleStat];}).filter(function(v){return isFinite(v);});
      if(rv.length>=4){ var recR=_mean(rv.slice(-5)), seaR=_mean(rv);
        if(seaR>3) mean *= (1 + _clamp((recR-seaR)/seaR*C.roleBeta, C.roleLo, C.roleHi)); }
    }
    mean *= (C.biasCorr||1);                                                    // backtested bias correction (per sport)
    if((SPORT.key)==='mlb' && ctx){ mean *= _mlbPlatoon(p,sk,ctx.oppHand); mean *= _mlbHomeAway(logs,sk,ctx.isHome,season); }
    var vsq=0; for(var j=0;j<n;j++){ var d=w[j]-recMean; vsq+=d*d; }             // dispersion: winsorized sd, shrunk to a mean-scaled prior
    var sampleSD=Math.sqrt(vsq/Math.max(1,n-1)), Ksd=4, priorSD=Math.max(0.8, mean*0.35);
    var sd=Math.max((n*sampleSD + Ksd*priorSD)/(n+Ksd), 0.6);
    var pModel = mean<C.poissonBelow ? (1-_poissonCDF(Math.ceil(line)-1, mean)) : (1-_normCDF((line-mean)/sd));
    pModel=_clamp(pModel, 0.02, 0.98);
    var prob=pModel;                                                            // blend model with empirical hit rate
    if(window.JTTScoring && JTTScoring.getHitRate){ var hr=JTTScoring.getHitRate(p.name, sk, line, true);
      if(hr && hr.n>=3){ var ew=Math.min(0.5, hr.n/(hr.n+8)); prob = ew*hr.rate + (1-ew)*pModel; } }
    return {mean:mean, sd:sd, prob:_clamp(prob,0.02,0.98), n:n};
  }
  // ===== MLB-specific adjustment layers =====
  function _mlbHand(nm){ if(!nm) return null; var m=state.idx._pHand; if(!m){ m=state.idx._pHand={}; (state.idx.players||[]).forEach(function(x){ if(x.throws) m[x.name]=x.throws; }); } return m[nm]||null; }
  function _mlbPlatoon(p, sk, oppHand){                         // batter split vs the opposing starter's hand
    if(!oppHand) return 1;
    var sp = oppHand==='L' ? p.splitVsL : p.splitVsR;
    if(!sp || (sp.PA||0)<20) return 1;                          // need a meaningful platoon sample
    var r=1;
    if(sk==='H' && +p.AVG>0) r=sp.AVG/(+p.AVG);
    else if(sk==='TB' && +p.SLG>0) r=sp.SLG/(+p.SLG);
    else if((sk==='RBI'||sk==='R') && +p.OPS>0) r=sp.OPS/(+p.OPS);
    else if(sk==='HR'){ var sr=(sp.HR||0)/Math.max(1,sp.PA), orr=(+p.HR||0)/Math.max(1,+p.PA); if(orr>0) r=sr/orr; }
    else return 1;
    var conf=Math.min(1, sp.PA/(sp.PA+50));                     // shrink the split ratio by its sample size
    return Math.max(0.75, Math.min(1.30, 1+(r-1)*conf));
  }
  function _mlbHomeAway(logs, sk, isHome, ov){                  // home/away split from game logs
    if(isHome==null || !ov) return 1;
    var rel=logs.filter(function(r){return !!r.home===isHome;}).map(function(r){return +r[sk];}).filter(function(v){return isFinite(v);});
    if(rel.length<10) return 1;
    var rm=rel.reduce(function(a,b){return a+b;},0)/rel.length, conf=Math.min(1, rel.length/(rel.length+15));
    return Math.max(0.85, Math.min(1.15, 1+(rm/ov-1)*conf));
  }
  function _legBadges(leg){
    const base=[], nerd=[], p=leg.p, sk=leg.statKey, line=leg.line, opp=leg.opp;
    const prob=leg.prob||0, season=leg.avg||0, avg=(leg.proj!=null?leg.proj:season), o=leg._odds;
    const vals=_mbLogsFor(p.name).map(r=>+r[sk]).filter(v=>isFinite(v));
    const l10=vals.slice(-10), l3=vals.slice(-3);
    const push=(a,k)=>{ if(a.indexOf(k)<0)a.push(k); };
    if(prob>=0.78 && (avg-line)>=1) push(base,'banker');
    if(o&&o.price){ const edge=prob-1/o.price; if(edge>=0.05)push(base,'value'); if(o.price>=3.0&&prob>=0.2)push(base,'longshot'); }
    else if(line>0 && (avg-line)/line>=0.18) push(base,'value');
    if(l3.length>=3){ const a3=l3.reduce((a,b)=>a+b,0)/l3.length; if(season&&a3>=season*1.12) push(base,'form'); }
    { let st=0; for(let i=vals.length-1;i>=0;i--){ if(vals[i]>=line)st++; else break; } if(st>=5) push(base,'streak'); }
    if(l10.length>=5 && Math.max.apply(null,l10)>=line*1.6) push(base,'ceiling');
    (leg.reasons||[]).forEach(r=>{ const L=(r.label||'').toLowerCase();
      if(/h2h|head|owns|vs\s/.test(L) && r.pct>=60) push(base,'bunny');
      if(/venue|ground|home/.test(L) && r.pct>=60) push(base,'venue'); });
    if(opp && window.JTTScoring && JTTScoring.getDVPPct){
      const dp=JTTScoring.getDVPPct(opp,(JTTScoring.POS_TO_DVP&&JTTScoring.POS_TO_DVP[p.position])||p.position,(typeof LEG_DVP!=='undefined'&&LEG_DVP[sk])||sk);
      if(dp!=null && dp>=15) push(base,'dvp'); }
    if((sk==='disposals'||sk==='clearances') && /Mid/.test(p.position||'') && typeof _taggerFacing==='function'){
      if(!_taggerFacing(p,opp)) push(base,'untagged'); }
    const cn=_mbLogsFor(p.name).map(r=>+r.contested).filter(v=>isFinite(v)); const dz=_mbLogsFor(p.name).map(r=>+r.disposals).filter(v=>isFinite(v));
    if(cn.length>=5 && dz.length>=5){ const cr=cn.slice(-6).reduce((a,b)=>a+b,0)/Math.min(6,cn.length), dr=dz.slice(-6).reduce((a,b)=>a+b,0)/Math.min(6,dz.length); if(dr&&cr/dr>=0.55) nerd.push(['Contested '+Math.round(cr/dr*100)+'%','ti-swords']); }
    const cb=_mbLogsFor(p.name).map(r=>+r.cba).filter(v=>isFinite(v));
    if(cb.length>=4){ const cr=cb.slice(-5).reduce((a,b)=>a+b,0)/Math.min(5,cb.length); if(cr>=70) nerd.push(['CBA '+Math.round(cr)+'%','ti-refresh']); }
    _sportBadges(leg).forEach(function(k){ if(base.indexOf(k)<0) base.push(k); });
    return {base:base, nerd:nerd};
  }
  function _dvMeetCached(a,b){ var k=(a<b?a+'|'+b:b+'|'+a), c=state.idx._dvMeetC||(state.idx._dvMeetC={}); if(!(k in c)) c[k]=_dvMeetings(a,b,20); return c[k]; }
  function _dejaVuStreak(leg){
    var p=leg.p, sk=leg.statKey, line=leg.line, opp=leg.opp; if(!opp) return 0;
    var vals;
    if((SPORT.key)==='mlb'){ var rows=(p.h2h&&p.h2h[opp])||[]; vals=rows.slice().reverse().map(function(r){return +r[sk];}).filter(function(v){return isFinite(v);}); }
    else { var meets=_dvMeetCached(p.team, opp); if(!meets.length) return 0;
      var lg=(state.idx.logsByName&&state.idx.logsByName[p.name])||[];
      vals=meets.map(function(mid){ for(var i=0;i<lg.length;i++){ if(lg[i].MatchId===mid) return +lg[i][sk]; } return null; }).filter(function(v){return v!=null&&isFinite(v);}); }
    var st=0; for(var i=0;i<vals.length;i++){ if(vals[i]>=line) st++; else break; } return st;
  }
  function _mbCandPool(){
    const banned=new Set((SETTINGS.multi&&SETTINGS.multi.banned)||[]);
    const pl=t=>(state.idx.players||[]).filter(p=>p.team===t&&(p.matches||0)>=3&&!banned.has(p.name)&&isPlaying(p.name,p.team));
    const fx=(state.data.fixture||[]).filter(g=>g.home&&g.away);
    const _foc=state.view.mbvFocus;
    const games=_foc?fx.filter(g=>g.home===_foc.home&&g.away===_foc.away):fx;
    let pool=[];
    var _mlb=SPORT.key==='mlb';
    games.forEach(g=>{
      pl(g.home).forEach(p=>legCandidates(p,g.away).forEach(c=>{ if(_mlb) c._pctx={isHome:true, oppHand:_mlbHand(g.awayPitcher)}; pool.push(c); }));
      pl(g.away).forEach(p=>legCandidates(p,g.home).forEach(c=>{ if(_mlb) c._pctx={isHome:false, oppHand:_mlbHand(g.homePitcher)}; pool.push(c); }));
    });
    if(_oddsLive()) pool=pool.filter(c=>c._odds);   // odds live -> only players/markets with a posted price
    const best={}; pool.forEach(c=>{ const key=c.p.name+'|'+c.statKey; if(!best[key]||c.score>best[key].score) best[key]=c; });
    const arr=Object.values(best); arr.forEach(function(c){ var m=_projModel(c.p,c.statKey,c.line,c.opp,c._pctx); c.proj=m.mean; c._deja=_dejaVuStreak(c); c.projSd=m.sd; if(m.prob!=null){ c.prob=m.prob; c.probReal=true; } });
    return arr.sort((a,b)=>(b.score-a.score)||(b.prob-a.prob));
  }
  function _mbvBody(){
    const M=SETTINGS.multi||(SETTINGS.multi={});
    if((state.data.fixture||[]).filter(g=>g.home&&g.away).length===0) return emptyState('ti-stack-2','No fixtures','The upcoming round hasn\'t loaded yet.');
    let pool=_mbCandPool();
    const oddsOn=_oddsLive();
    const mSel=M.cardMkt||'all', bSel=M.cardBook||'all';
    const mkts=[...new Set(pool.map(c=>c.statKey))];
    const books=oddsOn?[...new Set(pool.map(c=>c._odds&&c._odds.book).filter(Boolean))]:[];
    if(mSel!=='all') pool=pool.filter(c=>c.statKey===mSel);
    if(oddsOn&&bSel!=='all') pool=pool.filter(c=>c._odds&&c._odds.book===bSel);
    pool.forEach(l=>{ l._bd=_legBadges(l); });
    pool=pool.filter(l=>l._bd.base.length>0);                 // only legs that actually earn a badge
    pool=_mbBalance(pool);                                    // even split between the two teams
    const slip=state.view.mbSlip||(state.view.mbSlip={});
    const chosen=pool.filter(c=>slip[c.id]);
    const PER=10, pages=Math.max(1,Math.ceil(pool.length/PER));
    let page=Math.min(state.view.mbvPage||0, pages-1); if(page<0)page=0;
    const shown=pool.slice(page*PER, page*PER+PER);
    let comb=1, conf=1; chosen.forEach(c=>{ const px=(c._odds&&c._odds.price)||(c.prob>0?1/c.prob:null); if(px)comb*=px; conf*=Math.max(0.05,Math.min(0.99,c.prob||0)); });
    const _lbl=k=>({disposals:'Disposals',goals:'Goals',dreamteam:'Fantasy',marks:'Marks',tackles:'Tackles',clearances:'Clearances',kicks:'Kicks',handballs:'Handballs',shots:'Shots',points:'Points',assists:'Assists',saves:'Saves',hits:'Hits',blocks:'Blocks',H:'Hits',TB:'Bases',HR:'HR',RBI:'RBI',R:'Runs',K:'Ks'}[k]||k);
    // header + summary
    let h='<div class="mbv-head"><div class="mbv-title"><i class="ti ti-stack-2"></i> Multi Builder</div>'
      +'<div class="mbv-sum">'+chosen.length+' leg'+(chosen.length!==1?'s':'')+(chosen.length?(oddsOn?' \u00b7 $'+comb.toFixed(2):' \u00b7 '+Math.round(conf*100)+'%'):'')+'</div></div>';
    // filters (dropdowns)
    h+='<div class="mbv-filters">'
      +'<select class="ob-sel" onchange="_mbSetCardMkt(this.value)"><option value="all"'+(mSel==='all'?' selected':'')+'>All markets</option>'
        +mkts.map(k=>'<option value="'+k+'"'+(k===mSel?' selected':'')+'>'+_lbl(k)+'</option>').join('')+'</select>';
    if(oddsOn){ h+='<select class="ob-sel" onchange="_mbSetCardBook(this.value)"><option value="all"'+(bSel==='all'?' selected':'')+'>All books</option>'
        +books.map(b=>'<option value="'+b+'"'+(b===bSel?' selected':'')+'>'+b+'</option>').join('')+'</select>'; }
    h+='</div>';
    if(!pool.length) return h+emptyState('ti-search-off','No legs','No qualifying legs for this filter.');
    // cards
    h+='<div class="mbv-list">'+shown.map(l=>{
      const bd=l._bd, on=!!slip[l.id];
      const dots=(()=>{ const n=Math.round((l.prob||0)*5); let s=''; for(let i=0;i<5;i++)s+='<span class="mbv-dot'+(i<n?' on':'')+'"></span>'; return s; })();
      const px=l._odds&&l._odds.price;
      const edge=px?('+'+((l.prob-1/px)*100).toFixed(0)+'% EV'):(((l.proj!=null?l.proj:l.avg)-l.line)>0?('+'+((l.proj!=null?l.proj:l.avg)-l.line).toFixed(1)):'');
      const oddsPill=px?('<div class="mbv-op"><span class="mbv-op-price">$'+px.toFixed(2)+'</span><span class="mbv-op-book">'+esc((l._odds.book||'').slice(0,9))+'</span></div>'):('<div class="mbv-op mbv-op-nb"><span class="mbv-op-price">'+Math.round((l.prob||0)*100)+'%</span><span class="mbv-op-book">proj</span></div>');
      const ctx=(l.reasons&&l.reasons.length)?l.reasons.map(function(r){return esc(r.label)+(r.pct?' '+Math.round(r.pct)+'%':'');}).join(' \u00b7 '):'';
      return '<div class="mbv-card'+(on?' on':'')+'" onclick="_mbToggleLeg(\''+l.id.replace(/'/g,"\\'")+'\')">'
        +'<div class="mbv-row1">'+teamLogo(l.p.team,30)
          +'<div class="mbv-pn"><div class="mbv-nm">'+esc(l.p.name)+'<span class="mbv-tm">'+abbr(l.p.team)+' \u00b7 '+posShort(l.p.position)+'</span></div>'
          +'<div class="mbv-mk">'+l.line+'+ '+_lbl(l.statKey)+'<span class="mbv-proj">proj '+fmt(l.proj!=null?l.proj:l.avg,l.statKey==='goals'?1:l.statKey==='dreamteam'?0:1)+'</span></div></div>'
          +oddsPill+'<div class="mbv-add"><i class="ti '+(on?'ti-circle-check-filled':'ti-circle-plus')+'"></i></div></div>'
        +'<div class="mbv-row2"><span class="mbv-dots">'+dots+'</span>'
          +'<span class="mbv-hit">'+(l.probReal?Math.round(l.prob*100)+'% hit':'~'+Math.round(l.prob*100)+'%')+'</span>'
          +(edge?'<span class="mbv-edge">'+edge+'</span>':'')
          +'<span class="mbv-opp">v '+abbr(l.opp||'')+'</span></div>'
        +(ctx?'<div class="mbv-ctx">'+ctx+'</div>':'')+'<div class="mbv-badges">'+((l._deja>=2)?'<span class="mbv-bdg" style="background:#EEEDFE;color:#26215C"><i class="ti ti-versions"></i>Déjà Vu x'+l._deja+'</span>':'')+bd.base.map(k=>{const b=_MB_ALLBDG[k];if(!b)return '';return '<span class="mbv-bdg" style="background:'+b[1]+';color:'+b[2]+'"><i class="ti '+b[3]+'"></i>'+b[0]+'</span>';}).join('')
          +bd.nerd.map(nx=>'<span class="mbv-nerd"><i class="ti '+nx[1]+'"></i>'+nx[0]+'</span>').join('')+'</div>'
        +'</div>';
    }).join('')+'</div>';
    if(pages>1){ h+='<div class="mbv-pager">'+'<button class="mbv-pg"'+(page<=0?' disabled':'')+' onclick="_mbvPage(-1)"><i class="ti ti-chevron-left"></i></button>'+'<span class="mbv-pg-lbl">'+(page*PER+1)+'\u2013'+Math.min(pool.length,page*PER+PER)+' of '+pool.length+'</span>'+'<button class="mbv-pg"'+(page>=pages-1?' disabled':'')+' onclick="_mbvPage(1)"><i class="ti ti-chevron-right"></i></button></div>'; }
    if(chosen.length>=2){ h+='<button class="mbv-save" onclick="_mbSaveCards()">Save '+chosen.length+'-leg multi to Bet Slip</button>'; }
    return h;
  }
  function _fxMultiCards(g){ if(!g) return ''; state.view.mbvFocus={home:g.home,away:g.away}; state.view.mbvPage=0; return '<div class="fx-mtool" style="margin-top:14px">'+_multiCardsView()+'</div>'; }
  function _multiCardsView(){ return '<div id="mbv-wrap">'+_mbvBody()+'</div>'; }
  window._mbvRerender=function(){ var el=document.getElementById('mbv-wrap'); if(el) el.innerHTML=_mbvBody(); };
  function _mbBalance(pool){ var foc=state.view.mbvFocus; if(!foc) return pool;
    var A=pool.filter(function(l){return l.p.team===foc.home;}), B=pool.filter(function(l){return l.p.team===foc.away;});
    var out=[], n=Math.max(A.length,B.length); for(var i=0;i<n;i++){ if(A[i])out.push(A[i]); if(B[i])out.push(B[i]); } return out; }
  window._mbvPage=function(d){ state.view.mbvPage=Math.max(0,(state.view.mbvPage||0)+d); _mbvRerender(); };
  window._mbSetCardMkt=function(v){ SETTINGS.multi.cardMkt=v; state.view.mbvPage=0; saveSettings(); _mbvRerender(); };
  window._mbSetCardBook=function(v){ SETTINGS.multi.cardBook=v; state.view.mbvPage=0; saveSettings(); _mbvRerender(); };
  window._mbToggleLeg=function(id){ const s=state.view.mbSlip||(state.view.mbSlip={}); if(s[id])delete s[id]; else s[id]=1; _mbvRerender(); };
  window._mbSaveCards=function(){ const pool=_mbCandPool(), s=state.view.mbSlip||{}; const legs=pool.filter(c=>s[c.id]); if(legs.length<2)return;
    let comb=1; legs.forEach(l=>{ const px=(l._odds&&l._odds.price)||(l.prob>0?1/l.prob:null); if(px)comb*=px; });
    _cardAddMultiEntry({ title:legs.length+'-leg multi', legs:legs.map(l=>({name:l.p.name,label:l.betLabel,odds:l._odds&&l._odds.price})), odds:comb, book:'', src:'Multi Builder' }); };
  ...
  function _degenTileKeys(){
    if(SPORT.key==='afl'||SPORT.key==='aflw'){ var _hc=(window.SPORT_CONFIG&&SPORT_CONFIG.hideCrew)||[]; return ['snags','elite','bunnies','hunting','nmu','kickins','bogey','streak','climb','form','taggers'].filter(function(k){return _hc.indexOf(k)<0;}); }
  ...
  function renderTile(k){
    ...
    if(SPORT.key!=='afl'&&SPORT.key!=='aflw'){ const _S=_signals(); if(_S&&_S.tiles&&typeof _S.tiles[k]==='function'){ try{ return _S.tiles[k](); }catch(_e){ return ''; } } if(k!=='multi') return ''; }
    ...
    if(k==='multi'){
      state.view.mbvFocus=null; state.view.mbvPage=0;
      return _multiCardsView();
    }
  ...
  function multiResults(){
    ...
    const mode=SETTINGS.multi.mode||'smart';
    if(!games.length){
      if((window.SPORT_CONFIG||{}).noPlayerOdds) return emptyState('ti-stack-2','No player-prop odds for this sport','The Multi Builder prices every leg against a posted line, so it needs a player-prop odds feed. Use the Degen signals — they run on the stats (form, H2H, DVP) — for your plays.');
  ...
  function comparePage(){
    ...
    let h=dl+'<div class="filterbar"><div class="cmp-pick">'+inp('a',_cmpA)+'<span class="cmp-vs">vs</span>'+inp('b',_cmpB)+'</div></div>';
  ...
  // ===== Degenerates page: high-variance round multis + the shared 10-day ladder =====
  function _mktLbl(k){ var m=[].concat(MULTI_MKTS||[],ODDS_MKTS||[],DV_MARKETS||[]); for(var i=0;i<m.length;i++){ if(m[i][0]===k) return m[i][1]; } return k; }
  function _gameKey(a,b){ return [a,b].sort().join('|'); }
  function _degWrap(title, icon, sub, body){
    return '<div class="deg-sec"><div class="deg-sec-h"><i class="ti '+icon+'"></i> '+title+'</div>'
      +(sub?'<div class="deg-sec-sub">'+sub+'</div>':'')+(body||'')+'</div>';
  }
  function _degCard(l){
    var bd=_legBadges(l), px=l._odds&&l._odds.price;
    var badges=bd.base.slice(0,3).map(function(k){var b=_MB_ALLBDG[k];if(!b)return '';return '<span class="mbv-bdg" style="background:'+b[1]+';color:'+b[2]+'"><i class="ti '+b[3]+'"></i>'+b[0]+'</span>';}).join('');
    return '<div class="mbv-card">'
      +'<div class="mbv-row1">'+teamLogo(l.p.team,28)
        +'<div class="mbv-pn"><div class="mbv-nm">'+esc(l.p.name)+'<span class="mbv-tm">'+abbr(l.p.team)+' \u00b7 v '+abbr(l.opp||'')+'</span></div>'
        +'<div class="mbv-mk"><b>'+l.line+'+</b> '+_mktLbl(l.statKey)+'<span class="mbv-proj">proj '+fmt(l.proj!=null?l.proj:l.avg,1)+'</span></div></div>'
        +(px?'<div class="mbv-op"><span class="mbv-op-price">$'+px.toFixed(2)+'</span><span class="mbv-op-book">'+esc((l._odds.book||'').slice(0,9))+'</span></div>':'<div class="mbv-op mbv-op-nb"><span class="mbv-op-price">'+Math.round((l.prob||0)*100)+'%</span><span class="mbv-op-book">proj</span></div>')+'</div>'
      +(badges?'<div class="mbv-badges">'+badges+'</div>':'')+'</div>';
  }

  // available books (most priced lines first), and the whole pool priced at ONE selected book
  function _degBooks(){
    var cnt={}, bi=state.idx.bookIdx||{};
    Object.keys(bi).forEach(function(nm){ var mk=bi[nm]; Object.keys(mk).forEach(function(sk){ mk[sk].forEach(function(r){ if(r.over!=null&&_bookOK(r.book)) cnt[r.book]=(cnt[r.book]||0)+1; }); }); });
    return Object.keys(cnt).sort(function(a,b){return cnt[b]-cnt[a];});
  }
  function _degBookLegs(book){
    var out=[], games=(state.data.fixture||[]).filter(function(g){return g.home&&g.away;}), opp={};
    games.forEach(function(g){ opp[g.home]=g.away; opp[g.away]=g.home; });
    (state.idx.players||[]).forEach(function(p){
      if(!opp[p.team]||!isPlaying(p.name,p.team)) return;
      var o=opp[p.team], mk=(state.idx.bookIdx||{})[p.name]||{};
      Object.keys(mk).forEach(function(sk){
        mk[sk].forEach(function(r){
          if(r.book!==book||r.over==null) return;
          var m=_projModel(p,sk,r.line,o);
          out.push({p:p,opp:o,statKey:sk,line:r.line,over:r.over,under:r.under,proj:m.mean,prob:m.prob,_odds:{price:r.over,book:book,line:r.line}});
        });
      });
    });
    return out;
  }
  function _degProjLegs(){ state.view.mbvFocus=null; return _mbCandPool(); }   // no-odds sports: projection legs

  // --- The Train: ONE leg per game (the best short-priced banker) ---
  function _gameStart(team, opp){ var g=(state.data.fixture||[]).find(function(x){ return (x.home===team||x.away===team)&&(x.home===opp||x.away===opp); }); if(!g) return null;
    var t=g.date||g.utc||g.commence_time||g.gameTimeUTC||g.kickoff_time||g.start||g.dateUtc; var ms=t?Date.parse(t):NaN; return isNaN(ms)?null:ms; }
  function _sameDayLocal(a,b){ var x=new Date(a), y=new Date(b); return x.getFullYear()===y.getFullYear()&&x.getMonth()===y.getMonth()&&x.getDate()===y.getDate(); }
  function _trainSection(legs, oddsOn, book){
    var cands = oddsOn ? legs.filter(function(l){return l.over>=1.30&&l.over<=1.60;}) : legs.filter(function(l){return (l.prob||0)>=0.80;});
    var byGame={};                                                   // best (highest prob) leg per game
    cands.forEach(function(l){ var g=_gameKey(l.p.team,l.opp), cur=byGame[g]; if(!cur||(l.prob||0)>(cur.prob||0)) byGame[g]=l; });
    var daily=_dailySched();
    var picks=Object.keys(byGame).map(function(g){return {l:byGame[g], t:_gameStart(byGame[g].p.team, byGame[g].opp)};})
      .filter(function(x){ return !daily || x.t==null || _sameDayLocal(x.t, Date.now()); })   // daily sports: only today's games
      .sort(function(a,b){ return (a.t||Infinity)-(b.t||Infinity); })                          // earliest start first
      .map(function(x){ return x.l; });
    if(!picks.length) return _degWrap('The Train','ti-train','No qualifying banker legs'+(oddsOn?' at '+bookName(book):'')+' today.','');
    var comb=1,conf=1; picks.forEach(function(l){ var px=oddsOn?l.over:(l.prob>0?1/l.prob:2); comb*=px; conf*=Math.max(0.5,l.prob||0.5); });
    var daily=_dailySched();
    var sub=oddsOn?('$'+comb.toFixed(2)+' @ '+bookName(book)):(Math.round(conf*100)+'%');
    return _degWrap('The Train','ti-train',sub,'<div class="mbv-list">'+picks.map(_degCard).join('')+'</div>');
  }

  // --- The Pyramid: sport-relevant stat, self-lowering apex, min $1.25/leg, rendered as a pyramid ---
  function _pyrCard(pl, line, over, statLbl){
    var ln=(pl.p.name||'').split(' '); ln=ln[ln.length-1];
    return '<div class="pyr-card"><button class="pyr-swap" data-nm="'+esc(pl.p.name)+'" onclick="_pyrSkip(this.getAttribute(\'data-nm\'))" title="Swap to next best"><i class="ti ti-refresh"></i></button>'+teamLogo(pl.p.team,22)+'<div class="pyr-nm">'+esc(ln)+'</div><div class="pyr-ln">'+line+'+ '+esc(statLbl||'')+'</div><div class="pyr-od">'+(over?'$'+over.toFixed(2):'proj '+Math.round(pl.proj))+'</div></div>';
  }
  var PYR_RUNG={disposals:5,kicks:3,handballs:3,marks:2,tackles:2,goals:1,clearances:2,hitouts:4,dreamteam:20,inside50s:2,
    passYds:25,rushYds:15,recYds:15,receptions:1,rushRecYds:15,anytimeTd:1,passComp:3,passAtt:5,rushAtt:3,soloTk:1,
    points:4,rebounds:2,assists:2,threes:1,steals:1,blocks:1,pra:6,
    shots:1,saves:5,ppPoints:1,
    H:1,TB:1,HR:1,RBI:1,R:1,SB:1,K:2};
  var PYR_STATS={
    afl:['disposals','kicks','handballs','marks','tackles','goals'],
    aflw:['disposals','kicks','handballs','marks','tackles','goals'],
    nfl:['recYds','rushYds','passYds','receptions'], ncaaf:['recYds','rushYds','passYds','receptions'],
    nba:['points','rebounds','assists','threes'], wnba:['points','rebounds','assists','threes'],
    nhl:['shots','points','saves'], mlb:['K'],
    epl:['shots','shotsOn','tackles','keyPasses','foulsCommitted']
  };
  function _pyrStatOpts(){
    var wl=PYR_STATS[SPORT.key], mk=(MULTI_MKTS||[]);
    if(!wl) return mk;
    return wl.map(function(k){ for(var i=0;i<mk.length;i++){ if(mk[i][0]===k) return mk[i]; } return [k,_mktLbl(k)]; });
  }
  function _pyrDefaultStat(){ var d={afl:'disposals',aflw:'disposals',nfl:'recYds',ncaaf:'recYds',nba:'points',wnba:'points',nhl:'shots',mlb:'K',epl:'shots'}[SPORT.key]; var mk=_pyrStatOpts().map(function(m){return m[0];}); return (d&&mk.indexOf(d)>=0)?d:(mk[0]||'disposals'); }
  window._degSetPyrStat=function(s){ state.view.pyrStat=s; var el=document.getElementById('deg-wrap'); if(el) el.innerHTML=_degBody(); };
  window._pyrSkip=function(name){ var st=(state.view.pyrStat&&_pyrStatOpts().some(function(m){return m[0]===state.view.pyrStat;}))?state.view.pyrStat:_pyrDefaultStat(); state.view.pyrSkip=state.view.pyrSkip||{}; state.view.pyrSkip[st]=state.view.pyrSkip[st]||[]; if(state.view.pyrSkip[st].indexOf(name)<0) state.view.pyrSkip[st].push(name); var el=document.getElementById('deg-wrap'); if(el) el.innerHTML=_degBody(); };
  window._pyrReset=function(){ var st=(state.view.pyrStat&&_pyrStatOpts().some(function(m){return m[0]===state.view.pyrStat;}))?state.view.pyrStat:_pyrDefaultStat(); if(state.view.pyrSkip) state.view.pyrSkip[st]=[]; var el=document.getElementById('deg-wrap'); if(el) el.innerHTML=_degBody(); };
  function _pyramidSection(legs, oddsOn, book){
    var _pyrMinM=(SPORT.key==='mlb')?2:5;   // pitchers (K) start infrequently
    var stat=(state.view.pyrStat&&_pyrStatOpts().some(function(m){return m[0]===state.view.pyrStat;}))?state.view.pyrStat:_pyrDefaultStat();
    var statLbl=_mktLbl(stat);
    var _skc=((state.view.pyrSkip||{})[stat])||[];
    var statSel='<div class="deg-bookbar"><span class="deg-bl">Stat</span><select class="ob-sel" onchange="_degSetPyrStat(this.value)">'+_pyrStatOpts().map(function(m){return '<option value="'+m[0]+'"'+(m[0]===stat?' selected':'')+'>'+m[1]+'</option>';}).join('')+'</select>'+(_skc.length?'<button class="pyr-reset" onclick="_pyrReset()"><i class="ti ti-rotate"></i> Reset '+_skc.length+'</button>':'')+'</div>';
    var byP={};
    if(oddsOn){
      legs.filter(function(l){return l.statKey===stat;}).forEach(function(l){ var e=byP[l.p.name]||(byP[l.p.name]={p:l.p,opp:l.opp,proj:l.proj,lines:{}}); e.lines[l.line]=l.over; });
    } else {
      var gs=(state.data.fixture||[]).filter(function(g){return g.home&&g.away;}), ts={}; gs.forEach(function(g){ts[g.home]=1;ts[g.away]=1;});
      (state.idx.players||[]).filter(function(p){return ts[p.team]&&(p.matches||0)>=_pyrMinM&&isPlaying(p.name,p.team);}).forEach(function(p){ var o=nextOpp(p.team), m=_projModel(p,stat,1,o); if(m.mean>0) byP[p.name]={p:p,opp:o,proj:m.mean,lines:{}}; });
    }
    var players=Object.keys(byP).map(function(k){return byP[k];}).sort(function(a,b){return b.proj-a.proj;});
    if(oddsOn && players.length<3){                          // book lacks an alt ladder for this stat -> fall back to projections
      byP={}; var gsf=(state.data.fixture||[]).filter(function(g){return g.home&&g.away;}), tsf={}; gsf.forEach(function(g){tsf[g.home]=1;tsf[g.away]=1;});
      (state.idx.players||[]).filter(function(p){return tsf[p.team]&&(p.matches||0)>=_pyrMinM&&isPlaying(p.name,p.team);}).forEach(function(p){ var o=nextOpp(p.team), m=_projModel(p,stat,1,o); if(m.mean>0) byP[p.name]={p:p,opp:o,proj:m.mean,lines:{}}; });
      players=Object.keys(byP).map(function(k){return byP[k];}).sort(function(a,b){return b.proj-a.proj;});
      oddsOn=false;
    }
    var _sk=((state.view.pyrSkip||{})[stat])||[];
    if(_sk.length) players=players.filter(function(pl){return _sk.indexOf(pl.p.name)<0;});
    if(players.length<3) return _degWrap('The Pyramid','ti-triangle','', statSel+'<div class="deg-empty">Not enough '+esc(statLbl)+' data to build a pyramid.</div>');
    var rung=PYR_RUNG[stat]||Math.max(1,Math.round(players[0].proj/6));
    function overAt(pl,target){ var best=null,bd=99; Object.keys(pl.lines).forEach(function(ln){ var d=Math.abs(+ln-target); if(d<bd&&d<=Math.max(0.6,rung*0.6)){bd=d;best={line:+ln,over:pl.lines[ln]};} }); return best; }
    function tierPick(line,n,used){ return players.filter(function(pl){ if(used[pl.p.name]||pl.proj<line)return false; if(!oddsOn)return true; var oa=overAt(pl,line); return oa&&oa.over>=1.25; }).slice(0,n); }
    var apexLine=Math.max(rung,Math.floor(players[0].proj/rung)*rung), apex=[],mid=[],base=[],used={};
    for(var t=0;t<10 && apexLine>=rung;t++){ used={}; apex=tierPick(apexLine,1,used); if(apex.length){ apex.forEach(function(x){used[x.p.name]=1;}); mid=tierPick(apexLine-rung,2,used); mid.forEach(function(x){used[x.p.name]=1;}); base=tierPick(Math.max(rung*0.5,apexLine-2*rung),3,used); if(mid.length&&base.length) break; } apexLine-=rung; }
    var midLine=apexLine-rung, baseLine=Math.max(rung*0.5,apexLine-2*rung);
    var total=apex.length+mid.length+base.length;
    if(!apex.length||total<3) return _degWrap('The Pyramid','ti-triangle','', statSel+'<div class="deg-empty">Couldn\u2019t form a full '+esc(statLbl)+' pyramid'+(oddsOn?' at '+bookName(book):'')+'.</div>');
    function row(list,line){ return '<div class="pyr-row">'+list.map(function(pl){ var oa=oddsOn?overAt(pl,line):null; return _pyrCard(pl,line,oa?oa.over:null,statLbl); }).join('')+'</div>'; }
    var comb=1; if(oddsOn){ [[apex,apexLine],[mid,midLine],[base,baseLine]].forEach(function(tt){ tt[0].forEach(function(pl){ var oa=overAt(pl,tt[1]); if(oa)comb*=oa.over; }); }); }
    var body=statSel+'<div class="pyr">'+row(apex,apexLine)+row(mid,midLine)+row(base,baseLine)+'</div>';
    var sub=oddsOn?('$'+comb.toFixed(2)+' @ '+bookName(book)):'';
    return _degWrap('The Pyramid','ti-triangle',sub, body);
  }

  // --- The 10-day Ladder (shared, pipeline-fed via AFL/data/ladder.json) ---
  function _ladderSection(){
    var L=(state.data.ladder||null);
    if(!L||!L.days||!L.days.length){
      return _degWrap('The Ladder','ti-stairs-up','',
        '<div class="deg-empty"><i class="ti ti-hourglass"></i> The climb starts here once the odds feed is live.</div>');
    }
    var start=L.start||10, target=L.target||10000, bank=L.bank!=null?L.bank:start;
    var busted=L.days.some(function(d){return d.result==='loss';});
    var topped=L.days.some(function(d){return d.complete;});
    // rungs shown newest at the top so the climb reads upward
    var rows=L.days.slice().reverse().map(function(d){
      var st=d.result==='win'?'win':d.result==='loss'?'loss':'pending';
      var ic=st==='win'?'ti-circle-check-filled':st==='loss'?'ti-circle-x-filled':'ti-arrow-big-up-lines';
      var legs=(d.legs&&d.legs.length)?d.legs:[];
      function _ladLeg(l,cls){var nm=(l.name||'').split(' ');nm=nm[nm.length-1];return '<span class="lad-leg'+(cls?' '+cls:'')+'">'+(l.team?teamLogo(l.team,14):'')+esc(nm)+' <b>'+l.line+'+</b> '+_mktLbl(l.market)+(l.opp?' <span class="lad-vs">v '+abbr(l.opp)+'</span>':'')+'</span>';}
      var legHtml=legs.map(function(l){return _ladLeg(l);}).join('');
      var subHtml=(d.sub&&st==='pending')?('<div class="lad-sub"><span class="lad-sub-lbl">SUB</span>'+_ladLeg(d.sub)+'</div>'):'';
      var bb=d.bank_before, ba=d.bank_after;
      var money=st==='win'?('$'+(+bb).toFixed(2)+' \u2192 <b>$'+(+ba).toFixed(2)+'</b>')
               :st==='loss'?('$'+(+bb).toFixed(2)+' \u2192 <b>busted</b>')
               :('$'+(bb!=null?(+bb).toFixed(2):'-')+' \u00d7 '+(d.odds?(+d.odds).toFixed(2):'-')+' \u2192 $'+(bb!=null&&d.odds?(bb*d.odds).toFixed(2):'-'));
      return '<div class="lad-day lad-'+st+'">'
        +'<div class="lad-day-h"><span class="lad-rung">Rung '+(d.rung||'?')+'</span>'
        +(d.type?'<span class="lad-type lad-type-'+d.type+'">'+(d.type==='sgm'?'SGM':'MULTI')+'</span>':'')
        +'<i class="ti '+ic+' lad-ic"></i>'
        +'<span class="lad-money">'+money+'</span>'
        +(d.book?'<span class="lad-book">@ '+esc(d.book)+'</span>':'')+'</div>'
        +(legHtml?'<div class="lad-legs">'+legHtml+'</div>':'')+subHtml+'</div>';
    }).join('');
    var wins=L.days.filter(function(d){return d.result==='win';}).length;
    // progress on a log scale ($10 -> $10k spans 3 orders of magnitude)
    var prog=Math.max(0,Math.min(1, Math.log(Math.max(bank,start)/start)/Math.log(target/start)));
    var rung=(L.days[L.days.length-1]||{}).rung||wins+1;
    var head='<div class="lad-hero">'
      +'<div class="lad-hero-top"><span class="lad-goal">$'+start+' \u2192 $'+(target>=1000?(target/1000)+'k':target)+'</span>'
        +'<span class="lad-cur">$'+(+bank).toFixed(2)+'</span></div>'
      +'<div class="lad-bar"><div class="lad-bar-fill" style="width:'+(prog*100).toFixed(1)+'%"></div></div>'
      +'<div class="lad-hero-sub">'+(topped?'<b class="lad-tag-win">Top of the ladder \u2014 challenge complete!</b>'
          :busted?'<b class="lad-tag-bust">Busted \u2014 next climb starts from $'+start+'</b>'
          :'Rung '+rung+' \u00b7 '+wins+' cleared \u00b7 peak $'+(+(L.peak||bank)).toFixed(2))
        +(L.attempt>1?' \u00b7 attempt #'+L.attempt:'')+'</div></div>';
    return _degWrap('The Ladder','ti-stairs-up','',
      head+'<div class="lad-grid">'+rows+'</div>');
  }
  // --- The Yankee: 4 SGMs (~$3 each) across 4 games, staked as an 11-bet system ---
  function _sgmCard(sgm){
    return '<div class="mbv-card yk-sgm">'
      +'<div class="yk-sgm-h">'+teamLogo(sgm.legs[0].p.team,20)+'<span class="yk-sgm-g">'+abbr(sgm.legs[0].p.team)+' v '+abbr(sgm.legs[0].opp)+'</span><span class="yk-sgm-od">$'+sgm.od.toFixed(2)+' SGM</span></div>'
      +sgm.legs.map(function(l){return '<div class="yk-leg"><span class="yk-lp">'+esc(l.p.name)+'</span><span class="yk-lm">'+l.line+'+ '+_mktLbl(l.statKey)+'</span><span class="yk-lo">$'+l.over.toFixed(2)+'</span></div>';}).join('')
      +'</div>';
  }
  function _yankeeSGMs(legs){
    var byGame={}; legs.forEach(function(l){ if(l.over<1.4||l.over>2.1)return; var g=_gameKey(l.p.team,l.opp); (byGame[g]=byGame[g]||[]).push(l); });
    var sgms=[];
    Object.keys(byGame).forEach(function(g){
      var gl=byGame[g], byPl={}; gl.forEach(function(l){ if(!byPl[l.p.name]||l.prob>byPl[l.p.name].prob) byPl[l.p.name]=l; });
      var cand=Object.keys(byPl).map(function(k){return byPl[k];}).sort(function(a,b){return b.prob-a.prob;}).slice(0,8);
      var best=null;
      for(var i=0;i<cand.length;i++) for(var j=i+1;j<cand.length;j++){
        var od2=cand[i].over*cand[j].over;
        if(od2>=2.5&&od2<=3.5){ var pr2=(cand[i].prob||0)*(cand[j].prob||0); if(!best||pr2>best.pr) best={legs:[cand[i],cand[j]],od:od2,pr:pr2}; }
        for(var k=j+1;k<cand.length;k++){                                   // 3-leg SGMs too (2-3 legs)
          var od3=od2*cand[k].over; if(od3<2.5||od3>3.5) continue;
          var pr3=(cand[i].prob||0)*(cand[j].prob||0)*(cand[k].prob||0); if(!best||pr3>best.pr) best={legs:[cand[i],cand[j],cand[k]],od:od3,pr:pr3};
        }
      }
      if(best) sgms.push(best);
    });
    return sgms.sort(function(a,b){return b.pr-a.pr;}).slice(0,4);
  }
  function _yankeeSystem(o){
    var doubles=[],trebles=[],four=1,i,j,k;
    for(i=0;i<4;i++)for(j=i+1;j<4;j++) doubles.push(o[i]*o[j]);
    for(i=0;i<4;i++)for(j=i+1;j<4;j++)for(k=j+1;k<4;k++) trebles.push(o[i]*o[j]*o[k]);
    four=o[0]*o[1]*o[2]*o[3];
    var sum=function(a){return a.reduce(function(x,y){return x+y;},0);};
    return {allWin:sum(doubles)+sum(trebles)+four, doubles:doubles, trebles:trebles, four:four, outlay:11};
  }
  function _yankeeSection(legs, oddsOn, book){
    if(!oddsOn) return _degWrap('The Yankee','ti-versions','','<div class="deg-empty"><i class="ti ti-lock"></i> Needs live odds \u2014 four priced ~$3 SGMs.</div>');
    var sgms=_yankeeSGMs(legs);
    if(sgms.length<4) return _degWrap('The Yankee','ti-versions','','<div class="deg-empty">Not enough games with a ~$3 SGM at '+bookName(book)+'.</div>');
    var sys=_yankeeSystem(sgms.map(function(s){return s.od;}));
    var cards='<div class="mbv-list">'+sgms.map(_sgmCard).join('')+'</div>';
    var box='<div class="yk-sys"><div class="yk-sys-r"><span>Stake</span><span>$11 ($1 \u00d7 11 bets)</span></div>'
      +'<div class="yk-sys-r"><span>6 doubles + 4 trebles + 1 four-fold</span><span></span></div>'
      +'<div class="yk-sys-r yk-sys-max"><span>Return if all 4 land</span><span>$'+sys.allWin.toFixed(2)+'</span></div>'
      +'</div>';
    return _degWrap('The Yankee','ti-versions','', cards+box);
  }

  function _degBody(){
    var oddsOn=_oddsLive();
    var books=oddsOn?_degBooks():[];
    var book=(oddsOn&&(state.view.degBook&&books.indexOf(state.view.degBook)>=0?state.view.degBook:books[0]))||null;
    var bar=(oddsOn&&books.length)?('<div class="deg-bookbar"><span class="deg-bl">Bookmaker</span><select class="ob-sel" onchange="_degSetBook(this.value)">'+books.map(function(b){return '<option value="'+b+'"'+(b===book?' selected':'')+'>'+bookName(b)+'</option>';}).join('')+'</select><span class="deg-bl" style="font-weight:400">one book \u2014 legs are stakeable together</span></div>'):'';
    var legs=oddsOn?_degBookLegs(book):_degProjLegs();
    // Degenerates eligibility: drop tiny-sample players and anyone flagged injured/just-returning
    var _minG=((window.SPORT_CONFIG&&SPORT_CONFIG.legMinGames)!=null?SPORT_CONFIG.legMinGames:({afl:4,aflw:4,nfl:3,ncaaf:3,mlb:12,nba:8,wnba:6,nhl:8}[SPORT.key]))||3;
    var _inj=new Set((state.data.injury||[]).map(function(r){return r.Player||r.player||r.name;}).filter(Boolean));
    var _gp=function(p){return p.matches!=null?+p.matches:(p.G!=null?+p.G:(p.games!=null?+p.games:(p.gp!=null?+p.gp:null)));};
    legs=legs.filter(function(l){ if(!l||!l.p) return false; var g=_gp(l.p); if(g!=null&&g<_minG) return false; if(_inj.has(l.p.name)) return false; return true; });
    return bar+_ladderSection()+_trainSection(legs,oddsOn,book)+_pyramidSection(legs,oddsOn,book)+_yankeeSection(legs,oddsOn,book);
  }
  window._degSetBook=function(b){ state.view.degBook=b; var el=document.getElementById('deg-wrap'); if(el) el.innerHTML=_degBody(); };
  function degeneratesPage(){ state.view.mbvFocus=null; return '<div id="deg-wrap" class="deg-page">'+_degBody()+'</div>'; }

  function _crewTileHTML(k){
    const c=CREW.find(x=>x.k===k); if(!c) return '';
    let body;
    const teams=(state.data.fixture||[]).flatMap(g=>[g.home,g.away]);
    if(k==='board') body=todaysBoardPage();
    else if(k==='degenerates') body=degeneratesPage();
    ...
    return '<button class="back-btn" onclick="_switchTab(\'degen\')"><i class="ti ti-arrow-left"></i> Degen Crew</button><div class="page-header"><div class="page-title">'+c.n+'</div><div class="page-sub">'+c.d+'</div></div>'+body;
  }
  window.degenTile=function(k){
    state.view.crewTile=k;                                  // persist so the 2.5-min odds refresh re-renders this tile, not the grid
    const t=_crewTileHTML(k); if(!t) return;
    document.getElementById('page-content').innerHTML=t;
  ...
  function renderInfo(){
    ...
    h+='<div class="data-status">'+
      ...
      (function(){const o=state.idx.oddsMeta||{count:0};const tot=(o.total!=null?o.total:o.count)||0;const cls=tot?'ok':'warn';const lbl=tot?((o.count||0)+' lines \u00b7 '+(o.altCount||0)+' ladder'+(o.sample?' (sample)':'')+(o.fromPaste?' \u00b7 pasted':'')):'none loaded';return row('Odds / lines', lbl, cls);})()+
  ...
  function _pcDefaultLine(name,stat){
    ...
    const p=state.idx.byName[name]; if(!p) return 1;
    // MLB (leadersPerGame) stores SEASON TOTALS -> convert to per-game; others are already per-game
    const perGame=(window.SPORT_CONFIG&&SPORT_CONFIG.leadersPerGame)?((p[stat]||0)/(p.matches||1)):(p[stat]||0);
    if(window.SPORT_CONFIG&&SPORT_CONFIG.leadersPerGame) return Math.max(0.5,Math.round(perGame-0.5)+0.5);  // snap to X.5 prop line
    return Math.max(1,Math.round(perGame));
  }
