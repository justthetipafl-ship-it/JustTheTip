...
empty scaffold is written. Wire a secondary fixtures feed (or edit by hand)
for upcoming-match focused fixtures.

Scope (Phase 1): T20 (international IT20 + league IPL/BBL) + all international
formats (ODI, Test). Edit SOURCES to widen.
...
# Cricsheet competition zips → (level, comp display name)
SOURCES = {
    "https://cricsheet.org/downloads/t20s_json.zip":  ("INTL", None),  # IT20s
    "https://cricsheet.org/downloads/odis_json.zip":  ("INTL", None),
    "https://cricsheet.org/downloads/tests_json.zip": ("INTL", None),
    "https://cricsheet.org/downloads/ipl_json.zip":   ("LEAGUE", "Indian Premier League"),
    "https://cricsheet.org/downloads/bbl_json.zip":   ("LEAGUE", "Big Bash League"),
}
...
def parse_match(info, innings, mid, src_level, src_comp):
    ...
    for inn_i, inn in enumerate(innings, start=1):
        ...
        for over in inn.get("overs", []):
            ovn = over.get("over", 0)
            phase = "PP" if ovn < 6 else ("Death" if ovn >= 16 else "Mid") if fmt == "T20" else "Mid"
...
def main():
    ...
    rows = []
    for url, (level, comp) in SOURCES.items():
        try:
            z = fetch_zip(url)
        ...
        for n in names:
            ...
            rows.extend(parse_match(info, m.get("innings", []), mid, level, comp))
            kept += 1
        log(f"  {url.split('/')[-1]}: {kept} matches in window")
    ...
    for (team, fmt, level), s in teams_stat.items():
        ...
        teams_out.setdefault(team, {})[f"{fmt}/{level}"] = {
            ...
            "bowlEcon": round(s["wr"] / max(1, s["wb"]) * 6, 2),
    ...
    with open(os.path.join(OUT, "version.txt"), "w") as f:
        f.write(ver)
    log(f"DONE rows={len(rows)} matches={len({r['matchId'] for r in rows})} teams={len(teams_out)}")
