/* ============================================================================
   JTT NCAAF — scoring.js (window.JTTScoring)
   Ported from the NFL engine; same public surface, college signal matrix v1.

   College adaptations:
     • touchShare replaces snapPct everywhere. The representative-game filter
       is RELATIVE: a game counts if the player's touch share that week is at
       least 40% of their own median share (floor 3%) — this drops injury-
       shortened games and garbage-time cameos without a one-size threshold
       that would nuke low-share WR/TE roles.
     • DVP dual baseline: getDVPPct (fires signals) benchmarks vs the FBS-wide
       positional average; getDVPPctConf benchmarks vs the opponent's own
       conference — surfaced in Check My Bet so a MAC "soft" defence reads as
       conference-relative, not confused with an SEC one.
     • No targets/tgtShare/aDot (unreliable in college charting): reception
       volume + avg-long-reception proxies carry those signals.
     • ENV: league-average game total re-centred to the college number (54);
       QB status comes from the injury report only (no depth charts); rest
       edge kept (bye weeks) — computed by the shell from results dates.
     • MIN_AVG gates recalibrated from real 2024 FBS per-game distributions
       (play-level parquet, 4+ game players): WR rec p50 2.6, recYds p50 33.6;
       RB rushYds p50 33.7; QB passYds p50 160.
   ========================================================================== */
window.JTTScoring = (function () {
  ...
  const DVP_STAT_MAP = {
    ...
    rushRecYds:'rushRecYds', anytimeTd:'anytimeTd', fanPts:'fanPts'
  };
  ...
  const MIN_AVG = {
    passYds:150, passAtt:18, passComp:11, passTds:0.7, passInt:0.35,
  ...
  // Relative touch-share filter (see header). Unknown share kept.
  const TOUCH_FLOOR = 3;
  const _touchThr = {};
  function _thrFor(name, rows){
    if(_touchThr[name]!==undefined) return _touchThr[name];
    const v=rows.map(r=>+r.touchShare).filter(x=>isFinite(x)&&x>0).sort((a,b)=>a-b);
    const med=v.length?v[Math.floor(v.length/2)]:null;
    return (_touchThr[name]=med?Math.max(TOUCH_FLOOR, med*0.4):null);
  }
  function _isValidSnapGame(r, thr){
    if(!r) return true;
    const raw = r.touchShare != null ? r.touchShare : null;
    if(raw == null) return true;
    const t = parseFloat(raw);
    if(isNaN(t) || t <= 0) return true;   // 0 = missed join, not a real DNP
    return thr==null ? true : t >= thr;
  }
  function dvpByName(name){
    const rows=_dvpIdx[name]||[];
    const thr=_thrFor(name, rows);
    return rows.filter(r=>_isValidSnapGame(r, thr));
  }
  ...
  function teamAbbrev(t){ return t; }    // the shell's id->abbr adapter has already run
  ...
  function confOf(team){ const rec=DVP.find(d=>d.team===team); return rec?(rec.conf||null):null; }
  function getDVPAvgConf(conf, posGroup, statKey){
    const recs = DVP.filter(d=>d.pos===posGroup && d.conf===conf);
    if(!recs.length) return null;
    const vals = recs.map(d=>d[statKey]).filter(v=>v!=null);
    return vals.length ? vals.reduce((a,b)=>a+b,0)/vals.length : null;
  }
  // Conference-relative DVP: same defence, benchmarked only against its own
  // conference's positional average. Display context — signals fire FBS-wide.
  function getDVPPctConf(oppTeam, posGroup, statKey){
    const val = getDVP(oppTeam, posGroup, statKey);
    const conf = confOf(oppTeam); if(conf==null) return null;
    const avg = getDVPAvgConf(conf, posGroup, statKey);
    if(val==null || avg==null || avg===0) return null;
    return {pct:((val-avg)/avg)*100, conf};
  }
  ...
  function buildPDPoolAvgs(){
    ...
    const POOL = { recVol:'receptions', touchShare:'touchShare', rzTgt:'rzTgt',
      rzCarry:'rzCarry', glCarry:'glCarry', ypc:'ypc', longRec:'longRec',
  ...
  function aliasPlayer(p){
    p.touches = (+p.rushAtt||0) + (+p.receptions||0);
    return p;
  }
  ...
  function _getContextSignalsInner(p, statType, opp){
    ...
    const E = env || {};
    const total = (E.total!=null) ? ((E.total - 54) / 54) * 100 : null;           // FBS-avg total ~54
    ...
    const qb = E.qbStatus==='Q' ? 25 : (E.qbStatus==='D'||E.qbStatus==='O') ? 40 : null;
    ...
    if(statType==='receptions' && recv){
      addI('Reception Volume','receptions_a',1.2, p.receptions, 'recVol');
    ...
    if(statType==='recYds' && recv){
      addI('Reception Volume','passYds_a',1.1, p.receptions, 'recVol');
      add('Pass Yds Allowed','passYds_a',1.0);
      addOwn('Own Pass Volume','passAtt',0.8);
      addEnv(lblTotal,total,0.6);
      if(pos!=='RB') addI('Deep-Ball Role','passYds_a', pos==='WR'?0.5:0.3, p.longRec, 'longRec');
      addEnv(lblDog,dog,0.5);
    ...
    if(statType==='recTds' && recv){
      ...
      addI('Reception Volume','receptions_a',0.6, p.receptions, 'recVol');
    ...
    if(statType==='rushRecYds' && recv){
      ...
      addI('Touch Share','plays_a',0.8, p.touchShare, 'touchShare');
    ...
    if(statType==='fanPts' && (pos==='QB'||recv)){
      addI('Touch Share','plays_a',0.9, p.touchShare, 'touchShare');
  ...
  function cmpFactors(p, statKey, line, opp, lbl){
    ...
    if(dvpPct!==null){
      ...
      const dc=getDVPPctConf(teamAbbrev(opp),dvpPos,dvpSk);
      if(dc&&Math.abs(dc.pct-dvpPct)>=8) push('neutral',`Within the ${dc.conf}: ${dc.pct>=0?'+':''}${dc.pct.toFixed(0)}% vs conference avg — ${dc.pct>=5?'still soft':dc.pct<=-5?'tougher':'about average'} for their level`);
    }
  ...
  function configure(ctx){
    ...
    Object.keys(_touchThr).forEach(k=>delete _touchThr[k]);
  }

  return {
    configure, scoreCMP, cmpFactors, getContextSignals, scoreOverLine, scoreUnderLine,
    verdict, drLine, getDVPPct, getDVPPctConf, confOf, muPct, muInfo, getL5Avg, getRecentAvg, getHitRate,
