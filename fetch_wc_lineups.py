#!/usr/bin/env python3
"""
fetch_wc_lineups.py — matchday lineup poller. Runs every ~15 minutes during
matchday windows to catch lineup announcements (~60-90 mins before kick-off).

WHAT IT DOES:
    1. Reads worldcup_fixtures_2026.json.
    2. Filters to fixtures starting in the next 6 hours (so we catch lineups
       as soon as the API has them, but don't waste calls on next-week games).
    3. For each, calls /fixtures/lineups?fixture=X.
    4. Merges results into wc_lineups.json (preserving historical data).
    5. Bumps version.txt for cache busting.

CALL BUDGET:
    Per matchday window, ~8 fixtures × ~15 polls (every 15 mins over 4 hours)
    = ~120 calls per matchday. Daily quota is 7500, no concern.
...
def main():
    ...
    with open("worldcup_fixtures_2026.json") as f:
    ...
    for fx in upcoming:
        mid = fx["matchId"]
        # Cache by (matchId, hour) so within an hour we don't re-hit the API
    ...
    if updated > 0:
        ...
        print(f"\nUpdated {updated} fixtures with lineups. Saved wc_lineups.json.")
    else:
