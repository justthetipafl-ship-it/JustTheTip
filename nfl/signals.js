  ...
  function create(deps) {
    ...
    function formAlerts(dir) {
      ...
      (players || []).filter(function (p) { return teams.has(p.team); }).forEach(function (p) {
        ...
        // early season: nobody has 3 current-season games yet, so use the most recent games
        // across seasons instead of returning an empty board for the first month
        if (cur.length < ALERT_MIN_L3) {
          cur = (logsFor(p.name) || []).slice(-10).reverse();
    ...
    function usageTrend() {
      ...
      (players || []).filter(function (p) { return teams.has(p.team) && (p.matches || 0) >= 3; }).forEach(function (p) {
        ...
        if (logs.length < USAGE_MIN_G) {
          logs = (logsFor(p.name) || []).slice(-12);        // early season: recent games, any year
    ...
    // Blends four things: the rusher's own production, how often the opposing OFFENCE gives up
    // sacks, how often the opposing QB takes them, and whether the game script asks that QB to
    // drop back a lot (trailing teams throw more, which means more rush opportunities).
    var SACK_POS = (typeof Set !== 'undefined') ? new Set(['DL', 'LB', 'DB'])
      : { has: function (x) { return x === 'DL' || x === 'LB' || x === 'DB'; } };
    function _teamRow(t) { var tm = teamMap ? teamMap() : null; return (tm && tm[t]) ? tm[t] : null; }
    function _leagueAvg(key) {
      var tm = teamMap ? teamMap() : null; if (!tm) return null;
      var v = Object.keys(tm).map(function (k) { return +tm[k][key]; }).filter(function (x) { return isFinite(x); });
      return v.length ? v.reduce(function (a, b) { return a + b; }, 0) / v.length : null;
    }
    function sackArtists() {
      var teams = _fixtureSet(), out = [];
      var lgAllow = _leagueAvg('sacks_a');
      (players || []).filter(function (p) {
        return teams.has(p.team) && SACK_POS.has(p.position) && (p.matches || 0) >= 3 && _available(p);
      }).forEach(function (p) {
        var logs = logsFor(p.name) || [];
        var vals = logs.map(function (r) { return +r.defSacks || 0; });
        if (vals.length < 4) return;
        var avg = _avg(vals); if (avg < 0.3) return;                       // needs real production
        var l5 = _avg(vals.slice(-5));
        var opp = nextOpp(p.team); if (!opp) return;
        var oRow = _teamRow(opp);
        var allowed = oRow ? +oRow.sacks_a : null;                          // sacks that offence concedes
        var mult = (allowed != null && lgAllow) ? Math.max(0.6, Math.min(1.7, allowed / lgAllow)) : 1;
        var games = vals.filter(function (v) { return v > 0; }).length;
        var hitRate = games / vals.length;
        var posted = oddsFor(p.name, 'sacks');
        var line = (posted && posted.line != null) ? posted.line : 0.5;
        var hit = vals.filter(function (v) { return v > line; }).length / vals.length;
        var score = avg * 40 * mult + l5 * 20 + hit * 25 + (+p.tfl || 0) * 3;
        out.push({ p: p, opp: opp, avg: avg, l5: l5, hitRate: hitRate, hit: hit, line: line,
                   allowed: allowed, mult: mult, n: vals.length, posted: !!(posted && posted.line != null),
                   price: posted ? posted.over : null, book: posted ? posted.book : null,
                   tfl: +p.tfl || 0, snap: +p.snapPct || 0, score: score });
      });
      return out.sort(function (a, b) { return b.score - a.score; });
    }
    function sackCard(c) {
      var q = esc(c.p.name).replace(/'/g, "\\'");
      var img = (typeof window !== 'undefined' && window.playerImg) ? window.playerImg(c.p.name, 34) : '';
      var chips = '<span class="lu-p" style="color:#22c55e;border-color:#22c55e55">' + c.avg.toFixed(2) + ' sacks/g</span>'
        + '<span class="lu-p">' + Math.round(c.hit * 100) + '% over ' + c.line + '</span>'
        + (c.allowed != null ? '<span class="lu-p"' + (c.mult > 1.15 ? ' style="color:#22c55e;border-color:#22c55e55"' : '') + '>'
            + abbr(c.opp) + ' allows ' + c.allowed.toFixed(1) + '/g</span>' : '')
        + (c.tfl ? '<span class="lu-p">' + c.tfl.toFixed(1) + ' TFL/g</span>' : '')
        + (c.price ? '<span class="lu-p">$' + (+c.price).toFixed(2) + (c.book ? ' ' + esc(bookName(c.book)) : '') + '</span>' : '');
      return '<div class="lc-card" onclick="openPlayer(\'' + q + '\')">'
        + '<div class="lc-hd">' + img + '<span class="lc-nm">' + esc(c.p.name) + '</span>' + _degBadges(c.p.name)
        + '<span class="lc-meta">' + posShort(c.p.position) + ' \u00b7 ' + abbr(c.p.team) + ' v ' + abbr(c.opp)
        + ' \u00b7 L5 ' + c.l5.toFixed(2) + (c.posted ? '' : ' \u00b7 est line') + '</span></div>'
        + '<div class="lu-grid" style="gap:5px">' + chips + '</div></div>';
    }

    // ===== PICK WATCH — QBs into a defence that takes the ball away =====
    // INT props are noisy, so this leans on rate-per-attempt rather than raw totals, and pairs
    // it with the defence's own takeaway rate plus how much volume the script forces.
    function pickWatch() {
      var teams = _fixtureSet(), out = [];
      var lgTake = _leagueAvg('passInt_a');
      (players || []).filter(function (p) {
        return teams.has(p.team) && p.position === 'QB' && (p.matches || 0) >= 3 && _available(p);
      }).forEach(function (p) {
        var logs = logsFor(p.name) || [];
        var ints = logs.map(function (r) { return +r.passInt || 0; });
        var atts = logs.map(function (r) { return +r.passAtt || 0; });
        if (ints.length < 4) return;
        var attAvg = _avg(atts); if (attAvg < 15) return;                   // needs to be the starter
        var intAvg = _avg(ints);
        var rate = attAvg ? intAvg / attAvg : 0;                           // INT per attempt
        var opp = nextOpp(p.team); if (!opp) return;
        var oRow = _teamRow(opp);
        var takeaway = oRow ? +oRow.passInt_a : null;                       // INTs this defence forces
        var mult = (takeaway != null && lgTake) ? Math.max(0.6, Math.min(1.8, takeaway / lgTake)) : 1;
        var posted = oddsFor(p.name, 'passInt');
        var line = (posted && posted.line != null) ? posted.line : 0.5;
        var hit = ints.filter(function (v) { return v > line; }).length / ints.length;
        if (intAvg < 0.35 && mult < 1.1) return;                            // not a live angle
        var score = rate * 900 * mult + hit * 30 + intAvg * 15;
        out.push({ p: p, opp: opp, intAvg: intAvg, rate: rate, attAvg: attAvg, hit: hit, line: line,
                   takeaway: takeaway, mult: mult, n: ints.length,
                   posted: !!(posted && posted.line != null), price: posted ? posted.over : null,
                   book: posted ? posted.book : null, score: score });
      });
      return out.sort(function (a, b) { return b.score - a.score; });
    }
    function pickCard(c) {
      var q = esc(c.p.name).replace(/'/g, "\\'");
      var img = (typeof window !== 'undefined' && window.playerImg) ? window.playerImg(c.p.name, 34) : '';
      var chips = '<span class="lu-p" style="color:#f59e0b;border-color:#f59e0b55">' + c.intAvg.toFixed(2) + ' INT/g</span>'
        + '<span class="lu-p">' + (c.rate * 100).toFixed(1) + '% of att</span>'
        + '<span class="lu-p">' + Math.round(c.hit * 100) + '% over ' + c.line + '</span>'
        + (c.takeaway != null ? '<span class="lu-p"' + (c.mult > 1.15 ? ' style="color:#22c55e;border-color:#22c55e55"' : '') + '>'
            + abbr(c.opp) + ' forces ' + c.takeaway.toFixed(1) + '/g</span>' : '')
        + (c.price ? '<span class="lu-p">$' + (+c.price).toFixed(2) + (c.book ? ' ' + esc(bookName(c.book)) : '') + '</span>' : '');
      return '<div class="lc-card" onclick="openPlayer(\'' + q + '\')">'
        + '<div class="lc-hd">' + img + '<span class="lc-nm">' + esc(c.p.name) + '</span>' + _degBadges(c.p.name)
        + '<span class="lc-meta">QB \u00b7 ' + abbr(c.p.team) + ' v ' + abbr(c.opp) + ' \u00b7 ' + c.attAvg.toFixed(0)
        + ' att/g' + (c.posted ? '' : ' \u00b7 est line') + '</span></div>'
        + '<div class="lu-grid" style="gap:5px">' + chips + '</div></div>';
    }
    ...
    function _scriptFor(team) {
      ...
      for (var i = 0; i < mo.length; i++) {
        ...
        var spread = null;
        if (g.line) {
          spread = isHome ? +g.line.home : +g.line.away;        // negative = favourite
        } else if (g.h2h && g.h2h.home && g.h2h.away) {
          // no spread posted - infer one from the 2-way moneyline. De-vig, then map the
          // favourite's win probability onto a points margin (roughly 25 pts per 100% swing).
          var ph = 1 / (+g.h2h.home), pa = 1 / (+g.h2h.away), tot = ph + pa;
          if (!(tot > 0)) return null;
          var pHome = ph / tot;
          var margin = (pHome - 0.5) * 25;                      // + = home favoured
          spread = isHome ? -margin : margin;
        } else return null;
    ...
    var tiles = {
      ...
      sacks: function () {
        var rows = _byGame(_fc(sackArtists(), 6, 30), function (c) { return c.p.team; }, sackCard);
        return degWrap('ti-bolt-off', 'Sack Artists', rows, 'c-purple');
      },
      picks: function () {
        var rows = _byGame(_fc(pickWatch(), 6, 30), function (c) { return c.p.team; }, pickCard);
        return degWrap('ti-hand-stop', 'Pick Watch', rows, 'c-amber');
      },
      shootout: function () {
